...
export const connectedApp = pgTable(
	"connected_app",
	{
	...
	(table): PgTableExtraConfigValue[] => [
		...
		foreignKey({
			name: "connected_app_declared_revision_fk",
			columns: [table.id, table.declaredRevision],
			foreignColumns: [connectedAppRevision.appId, connectedAppRevision.revision],
		}).onDelete("restrict"),
		check(
			"connected_app_version_check",
			sql`${table.version} between 0 and 9007199254740991 and (${table.declaredRevision} is null or ${table.declaredRevision} between 1 and ${table.version})`,
		),
		check(
			"connected_app_epoch_check",
			sql`${table.authorityEpoch} between 0 and ${table.version} and (${table.version}=0 or ${table.authorityEpoch}>0)`,
		),
		check(
			"connected_app_state_check",
			sql`(${table.state}='draft' and ${table.version}=0 and ${table.declaredRevision} is null) or (${table.state} in ('active','disabled','retired') and ${table.version}>0 and ${table.declaredRevision} is not null)`,
		),
		check(
			"connected_app_trust_check",
			sql`${table.trust} in ('unreviewed','trusted','blocked') and (${table.trust}<>'blocked' or ${table.state} in ('disabled','retired'))`,
		),
	],
);

/** Private App control receipt; trust authority is separate from publisher authority. @internal */
export const connectedAppEvent = pgTable(
	"connected_app_event",
	{
		appId: uuid()
			.notNull()
		...
		operation: text()
			.$type<"create" | "revise" | "disable" | "enable" | "retire" | "set-trust">()
			.notNull(),
	...
	(table): PgTableExtraConfigValue[] => [
		...
		foreignKey({
			name: "connected_app_event_retained_revision_fk",
			columns: [table.appId, table.retainedDeclaredRevision],
			foreignColumns: [connectedAppRevision.appId, connectedAppRevision.revision],
		}).onDelete("restrict"),
		check(
			"connected_app_event_version_check",
			sql`${table.version} between 1 and 9007199254740991`,
		),
		check(
			"connected_app_event_epoch_check",
			sql`${table.authorityEpochAfter} between 1 and ${table.version}`,
		),
		check("connected_app_event_digest_check", sql`${table.requestDigest} ~ '^[0-9a-f]{64}$'`),
		check(
			"connected_app_event_operation_check",
			inArray(table.operation, ["create", "revise", "disable", "enable", "retire", "set-trust"]),
		),
		check(
			"connected_app_event_state_check",
			inArray(table.stateAfter, ["active", "disabled", "retired"]),
		),
		check(
			"connected_app_event_trust_check",
			inArray(table.trustAfter, ["unreviewed", "trusted", "blocked"]),
		),
		check(
			"connected_app_event_revision_check",
			sql`(${table.operation} in ('create','revise') and ${table.retainedDeclaredRevision} is null) or (${table.operation} not in ('create','revise') and ${table.retainedDeclaredRevision} is not null and ${table.retainedDeclaredRevision} between 1 and ${table.version}-1)`,
		),
	],
);

/** Sealed declared capabilities; existing approvals retain their own exact selected revision. @internal */
export const connectedAppRevision = pgTable(
	"connected_app_revision",
	{
		appId: uuid().notNull(),
	...
	(table): PgTableExtraConfigValue[] => [
		...
		foreignKey({
			name: "connected_app_revision_event_fk",
			columns: [table.appId, table.revision],
			foreignColumns: [connectedAppEvent.appId, connectedAppEvent.version],
		}).onDelete("restrict"),
		check(
			"connected_app_revision_label_check",
			sql`length(btrim(${table.label}))>0 and octet_length(${table.label})<=512`,
		),
		check(
			"connected_app_revision_description_check",
			sql`${table.description} is null or octet_length(${table.description})<=4096`,
		),
		check(
			"connected_app_revision_count_check",
...
export const connectedAppCapability = pgTable(
	...
	(table) => [
		...
		foreignKey({
			name: "connected_app_capability_revision_fk",
			columns: [table.appId, table.revision],
			foreignColumns: [connectedAppRevision.appId, connectedAppRevision.revision],
		}).onDelete("restrict"),
		check(
			"connected_app_capability_registered_check",
			sql`${or(
