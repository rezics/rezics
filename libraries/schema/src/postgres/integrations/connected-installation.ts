...
import { accessRoleBindingRevision } from "../access/access-role-binding";
...
export const connectedInstallation = pgTable(
	"connected_installation",
	{
		...
		state: text()
			.$type<"draft" | "pending" | "active" | "suspended" | "revoked">()
			.notNull()
			.default("draft"),
	},
	(table): PgTableExtraConfigValue[] => [
		...
		uniqueIndex("connected_installation_workload_app_key").on(
			table.workloadPrincipalId,
			table.appId,
		),
		...
		foreignKey({
			name: "connected_installation_workload_scope_fk",
			columns: [table.workloadPrincipalId, table.ownerScopeId],
			foreignColumns: [workloadPrincipal.authUserId, workloadPrincipal.ownerScopeId],
		}).onDelete("restrict"),
		foreignKey({
			name: "connected_installation_approval_fk",
			columns: [table.id, table.approvedRevision],
			foreignColumns: [
		...
		check(
			"connected_installation_version_check",
			sql`${table.version} between 0 and 9007199254740991 and ${table.credentialEpoch} between 0 and ${table.version} and (${table.approvedRevision} is null or ${table.approvedRevision} between 1 and ${table.version})`,
		),
		check(
			"connected_installation_state_check",
			sql`(${table.state}='draft' and ${table.version}=0 and ${table.credentialEpoch}=0 and ${table.approvedRevision} is null) or
...
export const connectedInstallationEvent = pgTable(
	"connected_installation_event",
	{
		installationId: uuid()
			.notNull()
			.references(() => connectedInstallation.id, { onDelete: "restrict" }),
	...
	(table): PgTableExtraConfigValue[] => [
		...
		uniqueIndex("connected_installation_event_operation_key").on(
			table.installationId,
			table.operationId,
		),
		foreignKey({
			name: "connected_installation_event_retained_approval_fk",
			columns: [table.installationId, table.retainedApprovedRevision],
			foreignColumns: [
				connectedInstallationRevision.installationId,
				connectedInstallationRevision.revision,
			],
		...
		check(
			"connected_installation_event_version_check",
			sql`${table.version} between 1 and 9007199254740991 and ${table.credentialEpochAfter} between 1 and ${table.version}`,
		),
		check(
			"connected_installation_event_digest_check",
			sql`${table.requestDigest} ~ '^[0-9a-f]{64}$'`,
		),
		check(
			"connected_installation_event_operation_check",
			inArray(table.operation, ["prepare", "approve", "suspend", "resume", "revoke"]),
		),
		check(
			"connected_installation_event_state_check",
			inArray(table.stateAfter, ["pending", "active", "suspended", "revoked"]),
		),
		check(
			"connected_installation_event_approval_check",
			sql`(${table.operation} in ('prepare','approve') and ${table.retainedApprovedRevision} is null) or
...
export const connectedInstallationRevision = pgTable(
	"connected_installation_revision",
	{
		...
		bindingCount: integer().notNull(),
		bindingDigest: text().notNull(),
		attributionEntityId: uuid().references(() => entityIdentity.id, { onDelete: "restrict" }),
		attributionCount: integer().notNull(),
		attributionDigest: text().notNull(),
		sealed: boolean().notNull().default(false),
	},
	(table): PgTableExtraConfigValue[] => [
		primaryKey({ columns: [table.installationId, table.revision] }),
		foreignKey({
			name: "connected_installation_revision_event_fk",
			columns: [table.installationId, table.revision],
			foreignColumns: [
				connectedInstallationEvent.installationId,
				connectedInstallationEvent.version,
			],
		}).onDelete("restrict"),
		foreignKey({
			name: "connected_installation_revision_owner_fk",
			columns: [table.installationId, table.appId],
			foreignColumns: [connectedInstallation.id, connectedInstallation.appId],
		}).onDelete("restrict"),
		foreignKey({
			name: "connected_installation_revision_app_fk",
			columns: [table.appId, table.appRevision],
			foreignColumns: [connectedAppRevision.appId, connectedAppRevision.revision],
		}).onDelete("restrict"),
		index("connected_installation_revision_app_idx").on(
			table.appId,
			table.appRevision,
			table.installationId,
			table.revision,
		...
		check(
			"connected_installation_revision_time_check",
		...
		check(
			"connected_installation_revision_counts_check",
			sql`${table.capabilityCount} between 0 and ${sql.raw(String(AccessPermissionValues.length + ApiPermissionValues.length))} and ${table.bindingCount} between 0 and 64 and
		...
		check(
			"connected_installation_revision_digests_check",
			sql`${table.capabilityDigest} ~ '^[0-9a-f]{64}$' and ${table.bindingDigest} ~ '^[0-9a-f]{64}$' and ${table.attributionDigest} ~ '^[0-9a-f]{64}$'`,
		),
	],
);

/** Literal installation ceiling; neither an App declaration nor a Role can expand it. @internal */
export const connectedInstallationCapability = pgTable(
	"connected_installation_capability",
	{
		installationId: uuid().notNull(),
		revision: bigint({ mode: "number" }).notNull(),
	...
	(table) => [
		...
		foreignKey({
			name: "connected_installation_capability_revision_fk",
			columns: [table.installationId, table.revision],
			foreignColumns: [
				connectedInstallationRevision.installationId,
				connectedInstallationRevision.revision,
		...
		check(
			"connected_installation_capability_registered_check",
			sql`${or(
...
export const connectedInstallationBinding = pgTable(
	"connected_installation_binding",
	{
		installationId: uuid().notNull(),
		revision: bigint({ mode: "number" }).notNull(),
	...
	(table) => [
		...
		foreignKey({
			name: "connected_installation_binding_revision_fk",
			columns: [table.installationId, table.revision],
			foreignColumns: [
				connectedInstallationRevision.installationId,
				connectedInstallationRevision.revision,
		...
		foreignKey({
			name: "connected_installation_binding_terms_fk",
			columns: [table.bindingId, table.termsRevision],
			foreignColumns: [accessRoleBindingRevision.bindingId, accessRoleBindingRevision.revision],
		}).onDelete("restrict"),
		index("connected_installation_binding_source_idx").on(
			table.bindingId,
			table.termsRevision,
			table.installationId,
			table.revision,
...
export const connectedInstallationAttribution = pgTable(
	"connected_installation_attribution",
	{
		installationId: uuid().notNull(),
		revision: bigint({ mode: "number" }).notNull(),
	...
	(table) => [
		...
		foreignKey({
			name: "connected_installation_attribution_revision_fk",
			columns: [table.installationId, table.revision],
			foreignColumns: [
				connectedInstallationRevision.installationId,
				connectedInstallationRevision.revision,
		...
		foreignKey({
			name: "connected_installation_attribution_terms_fk",
		...
		index("connected_installation_attribution_source_idx").on(
			table.grantId,
			table.termsRevision,
			table.installationId,
			table.revision,
