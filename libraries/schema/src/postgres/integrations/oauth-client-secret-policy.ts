...
export const oauthClientSecretPolicy = pgTable(
	"oauth_client_secret_policy",
	{
		clientId: uuid()
			.primaryKey()
	...
	(table) => [
		index("oauth_client_secret_policy_expiry_idx")
			.on(table.validUntil, table.clientId)
			.where(sql`${table.state}='active'`),
		check(
			"oauth_client_secret_policy_version_check",
			sql`${table.version} between 0 and 9007199254740991`,
		),
		check(
			"oauth_client_secret_policy_state_check",
			sql`(${table.state}='draft' and ${table.version}=0 and ${table.secretDigest} is null and ${table.validFrom} is null and ${table.validUntil} is null) or
...
export const oauthClientSecretEvent = pgTable(
	"oauth_client_secret_event",
	{
		clientId: uuid()
			.notNull()
			.references(() => oauthClientSecretPolicy.clientId, { onDelete: "restrict" }),
	...
	(table) => [
		...
		uniqueIndex("oauth_client_secret_event_material_key")
			.on(table.clientId, table.secretDigest)
			.where(sql`${table.operation} in ('issue','rotate')`),
		check(
			"oauth_client_secret_event_version_check",
			sql`${table.version} between 1 and 9007199254740991`,
		),
		check(
			"oauth_client_secret_event_digest_check",
			sql`${table.requestDigest} ~ '^[0-9a-f]{64}$' and ${table.secretDigest} ~ '^[A-Za-z0-9_-]{43}$'`,
		),
		check(
			"oauth_client_secret_event_operation_check",
			inArray(table.operation, ["issue", "rotate", "retire"]),
		),
		check(
			"oauth_client_secret_event_state_check",
			sql`(${table.operation}='retire' and ${table.stateAfter}='retired') or (${table.operation}<>'retire' and ${table.stateAfter}='active')`,
		),
		check(
			"oauth_client_secret_event_time_check",
