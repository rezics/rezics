...
export const catalogDefinitionRevision = pgTable(
	"catalog_definition_revision",
	{
		...
		constraints: jsonb()
			.$type<import("../../contracts/native/definition").CatalogDefinitionConstraints>()
			.notNull()
	...
	return pgTable(
		...
		(table) => [
			...
			index(`${owner}_identity_public_created_idx`)
				...
				.where(
					sql`${table.status}='published' and ${table.visibility}='public' and ${table.moderationStatus}='approved' and ${table.deletedAt} is null`,
				),
			...
			index(`${owner}_identity_public_shape_created_idx`)
				.on(table.shape, table.createdAt.desc(), table.id.desc())
				.where(
					sql`${table.status}='published' and ${table.visibility}='public' and ${table.moderationStatus}='approved' and ${table.deletedAt} is null`,
				),
			index(`${owner}_identity_public_shape_updated_idx`)
				.on(table.shape, table.updatedAt.desc(), table.id.desc())
				.where(
					sql`${table.status}='published' and ${table.visibility}='public' and ${table.moderationStatus}='approved' and ${table.deletedAt} is null`,
				),
