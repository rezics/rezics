...
import { createCreatedAtColumn, createJsonDocumentColumn } from "../shared/columns";

function editorialTables(owner: CatalogOwner) {
	const current = pgTable(
		`${owner}_editorial`,
		{
			ownerId: uuid()
				.notNull()
		...
		(table) => [
		 ...
		 or (${table.avatarType}='icon' and ${table.avatarIconPrefix} in (${sql.join(
				FontAwesomeIconPrefixValues.map((value) => sql`${value}`),
				sql`,`,
			)}) and ${table.avatarIconPrefix} is not null and ${table.avatarIconName} is not null and ${table.avatarIconName} ~ '^[a-z0-9]+(-[a-z0-9]+)*$' and char_length(${table.avatarIconName}) <= 128 and num_nonnulls(${table.avatarAssetId},${table.avatarEmoji})=0)`,
			),
		],
	);
	const history = pgTable(
		`${owner}_editorial_revision`,
		{
		...
		(table) => [
			...
			foreignKey({
				columns: [table.ownerId, table.language],
				foreignColumns: [current.ownerId, current.language],
				name: `${owner}_editorial_revision_owner_fk`,
			}).onDelete("restrict"),
			check(
				`${owner}_editorial_revision_snapshot_check`,
...
export const CatalogEditorialTables = {
	publishing: editorialTables("publishing"),
	music: editorialTables("music"),
	program: editorialTables("program"),
	software: editorialTables("software"),
	entity: editorialTables("entity"),
	grouping: editorialTables("grouping"),
	reference: editorialTables("reference"),
	distribution: editorialTables("distribution"),
};
export const { current: publishingEditorial, history: publishingEditorialRevision } =
	CatalogEditorialTables.publishing;
export const { current: musicEditorial, history: musicEditorialRevision } =
	CatalogEditorialTables.music;
export const { current: programEditorial, history: programEditorialRevision } =
	CatalogEditorialTables.program;
export const { current: softwareEditorial, history: softwareEditorialRevision } =
	CatalogEditorialTables.software;
export const { current: entityEditorial, history: entityEditorialRevision } =
	CatalogEditorialTables.entity;
export const { current: groupingEditorial, history: groupingEditorialRevision } =
	CatalogEditorialTables.grouping;
export const { current: referenceEditorial, history: referenceEditorialRevision } =
	CatalogEditorialTables.reference;
export const { current: distributionEditorial, history: distributionEditorialRevision } =
	CatalogEditorialTables.distribution;
