...
import { contentStructureRevision } from "../history/content-structure-history";
import {
...
export const unitProgress = pgTable(
	...
	(table) => [
		...
		index("unit_progress_auth_user_seen_idx")
			...
			.where(sql`${table.deletedAt} is null`),
		index("unit_progress_auth_status_seen_idx")
			.on(table.authUserId, table.status, table.lastSeenAt.desc(), table.unitId.desc())
			.where(sql`${table.deletedAt} is null`),
