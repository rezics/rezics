...
import { contentRating } from "../shared/platform-identity";
...
export const contentStructureNode = pgTable(
	...
	(table) => [
		...
		check(
			"content_structure_live_structure_slot_check",
			sql`${table.liveStructureSlot} between 0 and 2047`,
		),
		check(
			"content_structure_live_content_slot_check",
			sql`${table.liveContentSlot} between 0 and 63`,
		),
		uniqueIndex("content_structure_live_structure_slot_key")
			.on(table.structureId, table.liveStructureSlot)
			.where(sql`${table.deletedAt} is null`),
		uniqueIndex("content_structure_live_content_slot_key")
			.on(table.contentUnitId, table.liveContentSlot)
			.where(sql`${table.deletedAt} is null`),
