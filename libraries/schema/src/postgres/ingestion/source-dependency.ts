import { MUSIC_SOURCE_DEPENDENCY_POSITION_LIMIT } from "./source-limits";
...
export const catalogSourceProposalDependency = pgTable(
	...
	(t) => [
		...
		check(
			"catalog_source_dependency_position_check",
			sql`${t.position} between 0 and ${sql.raw(String(MUSIC_SOURCE_DEPENDENCY_POSITION_LIMIT - 1))}`,
		),
		check(
