...
export const accessAssignmentCeiling = pgTable(
	"access_assignment_ceiling",
	{
		id: createUuidv7PrimaryKey(),
		scopeId: uuid()
			.notNull()
			.references(() => accessRoleBindingScope.scopeId, { onDelete: "restrict" }),
		managerBindingId: uuid()
			.notNull()
	...
	(table) => [
		foreignKey({
			name: "access_assignment_ceiling_manager_terms_fk",
			columns: [table.managerBindingId, table.managerTermsRevision],
			foreignColumns: [accessRoleBindingRevision.bindingId, accessRoleBindingRevision.revision],
		}).onDelete("restrict"),
		foreignKey({
			name: "access_assignment_ceiling_group_fk",
		...
		index("access_assignment_ceiling_manager_idx").on(
			table.scopeId,
			table.managerBindingId,
			table.roleId,
			table.id,
		),
		index("access_assignment_ceiling_role_idx").on(table.roleId, table.scopeId, table.id),
		index("access_assignment_ceiling_subject_idx")
			.on(table.recipientSubjectId, table.id)
			.where(sql`${table.recipientSubjectId} is not null`),
		index("access_assignment_ceiling_group_idx")
			.on(table.recipientGroupId, table.id)
			.where(sql`${table.recipientGroupId} is not null`),
		index("access_assignment_ceiling_recipient_scope_idx")
			.on(table.recipientScopeId, table.id)
			.where(sql`${table.recipientScopeId} is not null`),
		check(
			"access_assignment_ceiling_version_check",
			sql`${table.version} between 0 and 2 and ${table.managerTermsRevision} between 1 and 9007199254740991`,
		),
		check(
			"access_assignment_ceiling_state_check",
			sql`(${table.version}=0 and ${table.state}='draft' and not ${table.sealed}) or (${table.version}=1 and ${table.state}='active' and ${table.sealed}) or (${table.version}=2 and ${table.state}='revoked' and ${table.sealed})`,
		),
		check(
			"access_assignment_ceiling_recipient_check",
			sql`
		...
		check(
			"access_assignment_ceiling_path_check",
		...
		check(
			"access_assignment_ceiling_validity_check",
		...
		check(
			"access_assignment_ceiling_grant_lifetime_check",
			sql`(${table.maximumGrantDurationSeconds} is null or ${table.maximumGrantDurationSeconds}>0) and (${table.grantNotAfter} is null or isfinite(${table.grantNotAfter}))`,
		),
		check(
			"access_assignment_ceiling_count_check",
			sql`${table.permissionCount} between 0 and ${sql.raw(String(AccessPermissionValues.length))}`,
		),
		check(
			"access_assignment_ceiling_digest_check",
...
export const accessAssignmentCeilingPermission = pgTable(
	"access_assignment_ceiling_permission",
	{
		ceilingId: uuid()
			.notNull()
			.references(() => accessAssignmentCeiling.id, { onDelete: "restrict" }),
	...
	(table) => [
		...
		check(
			"access_assignment_ceiling_permission_check",
			or(
...
export const accessAssignmentCeilingEvent = pgTable(
	"access_assignment_ceiling_event",
	{
		ceilingId: uuid()
			.notNull()
			.references(() => accessAssignmentCeiling.id, { onDelete: "restrict" }),
	...
	(table) => [
		...
		uniqueIndex("access_assignment_ceiling_event_operation_key").on(
			table.ceilingId,
			table.operationId,
		),
		check(
			"access_assignment_ceiling_event_version_check",
			sql`(${table.operation}='create' and ${table.version}=1) or (${table.operation}='revoke' and ${table.version}=2)`,
		),
		check(
			"access_assignment_ceiling_event_digest_check",
