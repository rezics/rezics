...
export const accessRepresentation = pgTable(
	"access_representation",
	{
		...
		entityId: uuid()
			.notNull()
			.references(() => accessRepresentationEntity.entityId, { onDelete: "restrict" }),
	...
	(table): PgTableExtraConfigValue[] => [
		check(
			"access_representation_target_check",
			sql`(${table.targetKind}='all-scopes' and ${table.targetScopeId} is null) or (${table.targetKind}='scope' and ${table.targetScopeId} is not null)`,
		),
		foreignKey({
		...
		foreignKey({
			...
			columns: [
				...
				table.parentMembershipGeneration,
				table.parentSelectionGroupId,
				table.parentSelectionVersion,
			],
			foreignColumns: [
				accessGroupMembershipEvent.membershipId,
				accessGroupMembershipEvent.generation,
				accessGroupMembershipEvent.groupId,
				accessGroupMembershipEvent.version,
			],
		}).onDelete("restrict"),
		check(
			"access_representation_parent_basis_check",
			sql`
		...
		index("access_representation_group_page_idx")
			.on(table.recipientGroupId, table.id)
			.where(sql`${table.recipientGroupId} is not null`),
		index("access_representation_child_page_idx")
			.on(table.parentGrantId, table.id)
			.where(sql`${table.parentGrantId} is not null`),
		index("access_representation_parent_selection_page_idx")
			.on(table.parentSelectionGroupId, table.id)
			.where(sql`${table.parentSelectionGroupId} is not null`),
		index("access_representation_parent_subject_idx")
			.on(table.parentSubjectId, table.id)
			.where(sql`${table.parentSubjectId} is not null`),
		index("access_representation_parent_admission_idx")
			.on(table.parentMembershipId, table.parentMembershipGeneration, table.id)
			.where(sql`${table.parentMembershipId} is not null`),
		check(
			"access_representation_parent_check",
			sql`(${table.parentGrantId} is null and ${table.parentRevision} is null) or (${table.parentGrantId} is not null and ${table.parentGrantId}<>${table.id} and ${table.parentRevision} is not null and ${table.parentRevision} between 1 and 9007199254740991)`,
		),
		index("access_representation_active_controller_idx")
		...
		index("access_representation_recipient_scope_page_idx")
			.on(table.recipientScopeId, table.id)
			.where(sql`${table.recipientScopeId} is not null`),
		...
		index("access_representation_parent_idx")
			.on(table.parentGrantId, table.parentRevision, table.id)
			.where(sql`${table.parentGrantId} is not null`),
		index("access_representation_subject_page_idx")
			.on(table.recipientSubjectId, table.id)
...
export const accessRepresentationEvent = pgTable(
	...
	(table): PgTableExtraConfigValue[] => [
		...
		check(
			"access_representation_event_digest_check",
...
export const accessRepresentationRevision = pgTable(
	...
	(table) => [
		...
		check(
			"access_representation_revision_target_check",
			sql`(${table.targetKind}='all-scopes' and ${table.targetScopeId} is null and cardinality(${table.targetPath})=0) or (${table.targetKind}='scope' and ${table.targetScopeId} is not null)`,
		),
		foreignKey({
		...
		index("access_representation_selection_page_idx")
			.on(table.selectionGroupId, table.grantId, table.revision)
			.where(sql`${table.selectionGroupId} is not null`),
