...
export const recommendationSnapshot = pgTable(
	...
	(table) => [
		...
		uniqueIndex("recommendation_snapshot_building_key")
			.on(table.state)
			.where(sql`${table.state}='building'`),
		unique("recommendation_snapshot_policy_watermark_key").on(
			table.policyVersion,
			table.sourceWatermark,
		),
		index("recommendation_snapshot_retention_idx")
			.on(table.startedAt, table.id)
			.where(sql`not ${table.active} and ${table.state}<>'building'`),
...
export const recommendationSnapshotPartition = pgTable(
	"recommendation_snapshot_partition",
	{
		snapshotId: uuid()
			.notNull()
		...
		afterBucketStart: createTimestampMsColumn(),
		afterUnitId: uuid(),
		afterKind: text(),
		scannedRows: bigint({ mode: "bigint" }).default(0n).notNull(),
		failures: smallint().default(0).notNull(),
		nextAttemptAt: createTimestampMsColumn().defaultNow().notNull(),
		error: text(),
	},
	(table) => [
		primaryKey({ columns: [table.snapshotId, table.bucket] }),
		index("recommendation_snapshot_partition_claim_idx").on(
			table.snapshotId,
			table.state,
			table.nextAttemptAt,
		...
		check(
			"recommendation_snapshot_partition_values",
			sql`${table.bucket} between 0 and 63 and ${table.state} in ('pending','working','done','failed') and ${table.generation} between 0 and 9007199254740991 and ${table.scannedRows}>=0 and ${table.failures} between 0 and 12 and (${table.error} is null or octet_length(${table.error})<=2048)`,
		),
		check(
			"recommendation_snapshot_partition_lease",
			sql`(${table.state}='working') = (${table.leaseToken} is not null and ${table.leaseExpiresAt} is not null) and num_nonnulls(${table.leaseToken},${table.leaseExpiresAt}) in (0,2)`,
		),
		check(
			"recommendation_snapshot_partition_cursor",
			sql`num_nonnulls(${table.afterBucketStart},${table.afterUnitId},${table.afterKind}) in (0,3)`,
		),
	],
);
...
export const unitBestScore = pgTable(
	...
	(table) => [
		...
		index("unit_best_score_owner_shape_order_idx").on(
			table.snapshotId,
			table.unitOwner,
			table.unitShape,
			table.score.desc(),
			table.unitUpdatedAt.desc(),
			table.unitId.desc(),
		),
		index("unit_best_score_unit_merge_idx").on(table.unitId, table.snapshotId),
		check(
			"unit_best_score_positive_check",
			sql`${table.score} > 0 and ${table.score} < 'Infinity'::double precision`,
		),
	],
);
