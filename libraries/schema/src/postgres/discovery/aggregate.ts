import { publishingTextVersion } from "../publishing/publishing";
...
import { conversation, message } from "../messaging/communication";
import { type ContentLanguage, ContentLanguageValues } from "../shared/contract-values";
import { subjectAssociation } from "../identity/entity";
import { pollOption } from "../forum/poll";
import { post } from "../forum/post";
import { reactionKind } from "../forum/reaction";
import { realm } from "../realms/realm";
import { realmTagContext, tag } from "../knowledge/tag";
import { unitEffectiveTag } from "../knowledge/tag-expression";
import {
	realmTagPath,
	realmUnitTagPathApplication,
	tagPath,
	unitTagPathApplication,
} from "../knowledge/tag-path";
import { unitAlias, unitExternalLink } from "../documents/unit";
...
export const recommendationUnitSignalHourly = pgTable(
	...
	(table) => [
		...
		index("recommendation_unit_signal_hourly_bucket_idx").on(
			table.bucketStart,
			table.unitId,
			table.kind,
		),
		index("recommendation_unit_signal_hourly_positive_idx")
			.on(table.bucketStart, table.unitId, table.kind)
			.where(sql`${table.weight} > 0`),
		check("recommendation_unit_signal_hourly_count_check", sql`${table.signalCount} >= 0`),
		check(
			"recommendation_unit_signal_hourly_weight_check",
			sql`${table.weight} >= 0 and ${table.weight} < 'Infinity'::double precision`,
		),
	],
);
