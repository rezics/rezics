...
export const governanceReportDelivery = pgTable(
	"governance_report_delivery",
	{
		...
		publicNoticePostId: uuid().references(() => governancePostBinding.postId, {
			onDelete: "restrict",
		}),
		actorEntityId: uuid()
			.notNull()
	...
	(table) => [
		uniqueIndex("governance_report_delivery_action_key")
			.on(table.actionId)
			.where(sql`${table.actionId} is not null`),
		uniqueIndex("governance_report_delivery_dismissal_key")
			.on(table.caseId)
			.where(sql`${table.kind} = 'dismissal'`),
		uniqueIndex("governance_report_delivery_notice_key")
			.on(table.publicNoticePostId)
			.where(sql`${table.kind} = 'notice'`),
		index("governance_report_delivery_ready_idx")
			.on(table.shard, table.availableAt, table.id)
			.where(sql`${table.completedAt} is null`),
		index("governance_report_delivery_shard_idx").on(table.shard, table.id),
		index("governance_report_delivery_completed_idx")
		...
		index("governance_report_delivery_case_pending_idx")
			.on(table.caseId)
			.where(sql`${table.completedAt} is null`),
		...
		check(
			"governance_report_delivery_kind_check",
			sql`${table.kind} in ('action','dismissal','notice') and (${table.kind} = 'action') = (${table.actionId} is not null) and (${table.kind} <> 'notice' or ${table.publicNoticePostId} is not null)`,
		),
		check("governance_report_delivery_shard_check", sql`${table.shard} between 0 and 63`),
		check(
			"governance_report_delivery_cursor_check",
			sql`${table.afterReferralId} is null or ${table.afterReferralId} <= ${table.throughReferralId}`,
		),
		check(
			"governance_report_delivery_failure_check",
			sql`${table.failureCount} >= 0 and (${table.lastError} is null or octet_length(${table.lastError}) <= 2048)`,
		),
	],
);

/** Auth-private read receipts avoid corpus-sized ACL arrays on popular moderation notices. */
export const governanceNoticeRecipient = pgTable(
	"governance_notice_recipient",
	{
		postId: uuid()
			.notNull()
			.references(() => governancePostBinding.postId, { onDelete: "restrict" }),
		authUserId: uuid()
			.notNull()
	...
	(table) => [
		primaryKey({
			columns: [table.postId, table.authUserId],
			name: "governance_notice_recipient_pkey",
		}),
