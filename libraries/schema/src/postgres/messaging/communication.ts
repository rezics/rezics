...
export const conversation = pgTable(
	...
	{
		id: createUuidv7PrimaryKey(),
		kind: text().$type<"direct" | "group">().default("direct").notNull(),
		participantLowEntityId: uuid().references(() => entityIdentity.id, { onDelete: "restrict" }),
		participantHighEntityId: uuid().references(() => entityIdentity.id, { onDelete: "restrict" }),
		participantLowAuthUserId: uuid().references(() => users.id, { onDelete: "cascade" }),
		participantHighAuthUserId: uuid().references(() => users.id, { onDelete: "cascade" }),
		createdAt: createCreatedAtColumn(),
	},
	(table) => [
		...
		check(
			...
			sql`(${table.kind}='direct' and num_nonnulls(${table.participantLowEntityId},${table.participantHighEntityId},${table.participantLowAuthUserId},${table.participantHighAuthUserId})=4 and ${table.participantLowAuthUserId} < ${table.participantHighAuthUserId}) or (${table.kind}='group' and num_nonnulls(${table.participantLowEntityId},${table.participantHighEntityId},${table.participantLowAuthUserId},${table.participantHighAuthUserId})=0)`,
		),
	],
);
...
export const message = pgTable(
	...
	(table) => [
		unique("message_id_conversation_key").on(table.id, table.conversationId),
		check(
			"message_revision_positive",
...
export const conversationRead = pgTable(
	...
	(table) => [
		...
		foreignKey({
			name: "conversation_read_last_read_message_id_message_id_fkey",
