...
import { accessRepresentation } from "../access/access-representation";

/** Private idempotent outcome of creating a new controlled identity; never an ownership grant. @internal */
export const accountIdentityAdmission = pgTable(
	"account_identity_admission",
	{
		authUserId: uuid()
			.notNull()
	...
	(table) => [
		...
		index("account_identity_admission_created_idx").on(
			table.authUserId,
			table.createdAt,
			table.operationId,
		),
		check(
			"account_identity_admission_digest_check",
			sql`${table.requestDigest} ~ '^[0-9a-f]{64}$'`,
		),
		check(
			"account_identity_admission_version_check",
			sql`${table.mainPreferenceVersion} is null or ${table.mainPreferenceVersion} between 1 and 9007199254740991`,
		),
	],
);
