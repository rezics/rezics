...
export default {
	...
	progressByType: {
		...
		generic: {
			...
			progressLabel: "完了率",
			dialogDescription:
				"状態と進捗を記録します。これを完了すると、完了回数に1が加算されます。",
			listSummary: insert("{{percent}}% 完了 · 完了 {{count}} 回", {
				percent: Number,
				count: Number,
			}),
			status: "状態",
			...
			totalMinutes: "合計時間（分）",
			history: {
				completion: insert("{{date}} にこれを完了しました。", { date: String }),
				update: insert("{{date}} に {{percent}}% に達しました。", {
					date: String,
					percent: Number,
				}),
			},
			...
			updateAction: "進捗を更新",
			resumeAction: "再開",
			startAgainAction: "もう一度始める",
			restartAction: "最初からやり直す",
			completeAction: "この進捗を完了",
			completedFeedback: insert("完了 {{count}}", { count: Number }),
			statistics: {
				active: insert("現在 {{count}} 人が進行中", { count: String }),
				activeOne: "現在 1 人が進行中",
				backlog: insert("{{count}} 人が開始したい", { count: String }),
				backlogOne: "1 人が開始したい",
			},
			statuses: {
				backlog: "開始したい",
				active: "進行中",
