import { verbatimTerms } from "@rezics/i18n/verbatim-terms";
import { jaTerminology } from "@rezics/i18n/terminology/ja";
import { insert } from "native-i18n";

const { forms: licenseTerms } = jaTerminology.license;
const { forms: postTerms } = jaTerminology.post;
const { forms: videoTerms } = jaTerminology.video;
const { forms: audioTerms } = jaTerminology.audio;
const { forms: realmTerms } = jaTerminology.realm;
const { forms: followTerms } = jaTerminology.follow;
const { forms: metadataTerms } = jaTerminology.metadata;
const { forms: entityTerms } = jaTerminology.entity;

export default {
	nativePublishing: {
		work: jaTerminology.publishingWork.forms.label,
		text_version: jaTerminology.textVersion.forms.label,
		publication: jaTerminology.publication.forms.label,
		serialization: jaTerminology.serialization.forms.label,
		works: jaTerminology.publishingWork.forms.pluralLabel,
		textVersions: jaTerminology.textVersion.forms.pluralLabel,
		publications: jaTerminology.publication.forms.pluralLabel,
		serializations: jaTerminology.serialization.forms.pluralLabel,
		coverage: jaTerminology.publishingCoverage.forms.label,
		installments: jaTerminology.publishingInstallment.forms.pluralLabel,
		publisher: jaTerminology.publisher.forms.label,
		pageCount: "ページ数",
		pagination: "ページ表記",
		editTitle: "タイトルを編集",
		removed: "削除済み",
		noSubparts: "下位の項目はありません。",
		dateText: "記載された日付",
		events: `${jaTerminology.publisher.forms.label}と日付`,
	},
	nativeSemantics: {
		includeInactive: "取り消し済み・置換済みの記録を表示",
		title: "属性と関係",
		facts: "属性",
		relations: "関係",
		openRecord: "項目を開く",
		addFact: "属性を追加",
		addRelation: "関係を追加",
		noFacts: "現在のネタバレ設定で表示できる属性はありません。",
		noRelations: "現在のネタバレ設定で表示できる関係はありません。",
		value: "値",
		item: "要素",
		unknown: "不明",
		trueValue: "はい",
		falseValue: "いいえ",
		chooseValue: "値を選択",
		add: "追加",
		remove: "削除",
		chooseVocabularyValue: "語彙から値を選択",
		valueSelected: "値を選択済み",
		unnamedDefinition: "名前のない定義",
		unnamedTarget: "名前のない項目",
		unavailableTarget: "この参加者は閲覧できません。",
		property: "属性の定義",
		predicate: "関係の定義",
		spoiler: "ネタバレの程度",
		spoilerVisibility: "ネタバレを表示",
		spoilerNone: "ネタバレなし",
		spoilerMinor: "軽度のネタバレ",
		spoilerMajor: "すべて表示",
		invalid: "値、参加者の役割、必要な人数、許可された対象を確認してください。",
		saveFact: "属性を保存",
		saveRelation: "関係を保存",
		largeEdit: "この値は一度に編集できる上限を超えています。ページごとに閲覧できます。",
		incompleteValue: "値全体を読み込めませんでした。再読み込みしてから編集してください。",
		participant: "参加者",
		participants: "参加者",
		role: "役割",
		target: "対象の項目",
		chooseTarget: "既存の項目を検索",
		creditedAs: "クレジット名",
		addParticipant: "参加者を追加",
		addQualifier: "補足属性を追加",
		qualifiers: "補足属性",
		inspectValue: "値を確認",
		useValue: "この値を使う",
		noMatchingValues: "定義とネタバレ設定に一致する値がありません。先に属性を追加してください。",
		history: "改訂履歴",
		restore: "新しい改訂として復元",
		revise: "改訂",
		list: "リスト",
		record: "記録",
		states: {
			active: "有効",
			disputed: "異議あり",
			withdrawn: "取り消し済み",
			superseded: "置換済み",
		},
		transitions: {
			disputed: "異議を示す",
			withdrawn: "取り消す",
			superseded: "置換済みにする",
		},
	},
	nativeSources: {
		previewProfile: "比較する内容",
		rawProfile: "元の応答",
		nativeProfile: "対応付け後の出典表示",
		archiveDigest: "アーカイブの要約値",
		profileCaptureNotice:
			"これらの応答は個別に取得されています。時刻とハッシュは正確な根拠を識別しますが、提供元の同一時点の状態を保証するものではありません。",
		capturedAt: "取得日時",
		captureRequest: "リクエスト先",
		derivedFrom: "派生元",
		title: "出典と更新",
		intake: "出典から取り込む",
		privateDraft:
			"新しい項目は非公開の下書きになります。既存の項目への変更は、確認後に取り込まれます。",
		provider: "出典",
		objectType: "記録の種類",
		externalId: "出典の記録識別子",
		invalidInput: "出典、記録の種類、識別子を確認してください。",
		import: "取り込む",
		openRecord: "項目を開く",
		job: "取り込みの進行状況",
		preparedDependencies: "準備済みの関連記録",
		outcome: "結果コード",
		refresh: "進行状況を更新",
		resume: "再開",
		pause: "一時停止",
		noBindings: "この項目には出典が関連付けられていません。",
		history: "更新履歴",
		bindingSettings: "出典の設定",
		checkSource: "更新を確認",
		propose: "確認用の更新を準備",
		bindingState: "関連付けの状態",
		mode: "更新方法",
		reason: "理由",
		withdrawBindingNotice: "この出典からの今後の更新を停止します。取り込み済みの情報は残ります。",
		save: "保存",
		noProposals: "確認待ちの更新や更新履歴はありません。",
		review: "変更を確認",
		reviewWithdrawal: "取り消しを確認",
		reject: "更新を却下",
		supersede: "後続の更新で置換済みにする",
		sourcePreviewNotice:
			"出典の記録に生じた変更です。適用時には指定の変換処理が実行されるため、項目への実際の変更一覧とは異なります。",
		reviewDetails: "確認の詳細",
		mappingVersion: "変換のバージョン",
		targetRevision: "想定される項目の改訂番号",
		bindingRevision: "想定される関連付けの改訂番号",
		sourceDigest: "出典の検証値",
		noChanges: "この比較で異なる出典の値はありません。",
		withdrawProposalNotice:
			"取り消すと、現在の帰属ルールに従って取り込み済みの寄稿が削除されます。出典の値を確認し、理由を入力してください。",
		apply: "更新を適用",
		withdraw: "寄稿を取り消す",
		before: "変更前",
		after: "変更後",
		readValue: "値の全文を読む",
		absent: "なし",
		changedEvidence: "根拠が選択したプレビューと一致しません。再読み込みしてから判断してください。",
		providers: {
			vndb: verbatimTerms.vndb.value,
			musicbrainz: verbatimTerms.musicbrainz.value,
			bangumi: verbatimTerms.bangumi.value,
			openlibrary: verbatimTerms.openlibrary.value,
		},
		bindingStates: {
			active: "有効",
			paused: "一時停止中",
			withdrawn: "取り消し済み",
		},
		modes: {
			review: "更新を確認",
			manual: "手動更新",
		},
		intakeStates: {
			created: "非公開の下書きを作成しました",
			unchanged: "最新の状態です",
			paused: "更新は一時停止中です",
			review_required: "更新の確認が必要です",
		},
		proposeStates: {
			paused: "更新は一時停止中です",
			unchanged: "確認が必要な更新はありません",
			proposed: "更新の確認ができます",
		},
		proposalStates: {
			pending: "確認待ち",
			applied: "適用済み",
			rejected: "却下済み",
			superseded: "置換済み",
			withdrawn: "取り消し済み",
		},
		jobStates: {
			queued: "待機中",
			prepared: "準備完了",
			paused: "一時停止中",
			succeeded: "完了",
			superseded: "置換済み",
			blocked: "対応が必要",
			failed: "失敗",
		},
		objectTypes: {
			release: "発売記録",
			staff: "制作スタッフ",
			producer: "制作元",
			trait: "特徴",
			quote: "引用",
			work: "作品",
			artist: "アーティスト",
			series: "シリーズ",
			genre: "ジャンル",
			url: "ウェブアドレス",
			subject: "主題項目",
			index: "リスト",
			author: "著者",
			vn: jaTerminology.visualNovel.forms.label,
			recording: jaTerminology.musicRecording.forms.label,
			release_group: jaTerminology.musicReleaseGroup.forms.label,
			person: jaTerminology.entity.forms.personLabel,
			character: jaTerminology.entity.forms.characterLabel,
			label: jaTerminology.entity.forms.labelLabel,
			area: jaTerminology.referenceCatalog.forms.areaLabel,
			place: jaTerminology.referenceCatalog.forms.placeLabel,
			event: jaTerminology.referenceCatalog.forms.eventLabel,
			instrument: jaTerminology.referenceCatalog.forms.instrumentLabel,
			episode: jaTerminology.program.forms.episodeLabel,
			edition: jaTerminology.publication.forms.label,
			tag: "タグ",
		},
	},
	nativeDefinitions: {
		title: "目録用語の定義",
		newDefinition: "定義を作成",
		revise: "新しい版を作成",
		revisionNotice: "既存の記録には、記録時に指定した版の意味が引き続き適用されます。",
		kind: "定義の種類",
		namespace: jaTerminology.definitionNamespace.forms.label,
		key: "固定キー",
		labels: "各言語の名称",
		language: "言語",
		label: "名称",
		description: "説明",
		reason: "定義の理由",
		empty: "条件に一致する定義がありません。",
		noReview: "審査記録なし",
		reviewed: "審査済み",
		choose: "定義を選択",
		selected: "選択した定義",
		constraints: "検証規則",
		valueKind: "値の種類",
		nullable: "不明な値を許可",
		integer: "整数のみ",
		minimum: "最小値",
		maximum: "最大値",
		minLength: "最小文字数",
		maxLength: "最大文字数",
		unit: "計量単位",
		allowedValues: "許可する値",
		targets: "対象",
		shapes: "許可する形態",
		slots: "用途区分",
		roles: "参加者の役割",
		role: "参加者の役割",
		minParticipants: "最小参加者数",
		maxParticipants: "最大参加者数",
		qualifiers: "許可する修飾属性",
		members: "語彙の構成要素",
		vocabulary: "統制語彙",
		rules: "値の構造",
		parent: "親規則",
		memberKey: "項目名",
		root: "最上位の値",
		invalid: "必須項目、範囲、規則の関係を確認してください。",
		save: "審査した定義を保存",
		history: "版の履歴",
		details: "定義の詳細",
		add: "追加",
		remove: "削除",
		any: "すべての種類",
		namespaceHint: `完全な${jaTerminology.definitionNamespace.forms.inline}を入力して絞り込めます。`,
		noCurrent: "この定義にはまだ版がありません。",
		kinds: {
			class: "分類",
			property: "属性",
			predicate: "関係",
			role: "参加者の役割",
			vocabulary: "語彙",
		},
		valueKinds: {
			null: "空の値",
			string: "文字列",
			number: "数値",
			boolean: "真偽値",
			object: "複合項目",
			array: "一覧",
		},
	},
	nativeMusic: {
		music: jaTerminology.music.forms.label,
		recording: jaTerminology.musicRecording.forms.label,
		track: jaTerminology.musicTrack.forms.label,
		medium: jaTerminology.musicMedium.forms.label,
		release: jaTerminology.musicRelease.forms.label,
		release_group: jaTerminology.musicReleaseGroup.forms.label,
		work: jaTerminology.musicalWork.forms.label,
		release_candidate: `未照合の${jaTerminology.musicRelease.forms.inline}`,
		number: "番号",
		position: "順序",
		duration: "長さ（ミリ秒）",
		barcode: "バーコード",
		language: "言語",
		script: "文字体系",
		history: "変更履歴",
		restore: "この版を復元",
		remove: "削除",
		addMedium: `${jaTerminology.musicMedium.forms.inline}を追加`,
		addTrack: `${jaTerminology.musicTrack.forms.inline}を追加`,
		emptyMedia: `${jaTerminology.musicMedium.forms.inline}はまだありません`,
		emptyTracks: `${jaTerminology.musicTrack.forms.inline}はまだありません`,
		deleted: "削除済み",
		current: "現在",
		video: "映像",
		addName: "名前を追加",
	},
	nativeCatalog: { names: "名称", identifiers: "識別子" },
	nativeDomain: {
		any: "すべて",
		yes: "はい",
		no: "いいえ",

		duration: "長さ（ミリ秒）",
		details: metadataTerms.label,
		history: "変更履歴",
		restore: "この版に復元",
		cancel: "キャンセル",
		language: "言語",
		development: "開発状況",
		unknown: "未指定",
		inDevelopment: "開発中",
		finished: "完了",
		cancelled: "中止",
		notes: "備考",
		evidence: "この版を区別する根拠",
		minimumAge: "対象年齢",
		productCode: "製品番号",
		year: "年",
		month: "月",
		day: "日",
		mainEpisodes: "本編の話数",
		totalEpisodes: "総話数",
		number: "番号",
		episodeNumber: "話数番号",
		orderNumber: "並び順の番号",
		releases: "利用可能なリリース",
		languages: "言語",
		machineTranslated: "機械翻訳",
		episodes: `${jaTerminology.program.forms.episodeLabel}`,
		emptyReleases: "条件に一致するリリースは登録されていません。",
	},
	types: {
		book: "書籍",
		software: `${jaTerminology.software.forms.label}`,
		media: "メディア",
		series: "シリーズ",
		video: videoTerms.label,
		audio: audioTerms.label,
		release: "リリース",
	},
	creation: {
		workForm: "作品形式",
		commonCategories: "よく使われるカテゴリー",
		commonTopics: "よく使われるトピック",
		workOwnershipLabel: "作品の所有形態",
		ownedWork: "所有作品",
		ownedWorkDescription:
			"あなたのプロフィールがこの作品を所有し、管理します。編集権限は後から権限設定で付与できます。",
		publicWork: "公共作品",
		publicWorkDescription:
			"コミュニティが所有し、ログイン中の全員が編集できます。作成者に特別な権限は付与されません。",
		publicWorkLearnMore: "公共作品とは？",
		publicWorkHelpTitle: "公共作品とは？",
		publicWorkHelpOwnership:
			"公共作品はコミュニティが所有し、共同で管理します。作成しても所有者としての地位や追加権限は付与されず、ログイン中の全員が共通の権限で編集できます。",
		publicWorkHelpScope:
			"単一のプロフィールではなくコミュニティが作品を管理する場合に選択します。所有形態は章本文の表示可否を決定しません。",
		publicWorkHelpAlternative: `${jaTerminology.publishingWork.forms.label}権を保有している場合や、自分で管理しながら特定の相手との共同作業に開放したい場合は「所有作品」を選び、作成後に権限設定で適切な編集権限を付与してください。`,
		entryOwnershipLabel: "項目の所有形態",
		ownedEntry: "自分の項目",
		ownedEntryDescription: "あなたのプロフィールがこの項目を所有し、管理します。",
		publicEntry: "公開項目",
		publicEntryDescription:
			"コミュニティが所有し、ログイン中の全員が編集できます。作成者に特別な権限は付与されません。",
		creditAttributionsTitle: "クレジット",
		creditEntityLabel: insert(`${entityTerms.label} {{number}}`, { number: Number }),
		creditRoleLabel: insert("役割 {{number}}", { number: Number }),
		addCreditAttribution: "クレジットを追加",
		removeCreditAttribution: insert("クレジット {{number}} を削除", { number: Number }),
		creditEntityRequired: `${entityTerms.label}を選択してください。`,
		creditRoleRequired: "役割を選択してください。",
		creditDuplicate: `この${entityTerms.label}には同じ役割がすでに設定されています。`,
		creditRequestConfirmationTitle: "クレジット招待を確認",
		creditRequestConfirmationDescription: `選択した一部の${entityTerms.plural}は、クレジットを表示する前に同意が必要です。確認すると作品を作成し、招待を送信します。`,
		creditRequestConfirmationCancel: "キャンセル",
		creditRequestConfirmationConfirm: "作成して招待を送信",
		metadataOnlyConfirmationTitle: "完全なコンテンツを提供しますか？",
		metadataOnlyConfirmationDescription: `「${metadataTerms.label}のみ」をオフにすると、${verbatimTerms.rezics.value} で作品の完全なコンテンツを表示できるようになります。オープン${licenseTerms.label}、パブリックドメイン、または権利者からの直接の許諾がある場合に適しています。この設定と選択した${licenseTerms.label}は申告にすぎず、${verbatimTerms.rezics.value} は申告者が必要な権利を持つことを確認していません。`,
		metadataOnlyConfirmationCancel: `${metadataTerms.label}のみを維持`,
		metadataOnlyConfirmationConfirm: "完全なコンテンツを提供",
		versionRole: "バージョン種別",
		mainVersion: "メインバージョン",
		variantVersion: "派生バージョン",
		mainVersionEntity: "関連するメインバージョン",
	},
	ownershipClaim: {
		action: "このユニットの所有権を申請",
		pendingAction: "所有権を審査中",
		title: "ユニット所有権の申請",
		description:
			"このユニットが示す人物、組織、ブランド、または作品を代表する権限がある場合に申請してください。",
		detailsLabel: "申請の根拠",
		detailsPlaceholder:
			"このユニットとの関係と、プラットフォームが確認できる方法を説明してください。",
		detailsHint:
			"代表関係または権利関係を確認できる公開情報を提示してください。機密性の高い個人情報は送信しないでください。",
		effectNotice: `承認されると、この ${verbatimTerms.rezics.value} ユニットの管理上の所有権があなたのプロフィールに移ります。本人認証や、${jaTerminology.publishingWork.forms.label}権その他の法的権利の判断ではありません。`,
		submit: "申請を送信",
		cancel: "キャンセル",
		submitted: "申請を送信しました",
		submittedDescription: "プラットフォーム運営による審査後に結果をお知らせします。",
		pendingTitle: "所有権を審査中",
		pendingDescription: "このユニット所有権申請はまだ審査が完了していません。",
		submittedDetails: "送信した申請根拠",
		withdraw: "申請を取り下げる",
		withdrawn: "申請を取り下げました",
		withdrawnDescription: "このユニット所有権申請は審査待ちではなくなりました。",
	},
	attributionRoles: {
		author: "著者",
		"co-author": "共著者",
		translator: "翻訳者",
		illustrator: "イラストレーター",
		editor: "編集者",
		publisher: "発行者",
		letterer: "レタリング担当",
		colorist: "彩色担当",
		developer: "開発者",
		composer: "作曲者",
		designer: "デザイナー",
		director: "監督",
		producer: "プロデューサー",
		writer: "作家",
		actor: "俳優",
		narrator: "ナレーター",
		studio: verbatimTerms.studio.value,
		distributor: "配給者",
	},
	subjectAssociationRoles: {
		primary_character: "主役キャラクター",
		featured_character: "登場キャラクター",
		appears: "登場する",
		about: "情報",
		setting: "設定",
		source_work: "原作",
		canonical_wiki_page: "正典の知識ページ",
		related_subject: "関連項目",
	},
	workspace: {
		title: "ユニットを管理",
		description: `コンテンツ、${metadataTerms.inline}、ユニット間の関係、アクセス、改訂履歴を編集します。`,
		backToUnit: "ユニットに戻る",
		backToOverview: "設定に戻る",
		navigation: "ユニット管理ナビゲーション",
		overview: "すべての管理ツール",
		sections: {
			content: {
				label: "コンテンツ",
				description: "各コンテンツ言語でタイトル、概要、説明、カバーを編集。",
			},
			metadata: {
				label: metadataTerms.label,
				description: `ライフサイクル、表示、評価、${jaTerminology.license.forms.label}、およびタイプ別${metadataTerms.inline}を管理。`,
			},
			relationships: {
				label: "ユニット間の関係",
				description: "クレジット、主題、およびバリアント関係を管理。",
			},
			tags: {
				label: "タグ",
				description: "グローバルタグを注目タグに設定し、優先表示の順序を管理。",
			},
			realms: {
				label: `${realmTerms.label}への公開`,
				description: `${realmTerms.label}ごとの公開、取り下げ、再公開を管理します。`,
			},
			contentStructure: {
				label: "コンテンツ構造",
				description:
					"折りたたみ可能なインデント付きツリーエディターで書籍の章またはメディア項目を管理。",
				developmentBadge: "開発中",
			},
			releases: {
				label: "シリーズリリース",
				description: "このシリーズ内のユニット、順序、およびリリース日を管理。",
			},
			access: {
				label: "アクセス",
				description: "承認対象、公開権限、制限を管理します。",
			},
			history: {
				label: "改訂履歴",
				description: "コンテンツの改訂をレビュー、比較、元に戻す、または復元。",
			},
		},
	},
	realmPublications: {
		title: `${realmTerms.label}への公開`,
		description: `このコンテンツと${realmTerms.pluralLabel}の間に永続的に保持される収録関係を管理します。取り下げても関係は削除されず、公開だけが停止します。`,
		addTitle: `${realmTerms.label}に公開`,
		addDescription: `公開権限とルールを個別に確認するため、${realmTerms.label}を一つずつ追加します。`,
		realmLabel: realmTerms.label,
		add: "公開",
		publicationStateFilter: "公開状態",
		realmStatusFilter: `${realmTerms.label}の管理状態`,
		all: "すべて",
		current: "現在の管理状態",
		publicationStates: { active: "公開中", withdrawn: "取り下げ済み" },
		realmStatuses: {
			pending: "審査待ち",
			visible: "承認済み",
			hidden: "非表示",
			removed: "削除済み",
		},
		effectivelyVisible: `現在${realmTerms.label}に表示されています`,
		notEffectivelyVisible: `現在${realmTerms.label}には表示されていません`,
		empty: "現在の絞り込み条件に一致する公開関係はありません。",
		withdraw: "取り下げる",
		republish: "再公開",
		loadMore: "さらに読み込む",
		unnamedRealm: `名称未設定の${realmTerms.label}`,
		governanceAction: insert("ガバナンス措置：{{action}}", { action: String }),
	},
	series: {
		kind: "シリーズ種類",
		releases: "シリーズ作品",
		addRelease: "作品を追加",
		releaseUnit: "作品",
		releasedOn: "リリース日",
		removeRelease: "シリーズから削除",
		removeReleaseConfirm: insert(
			"{{title}}をこのシリーズから削除しますか？作品自体は削除されません。",
			{ title: String },
		),
		noReleases: "このシリーズにはまだ作品がありません。",
		moveEarlier: "前に移動",
		moveLater: "後に移動",
		viewAllReleases: "シリーズ作品をすべて表示",
	},
	fields: {
		isbn13: verbatimTerms.isbn13.value,
		metadataOnly: `${metadataTerms.label}のみ`,
		metadataOnlyDescription: {
			book: "作品情報、目次、進捗、ディスカッションは保持しますが、ホストされている章本文は表示しません。既存の本文は削除されません。",
			software: `${jaTerminology.software.forms.label}情報、バージョン、進捗、ディスカッションは保持しますが、ホストされているパッケージ、バイナリ、ソースアーカイブは提供しません。既存のファイルは削除されません。`,
			media:
				"メディア情報、目次、進捗、ディスカッションは保持しますが、ホストされているメディアコンテンツは表示しません。既存のコンテンツは削除されません。",
		},
		publicationDate: "出版日",
		pageCount: "ページ数",
		releaseStatus: "作品の更新状況",
		versionLabel: "バージョンラベル",
		parentUnit: "親ユニット",
		mediaKind: "メディア種類",
		releaseDate: "リリース日",
		runtimeMinutes: "再生時間（分）",
		episodeCount: `${jaTerminology.program.forms.episodeLabel}数`,
		seasonCount: `${jaTerminology.program.forms.seasonLabel}数`,
		durationSeconds: "再生時間（秒）",
		adaptedAudio: `差し替え用${audioTerms.label}`,
		adaptedAudioDescription: `この${videoTerms.inline}で元の${audioTerms.inline}の代わりに再生できる${audioTerms.inline}を選択します。`,
		removeAdaptedAudio: `差し替え用${audioTerms.inline}を削除`,
		yes: "はい",
		no: "いいえ",
	},
	releaseStatuses: {
		ongoing: "更新中",
		hiatus: "更新休止中",
		completed: "完結済み",
		cancelled: "打ち切り",
	},
	detail: {
		sections: "ユニットセクション",
		tabs: {
			book: {
				overview: "書籍の詳細",
				contents: "目次",
				tags: "タグ",
				associations: "関連付け",
				reviews: "読者レビュー",
				collections: "コレクション",
				discussion: "ディスカッション",
			},
			media: {
				overview: "メディアの詳細",
				contents: "コンテンツ",
				tags: "タグ",
				associations: "関連付け",
				reviews: "観客レビュー",
				collections: "コレクション",
				discussion: "ディスカッション",
			},
			software: {
				overview: `${jaTerminology.software.forms.label}情報`,
				requirements: "システム要件",
				tags: "タグ",
				associations: "関連付け",
				reviews: "ユーザーレビュー",
				collections: "コレクション",
				discussion: "ディスカッション",
			},
			series: {
				overview: "シリーズ情報",
				releases: "シリーズ作品",
				tags: "タグ",
				associations: "関連付け",
				reviews: "シリーズレビュー",
				collections: "コレクション",
				discussion: "ディスカッション",
			},
		},
		sectionDescriptions: {
			book: {
				overview: "書籍の説明、出版情報、および読書状況。",
				contents: "章構成を閲覧し、読み続ける。",
				tags: "選択した情報源からのグローバルタグと文脈上の評価を確認する。",
				associations: "書籍の主題、シリーズ、主バリアント関係を確認する。",
				reviews: "読者レビューを、任意でスコアを付けて読むまたは書く。",
				collections: "この書籍を含む公開コレクションを閲覧する。",
				discussion: `この書籍を主題として使用している${postTerms.pluralLabel}を閲覧する。`,
			},
			media: {
				overview: "メディアの説明、発売情報、視聴状況。",
				contents: `${videoTerms.label}と${audioTerms.label}の構造を閲覧し、視聴・聴取の進捗を記録する。`,
				tags: "選択した情報源からのグローバルタグと文脈上の評価を確認する。",
				associations: "この作品の主題、シリーズ、主バリアント関係を確認する。",
				reviews: "観客レビューを読むまたは書く。",
				collections: "この作品を含む公開コレクションを閲覧する。",
				discussion: `この作品を主題として使用している${postTerms.pluralLabel}を閲覧する。`,
			},
			software: {
				overview: `${jaTerminology.software.forms.label}の説明、バージョン情報、使用状況。`,
				requirements: "プラットフォームとティアごとのシステム要件を参照してください。",
				tags: "選択した情報源からのグローバルタグと文脈上の評価を確認する。",
				associations: `${jaTerminology.software.forms.label}の主題、シリーズ、メイン–バリアントの関係を確認してください。`,
				reviews: `${jaTerminology.software.forms.label}ユーザーレビューを読むか書き込む。`,
				collections: `この${jaTerminology.software.forms.label}を含む公開コレクションを閲覧する。`,
				discussion: `この${jaTerminology.software.forms.label}を主題として使用している ${postTerms.pluralLabel} を閲覧する。`,
			},
			series: {
				overview: "シリーズの説明、作品情報、総合評価。",
				releases: "シリーズに含まれる作品を順番に閲覧する。",
				tags: "選択した情報源からのグローバルタグと文脈上の評価を確認する。",
				associations: "このシリーズに関連する主題を確認する。",
				reviews: `シリーズ全体のレビューを読み、${postTerms.label}する。`,
				collections: "このシリーズを含む公開コレクションを閲覧する。",
				discussion: `このシリーズを主題として使用している${postTerms.pluralLabel}を閲覧する。`,
			},
		},
		information: "ユニット情報",
		contentLanguages: "コンテンツ言語",
		credits: "クレジット",
		aboutAuthor: "著者について",
		authorStatistics: {
			bookOne: "1冊の本",
			books: insert("{{count}} 書籍", { count: String }),
			followerOne: `1 ${followTerms.follower}`,
			followers: insert(`{{count}} ${followTerms.follower}s`, { count: String }),
		},
		viewAllCredits: "すべてのクレジットを表示",
		backToOverview: "概要に戻る",
		backToDiscussion: "ディスカッションに戻る",
		subjectAssociations: "主題",
		externalLinks: "外部リンク",
		tags: "タグ",
		versions: "バージョン",
		variants: "バリアント",
		variantsDescription: "同じ作品内の直接的なメインとバリアントの関係。",
		viewAssociations: "すべての関連を表示",
		aliases: "別名",
		releasedOn: "リリース日",
		license: licenseTerms.label,
		aiDisclosure: `${verbatimTerms.ai.value} 公開`,
		type: "タイプ",
		updatedAt: "最終更新",
		primary: "主要",
		main: "メイン",
		version: "バリアント",
		noVersions: "他のバージョンは利用できません。",
		noVariants: "他のバリアントは利用できません。",
		noSeriesMemberships: "この作品はシリーズに属していない。",
		noRequirements: "システム要件は利用できません。",
		requirementTier: "要件の階層",
		requirementPlatform: "プラットフォーム識別子",
		requirementSource: "外部リンク識別子",
		requirementNotSpecified: "未指定",
	},
	rating: {
		general: "一般",
		r15: verbatimTerms.r15.value,
		r18: verbatimTerms.r18.value,
		r18g: verbatimTerms.r18g.value,
	},
	aiDisclosure: {
		unknown: "不明",
		none: "なし",
		ai_assisted: `${verbatimTerms.ai.value}支援付き`,
		ai_originated: `${verbatimTerms.ai.value}発信`,
		machine_generated: "機械生成",
	},
	editor: {
		title: "編集ユニット",
		settings: "設定",
		languageCode: "言語コード",
		relationships: "ユニット間の関係",
		creditRole: "クレジットの役割",
		subjectRole: "主題の役割",
		externalUrl: `外部${verbatimTerms.url.value}`,
		credit: "クレジットを追加",
		subjectAssociation: "主題関連を追加",
		contextWikiPost: `任意のコンテキストウィキ ${postTerms.label}`,
		externalLink: "外部リンクを追加",
		tag: "タグを追加",
		version: "バージョンを添付",
		canonicalUnit: "主要ユニット",
		saveSettings: "設定を保存",
		bookDraftScope: "書籍の公開停止範囲",
		bookDraftScopeDescription: `${jaTerminology.chapter.forms.label}は独立した${postTerms.label}です。現在紐づいている公開済み${jaTerminology.chapter.forms.label}も下書きに戻すか、明示的に選択してください。`,
		bookDraftChoose: "公開停止範囲を選択",
		bookDraftBookOnly: "書籍のみ下書きに戻す",
		bookDraftBookAndChapters: `書籍と管理可能な公開済み${jaTerminology.chapter.forms.label}を下書きに戻す`,
		bookChapterDraftQueued: `${jaTerminology.chapter.forms.label}の公開停止処理を予約しました。管理権限のある${jaTerminology.chapter.forms.label}を順次処理します。`,
	},
	relationshipManagement: {
		credits: "クレジット",
		creditsDescription: "この作品の制作、開発、出版に携わった個人や組織を管理します。",
		subjects: "題材",
		subjectsDescription: "作品内で描写、登場、言及される人物、組織、題材を管理します。",
		externalLinks: "外部リンク",
		externalLinksDescription: "作品情報に関連する外部ページや参考資料を管理します。",
		variants: "メイン作品とバリエーション",
		variantsDescription: "この作品の直接のメイン作品とバリエーションの構成を管理します。",
		addCredit: "クレジットを追加",
		addCreditDescription: "クレジットする個人または組織と、その役割を選択します。",
		creditedEntity: "クレジットする個人または組織",
		addSubject: "題材を追加",
		addSubjectDescription: "題材、関係する役割、任意の文脈ページを選択します。",
		subjectEntity: "題材",
		addExternalLink: "外部リンクを追加",
		addExternalLinkDescription: "外部リンクを追加し、必要に応じて出典元を指定します。",
		sourceEntity: "出典元",
		urlPlaceholder: `${verbatimTerms.https.value}://`,
		cancel: "キャンセル",
		confirmRemove: "削除",
		noCredits: "クレジットはまだありません",
		noCreditsDescription: "この作品に携わった個人や組織を追加してください。",
		noSubjects: "題材はまだありません",
		noSubjectsDescription: "作品に登場または関連する人物、組織、題材を追加してください。",
		noExternalLinks: "外部リンクはまだありません",
		noExternalLinksDescription: "作品情報を裏付ける信頼できる公開ページを追加してください。",
		noVariants: "バリエーションはまだありません",
		noVariantsDescription: "この作品はメイン作品で、接続されたバリエーションはありません。",
		pendingRequests: "同意待ち",
		unavailableUnit: "利用できない項目",
		contextPost: insert("文脈：{{title}}", { title: String }),
		removeCreditTitle: "クレジットを削除しますか？",
		removeCreditDescription: insert("この作品のクレジットから{{name}}を削除しますか？", {
			name: String,
		}),
		removeCreditLabel: insert("{{name}}のクレジットを削除", { name: String }),
		removeSubjectTitle: "題材を削除しますか？",
		removeSubjectDescription: insert("この作品の題材から{{name}}を削除しますか？", {
			name: String,
		}),
		removeSubjectLabel: insert("題材「{{name}}」を削除", { name: String }),
		removeExternalLinkTitle: "外部リンクを削除しますか？",
		removeExternalLinkDescription: insert("{{name}}の外部リンクを削除しますか？", {
			name: String,
		}),
		removeExternalLinkLabel: insert("{{name}}の外部リンクを削除", { name: String }),
		openExternalLinkLabel: insert("{{name}}の外部リンクを開く", { name: String }),
		creditAdded: "クレジットを追加しました",
		subjectAdded: "題材を追加しました",
		externalLinkAdded: "外部リンクを追加しました",
		creditRemoved: "クレジットを削除しました",
		subjectRemoved: "題材を削除しました",
		externalLinkRemoved: "外部リンクを削除しました",
		requestConfirmationTitle: "相手の同意が必要です",
		requestConfirmationDescription:
			"この項目は受信する関連付けを管理しています。管理者が確認できるよう、リクエストを送信してください。",
		sendRequest: "リクエストを送信",
		requestSent: "リクエストを送信しました",
		requestSentDescription: "相手が承認すると、この関連付けが表示されます。",
		main: "メイン作品",
		variant: "バリエーション",
		standalone: "独立した作品",
		standaloneDescription: "この作品はメイン作品に接続されておらず、バリエーションもありません。",
		attachMain: "メイン作品を選択",
		changeMain: "メイン作品を変更",
		changeMainDescription:
			"同じ作品種別のメイン作品を選択します。保存時に現在の状態を再確認します。",
		mainUnit: "メイン作品",
		saveMain: "メイン作品を保存",
		mainUpdated: "メイン作品を更新しました",
		detachMain: "メイン作品から切り離す",
		detachMainTitle: "メイン作品から切り離しますか？",
		detachMainDescription:
			"この作品は独立した作品になります。メイン作品と他のバリエーションは変更されません。",
		mainDetached: "メイン作品から切り離しました",
		mainUnavailable: "メイン作品を利用できません",
		mainUnavailableDescription:
			"現在のメイン作品を読み取れないため、この画面から安全に変更できません。",
		promote: "メインに昇格",
		promoteTitle: "バリエーションをメイン作品にしますか？",
		promoteDescription: insert(
			"{{name}}をメイン作品にしますか？この作品と他のバリエーションはその配下へ移動します。",
			{ name: String },
		),
		promoteLabel: insert("{{name}}をメイン作品にする", { name: String }),
		promoted: "バリエーションをメイン作品にしました",
	},
	references: {
		title: "コミュニティ参照情報",
		description:
			"検索用の別名や外部リンクを提案し、投票できます。固定した情報を先に、その後をコミュニティ投票順で表示します。",
		aliasesTitle: "別名",
		aliasesDescription: "この作品を見つけやすくするための別の名称です。",
		externalLinksTitle: "外部リンク",
		externalLinksDescription: "この作品の情報を裏付ける公開ページです。",
		aliasTerm: "別名",
		aliasPlaceholder: "別の名称を入力",
		proposeAlias: "別名を提案",
		proposeExternalLink: "外部リンクを提案",
		sourceEntity: "出典元",
		sourcePlaceholder: "出典元を選択",
		externalUrl: `外部${verbatimTerms.url.value}`,
		urlPlaceholder: `${verbatimTerms.https.value}://`,
		pinned: "固定済み",
		voteSummary: insert("スコア {{score}}・{{count}}票", { score: String, count: String }),
		support: "支持",
		oppose: "反対",
		pin: "固定",
		unpin: "固定解除",
		moveEarlier: "前へ移動",
		moveLater: "後ろへ移動",
		withdraw: "参照情報を取り下げる",
		withdrawConfirm: "この参照情報を取り下げますか？投票履歴は保持されます。",
		aliasProposed: "別名を提案しました",
		externalLinkProposed: "外部リンクを提案しました",
		noAliases: "別名はまだありません。",
		noExternalLinks: "外部リンクはまだありません。",
	},
	contentLanguageSupport: {
		title: "コンテンツ利用言語",
		description:
			"作品を読んだり、視聴・聴取・操作したりできる言語を指定します。項目やコミュニティ、レビューなどの利用可否には影響しません。",
		language: "言語",
		languagePlaceholder: "言語を選択",
		addLanguage: "言語を追加",
		removeLanguage: "言語を削除",
		channelLabel: "対応チャンネル",
		unqualifiedChannels: "チャンネル未指定",
		channels: {
			text: "テキスト",
			audio: audioTerms.label,
			subtitle: "字幕",
			interface: "インターフェース",
		},
		evidenceTitle: "関連項目の言語情報",
		evidenceDescription:
			"直接関連する項目に保存された値を確認できます。採用した値だけが現在の項目に追加されます。",
		loadEvidence: "関連項目を表示",
		loadMoreEvidence: "さらに読み込む",
		noEvidence: "参照できる直接関連項目はありません。",
		adoptEvidence: "採用",
		evidenceSources: {
			parent: "親ユニット",
			main: "メイン版",
			variant: "バリアント",
			release: "リリース",
			occurrence: "コンテンツ項目",
			adapted_audio: `適合${audioTerms.label}`,
		},
	},
	contentLanguages: {
		controlLabel: "コンテンツ言語",
		settings: "言語設定",
		dialogTitle: "コンテンツ言語設定",
		dialogDescription:
			"読者の優先言語が利用できない場合に使用されるフォールバック表示順序を設定します。",
		add: "言語を追加",
		addDescription: "言語を選択して、その必須コンテンツを保存し、このユニットに追加します。",
		pending: "まだ追加されていません",
		drag: "ドラッグして順序を変更",
		moveFirst: "先頭に移動",
		moveUp: "上に移動",
		moveDown: "下に移動",
		moveLast: "最後に移動",
		remove: "言語を削除",
		removeConfirm: insert("このユニットから{{language}}とそのすべてのコンテンツを削除しますか？", {
			language: String,
		}),
		saveOrder: "並び順を保存",
		cancel: "キャンセル",
		noMoreLanguages: "すべての対応言語はすでに追加されています。",
		draftConflictTitle: "サーバー上の内容が変更されました",
		draftConflictDescription:
			"未保存の下書きは保持されていますが、古い版を基にしています。保存前に確認するか、下書きを破棄してサーバー上の最新内容を読み込んでください。",
		discardDraft: "下書きを破棄",
		instructions:
			"スペースキーを押して言語を選択します。矢印キーで移動し、再度スペースキーを押してドロップします。",
		moved: insert("{{language}} を {{count}} の位置 {{position}} に移動しました。", {
			language: String,
			position: Number,
			count: Number,
		}),
	},
	content: {
		title: "目次",
		edit: "コンテンツを編集",
		chapter: "章",
		label: "ラベル",
		structure: "構造",
		root: "(ルート)",
		structureSummary: insert(
			`書籍 {{books}} 件の${jaTerminology.textVersion.forms.label} · {{chapters}} 章 · {{labels}} ラベル`,
			{
				books: Number,
				chapters: Number,
				labels: Number,
			},
		),
		mediaStructureSummary: insert(
			`メディア {{media}} 件の${jaTerminology.program.forms.label} · ${videoTerms.label} {{videos}} 件 · ${audioTerms.label} {{audios}} 件 · ラベル {{labels}} 件`,
			{ media: Number, videos: Number, audios: Number, labels: Number },
		),
		childCount: insert("{{count}} 子項目", { count: Number }),
		totalWordCount: insert("合計 {{count}} 単語", { count: Number }),
		totalCharacterCount: insert("合計 {{count}} 文字", { count: Number }),
		views: "閲覧数",
		actions: "コンテンツ操作",
		selectionActions: "選択項目操作",
		rename: "名前を変更",
		moveToFirst: "このレベルの最初に移動",
		moveToLast: "このレベルの最後に移動",
		move: "移動",
		moveDescription: "子項目の最後に移動するラベルを選択するか、章を直接その下に移動します。",
		mediaMoveDescription:
			"ラベルを選ぶとその子項目の末尾へ、メディア項目を選ぶとその項目の直後へ移動します。",
		addChapter: "章を追加",
		addBook: `${jaTerminology.textVersion.forms.inline}を追加`,
		addMedia: `${jaTerminology.program.forms.inline}を追加`,
		addLabel: "ラベルを追加",
		addVideo: `${videoTerms.inline}を追加`,
		addAudio: `${audioTerms.inline}を追加`,
		addChapterDescription:
			"新しい章を作成するか、既存の章を検索して追加します。完了時にコンテンツ構造も保存されます。",
		chapterOwnership: "章の所有形態",
		addBookDescription: `リンク先の${jaTerminology.textVersion.forms.inline}の内容と編集権限は個別に管理されます。`,
		addMediaDescription: `リンク先の${jaTerminology.program.forms.inline}の内容と編集権限は個別に管理されます。`,
		profileOwnedChapter: "所有する章",
		communityOwnedChapter: "公共の章",
		addLabelDescription:
			"新しいラベルを作成するか、既存のラベルを検索して追加します。完了時にコンテンツ構造も保存されます。",
		addVideoDescription: `新しい${videoTerms.inline}を作成するか、既存の項目を検索して追加します。完了するとコンテンツ構造も保存されます。`,
		addAudioDescription: `新しい${audioTerms.inline}を作成するか、既存の項目を検索して追加します。完了するとコンテンツ構造も保存されます。`,
		addMode: "追加方法",
		createMode: "新規作成",
		attachMode: "既存から追加",
		existingChapter: "既存の章",
		existingBook: `既存の${jaTerminology.textVersion.forms.inline}`,
		existingMedia: `既存の${jaTerminology.program.forms.inline}`,
		existingLabel: "既存のラベル",
		existingMediaItem: "既存のメディア項目",
		searchExistingChapter: "既存の章を検索",
		searchExistingBook: `既存の${jaTerminology.textVersion.forms.inline}を検索`,
		searchExistingMedia: `既存の${jaTerminology.program.forms.inline}を検索`,
		searchExistingLabel: "既存のラベルを検索",
		searchExistingMediaItem: "既存のメディア項目を検索",
		mediaKindFilter: "メディア種別",
		allMediaKinds: "すべて",
		createChapterAndSave: "章を作成して保存",
		createLabelAndSave: "ラベルを作成して保存",
		createMediaItemAndSave: "メディア項目を作成して保存",
		attachChapterAndSave: "章を追加して保存",
		attachBookAndSave: `${jaTerminology.textVersion.forms.inline}を追加して保存`,
		attachMediaAndSave: `${jaTerminology.program.forms.inline}を追加して保存`,
		attachLabelAndSave: "ラベルを追加して保存",
		attachMediaItemAndSave: "メディア項目を追加して保存",
		saveCurrentChangesNotice:
			"この操作により、現在保存されていないコンテンツ構造の変更も保存されます。",
		choosePosition: "構造の位置を選択",
		choosePositionDescription: "子に追加するラベル、またはその直下に挿入する章を選択",
		mediaChoosePositionDescription:
			"ラベルを選ぶとその子項目の末尾へ、メディア項目を選ぶとその項目の直後へ追加します。",
		newChapter: "新しい章",
		newBook: `${jaTerminology.textVersion.forms.inline}を追加`,
		newLabel: "新しいラベル",
		newMediaItem: "メディア項目を追加",
		newVideo: `新しい${videoTerms.inline}`,
		newAudio: `新しい${audioTerms.inline}`,
		newChapterAfter: "下に新しい章",
		newLabelAfter: "下に新しいラベル",
		newVideoAfter: `下に新しい${videoTerms.inline}`,
		newAudioAfter: `下に新しい${audioTerms.inline}`,
		expand: "展開",
		collapse: "折りたたむ",
		multiSelect: "複数選択",
		selectedCount: insert("{{count}} 選択済み", { count: Number }),
		expandAll: "すべて展開",
		collapseAll: "すべて折りたたむ",
		dragHandle: "移動するにはドラッグ",
		noContent: "まだ章がありません。",
		noMediaContent: `${videoTerms.label}または${audioTerms.label}はまだありません。`,
		durationUnknown: "再生時間未設定",
		markMediaItemComplete: "完了としてマーク",
		markMediaItemIncomplete: "未完了に戻す",
		mediaItemComplete: "完了",
		mediaItemIncomplete: "未完了",
		saveDraft: "コンテンツ構造を保存",
		discardDraft: "下書きの変更を破棄",
		unsavedDraft: "保存されていない変更があります",
		draftHint:
			"名前変更と移動は保存するまで下書きに保持されます。コンテンツを作成または追加すると、現在の変更がすべて保存されます。",
		development: "コンテンツ構造エディターは開発中です",
		developmentDescription: `このユニットタイプではまだコンテンツ構造は利用できません。${verbatimTerms.api.value} プレビューアクセスには開発プレビュー機能が必要です。`,
	},
	chapter: {
		title: "章を編集",
		backToStructure: "コンテンツ構造に戻る",
		language: "章の言語",
		useLanguage: "言語を切り替える",
		save: "章を保存する",
		communityUnitContentPolicyTitle: "公共作品では章本文をここで編集できません",
		communityUnitContentPolicyDescription: `この書籍はコミュニティによって管理されています。コンテンツ構造の整理や既存${postTerms.label}の追加はできますが、このエディターには${postTerms.label}本文エディターがありません。`,
		wordCount: insert("{{count}} 単語", { count: Number }),
		characterCount: insert("{{count}} 文字", { count: Number }),
	},
	reader: {
		backToContents: "目次に戻る",
		settings: "リーダー設定",
		fontSize: "フォントサイズ",
		chapterLanguage: "章の言語",
		automaticLanguage: "自動",
		markChapterComplete: "章を完了としてマークする",
		markChapterIncomplete: "章を未完了としてマークする",
		read: "読み取り",
		unread: "未読",
	},
} satisfies typeof import("../zh-Hant/units").default;
