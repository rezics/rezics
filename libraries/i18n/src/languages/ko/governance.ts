...
export default {
	...
	ruleBasis: {
		label: "적용 규칙",
		description: `이 거버넌스 권한에서 사용할 수 있는 규칙 ${realmTerms.label}의 규칙을 하나 이상 선택하세요. 선택 내용은 변경할 수 없는 결정 근거로 기록됩니다.`,
		sourceAriaLabel: `규칙 ${realmTerms.label}`,
		chooseSource: `규칙 ${realmTerms.label} 선택`,
		sources: {
			platform: "플랫폼 공식 규칙",
			realm: `${realmTerms.label} 규칙`,
			local: "로컬 규칙",
		},
		rulesAriaLabel: "적용 규칙",
		chooseRules: "규칙 선택",
		clear: "선택한 규칙 지우기",
		selectedCount: insert("규칙 {{count}}개 선택됨", { count: Number }),
		loading: "규칙을 불러오는 중…",
		loadError: "사용 가능한 규칙을 불러오지 못했습니다. 나중에 다시 시도하세요.",
	},
	...
	capabilities: {
		...
		"platform.api_quota_policy.read": `${verbatimTerms.api.value} 할당량 정책 보기`,
		...
		"realm.tag-voting.update": `${realmTerms.label} 태그 투표 업데이트`,
		"realm.tag-contexts.manage": `${realmTerms.label} 태그 맥락 연결 관리`,
	...
	access: {
		...
		currentRealmMembersLabel: `이 ${realmTerms.label}의 모든 활성 멤버`,
		realmMembersLabel: insert("{{realm}}의 활성 멤버", { realm: String }),
		realmAccessManagersLabel: insert("{{realm}}의 접근 관리자", { realm: String }),
		builtInAudience: "기본 제공 대상",
		subjectsTitle: "인증 주체",
		...
		relinquishOwnership: "소유권 포기",
		transferOwnershipDescription:
			"협업 초대를 수락했고 현재 유닛 전체에 대한 영구 편집 권한이 있는 프로필만 선택할 수 있습니다.",
		searchOwnershipCandidates: "이전 대상 검색…",
		ownershipCandidates: "소유권 이전 대상",
		selectOwnershipCandidate: "선택",
		noOwnershipCandidates: "조건을 충족하는 협업자가 없습니다.",
		confirmTransferTitle: "소유권 이전 확인",
		confirmTransferDescription: insert("‘{{unit}}’의 소유권을 {{profile}}에게 이전할까요?", {
			unit: String,
			profile: String,
		}),
		untitledOwnershipUnit: "제목 없는 유닛",
		transferOwnershipWarning:
			"완료 즉시 소유자 권한을 잃습니다. 되돌리려면 새 소유자가 소유권을 다시 이전해야 합니다.",
		backToOwnershipCandidates: "이전 대상 목록으로",
		confirmTransfer: "이전 확인",
		relinquishOwnershipTitle: "소유권 포기 확인",
		relinquishOwnershipDescription:
			"유닛은 즉시 community@rezics.com 소유가 되며 소유자 권한을 잃습니다.",
		confirmRelinquishment: "커뮤니티에 넘기기",
		...
		subjectRelations: {
			member: `${realmTerms.label} 멤버`,
			access_manager: `${realmTerms.label} 접근 관리자`,
		},
		table: {
			subject: "권한 대상",
			kind: "유형",
		...
		permissions: {
			...
			"realm.tags.manage": `${realmTerms.label} 태그 관리`,
			"realm.tag-voting.update": "태그 투표 업데이트",
			"realm.tag-contexts.manage": "태그 맥락 연결 관리",
