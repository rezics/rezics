...
export default {
	...
	progressByType: {
		...
		generic: {
			...
			progressLabel: "완료율",
			dialogDescription:
				"상태와 진행률을 기록하세요. 이번에 완료하면 완료 횟수가 1 증가합니다.",
			listSummary: insert("{{percent}}% 완료 · 완료 {{count}}회", {
				percent: Number,
				count: Number,
			}),
			status: "상태",
			...
			totalMinutes: "총 시간(분)",
			history: {
				completion: insert("{{date}}에 완료했습니다.", { date: String }),
				update: insert("{{date}}에 {{percent}}%에 도달했습니다.", {
					date: String,
					percent: Number,
				}),
			},
			...
			updateAction: "진행률 업데이트",
			resumeAction: "재개",
			startAgainAction: "다시 시작",
			restartAction: "처음부터 시작",
			completeAction: "이번 진행 완료",
			completedFeedback: insert("완료 {{count}}", { count: Number }),
			statistics: {
				active: insert("현재 {{count}}명이 진행 중입니다", { count: String }),
				activeOne: "현재 1명이 진행 중입니다",
				backlog: insert("{{count}}명이 시작을 원합니다", { count: String }),
				backlogOne: "1명이 시작을 원합니다",
			},
			statuses: {
				backlog: "시작하고 싶음",
				active: "진행 중",
