import { insert } from "native-i18n";
import { zhHansTerminology } from "@rezics/i18n/terminology/zh-Hans";

const { forms: metadataTerms } = zhHansTerminology.metadata;

export default {
	privateFavorites: {
		note: "私人备注",
		remove: "取消收藏",
		save: "私人收藏",
		empty: "还没有收藏。",
		refreshPreview: "更新收藏预览",
		moveFirst: "移到最前",
		history: "历史记录",
		restore: "恢复",
		privacy: "只有你能看到这些收藏和备注。",
	},
	title: "收藏集",
	favorites: "我的收藏",
	newCollection: "创建收藏集",
	createDescription: "创建一个可整理、呈现与分享内容的收藏集。",
	editCollection: "管理收藏集",
	deleteCollection: "删除收藏集",
	deleteCollectionPrompt: "删除后将无法恢复这个收藏集及其编排。",
	emptyCollections: "还没有收藏集。",
	containingUnitEmpty: "还没有收录这个作品的公开收藏集。",
	emptyCollectionTitle: "这个收藏集当前是空的",
	emptyCollectionBody: "加入内容后，这里会使用与动态消息相同的内容卡片呈现。",
	contentLabel: "收藏集内容",
	itemCount: insert("{{count}} 个项目", { count: Number }),
	directCollectionHint: "收藏集只会加入为一个项目，不会递归导入其中的内容。",
	publishers: {
		label: "发行者",
		unknown: "未标注发行者",
		current: "当前发行者",
		currentDescription: "这些个人资料会在收藏集页面与动态消息中获得发行者署名。",
	},
	save: {
		action: "收藏",
		title: "收藏到收藏集",
		directDescription:
			"选择“我的收藏”或任一自定义收藏集。将评论加入自定义收藏集时，如有需要，会先加入被评论对象。",
		searchLabel: "查找收藏集",
		searchPlaceholder: "输入收藏集名称",
		noMatches: "找不到符合的收藏集。",
		noCollections: "尚未创建可加入内容的收藏集。",
		createLabel: "添加收藏集",
		createPlaceholder: "收藏集名称",
		createAndSave: "创建并收藏",
		manage: "管理收藏集",
		saved: "已收藏",
		notSaved: "尚未收藏",
	},
	workspace: {
		title: "收藏集管理",
		description: `管理内容、${metadataTerms.label}、排序、发行者、访问权与版本记录。`,
		navigation: "收藏集管理导航",
		overview: "收藏集管理项目",
		backToCollection: "返回收藏集",
		backToOverview: "返回设置",
		sections: {
			content: {
				label: "内容",
				description: "编辑各内容语言的标题、摘要与封面。",
			},
			metadata: {
				label: metadataTerms.label,
				description: `设置状态与可见性${metadataTerms.inline}，或删除收藏集。`,
			},
			items: {
				label: "内容与排序",
				description: "添加、移除、多选与排序内容。",
			},
			publishers: {
				label: "发行者",
				description: "管理收藏集公开显示的个人资料发行者署名。",
			},
			access: {
				label: "访问权",
				description: "管理授权对象、权限与限制。",
			},
			history: {
				label: "版本记录",
				description: "查看、比较与还原收藏集版本。",
			},
		},
	},
	items: {
		add: "添加内容",
		target: "内容",
		selectAll: "全选已加载项目",
		clearSelection: "清除选择",
		selectedCount: insert("已选择 {{count}} 个项目", { count: Number }),
		selectItem: insert("选择“{{title}}”", { title: String }),
		removeItem: insert("移除“{{title}}”", { title: String }),
		move: "移动",
		moveTitle: "移动所选项目",
		moveDescription: "所选项目的相对顺序会保持不变，变更将一次完成。",
		destination: "移动位置",
		moveToStart: "移到开头",
		moveToEnd: "移到最后",
		moveAfter: "移到指定项目之后",
		afterItem: "前一个项目",
		chooseDestination: "选择项目",
		applyMove: "应用移动",
		empty: "这个收藏集还没有可管理的内容。",
	},
	form: {
		language: "内容语言",
		title: "标题",
		summary: "摘要",
		cover: "封面",
		status: "状态",
		visibility: "可见性",
		save: "保存变更",
	},
	cancel: "取消",
	delete: "删除",
	close: "关闭",
} satisfies typeof import("../zh-Hant/collections").default;
