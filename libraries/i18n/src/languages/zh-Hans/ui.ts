import { insert } from "native-i18n";

import { zhHansTerminology } from "@rezics/i18n/terminology/zh-Hans";

const { forms: followTerms } = zhHansTerminology.follow;
const { forms: postTerms } = zhHansTerminology.post;
const { forms: realmTerms } = zhHansTerminology.realm;
const { forms: zoneTerms } = zhHansTerminology.zone;
const { forms: tagPathTerms } = zhHansTerminology.tagPath;
const { forms: entityTerms } = zhHansTerminology.entity;

export default {
	loading: "正在加载…",
	unnamed: "未命名条目",
	save: "保存",
	submit: "提交",
	create: "创建",
	edit: "编辑",
	clear: "清除",
	search: "搜索",
	shelf: {
		label: "内容轮播区",
		previous: "上一组内容",
		next: "下一组内容",
		page: insert("第 {{page}} 页，共 {{pageCount}} 页", { page: Number, pageCount: Number }),
		item: insert("第 {{item}} 项，共 {{itemCount}} 项", { item: Number, itemCount: Number }),
	},
	pickerPlaceholders: {
		unit: "输入名称或标题",
		entity: `输入${entityTerms.label}名称`,
		tag: "输入标签名称",
		tagPath: `输入${tagPathTerms.label}名称`,
		post: `输入${postTerms.label}标题或关键词`,
		realm: `输入${realmTerms.label}名称`,
		user: "输入用户名",
	},
	title: "标题",
	summary: "摘要",
	description: "说明",
	body: "正文",
	showMore: "显示更多",
	showLess: "显示较少",
	status: "状态",
	visibility: "可见性",
	public: "公开",
	unlisted: "不列出",
	private: "私密",
	draft: "草稿",
	published: "发布",
	archived: "归档",
	favorite: "收藏",
	follow: followTerms.actionLabel,
	followed: followTerms.stateLabel,
	unfollow: followTerms.undoActionLabel,
	join: "加入",
	upvote: "赞成",
	startTracking: "开始记录",
	score: "评分",
	replies: `回复${postTerms.pluralLabel}`,
	postReply: "发布回复",
	writeReply: "撰写回复",
	profile: "个人资料",
	preferences: "偏好设置",
	account: "账户",
	avatar: "头像",
	displayName: "显示名称",
	introduction: "简介",
	language: "接口语言",
	preferredLanguageUnavailable: "这项内容暂不支持你的偏好语言。",
	requestedLanguageUnavailable: "所选的语言版本不可用，已恢复自动选择。",
	timezone: "时区",
	contentRating: "内容分级",
	profileVisibility: "数据可见性",
	saved: "已保存",
	logout: "注销",
	entityKind: "类别",
	person: zhHansTerminology.entity.forms.personLabel,
	organization: zhHansTerminology.entity.forms.organizationLabel,
	character: zhHansTerminology.entity.forms.characterLabel,
	optionsPerLine: "选项（每行一个）",
	chapterTitle: `${zhHansTerminology.chapter.forms.label}标题`,
	chapterContent: `${zhHansTerminology.chapter.forms.label}内容`,
	position: "排序",
	publishChapter: `发布${zhHansTerminology.chapter.forms.label}`,
	previousChapter: "上一章",
	nextChapter: "下一章",
	backToUnit: "返回作品",
	markRead: "标记已读",
	tableOfContents: "目录",
	addChapter: `添加${zhHansTerminology.chapter.forms.label}`,
	reviewTarget: "评论对象",
	reviewFormat: "形式",
	shortReview: "短评",
	longReview: "长评",
	publishReview: "发布",
	collection: "收藏集",
	realm: realmTerms.label,
	zone: zoneTerms.label,
	share: "分享",
	joinPolicy: "加入方式",
	open: "开放",
	approval: "需核准",
	invite: "仅邀请",
	addToCollection: "加入收藏集",
	emptyFavorites: "还没有收藏内容。",
	emptyProgress: "还没有进度记录。",
	retryLater: "请求失败，请稍后重试。",
} satisfies typeof import("../zh-Hant/ui").default;
