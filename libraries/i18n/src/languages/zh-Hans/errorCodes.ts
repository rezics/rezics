import { verbatimTerms } from "@rezics/i18n/verbatim-terms";
import { zhHansTerminology } from "@rezics/i18n/terminology/zh-Hans";

const { forms: audioTerms } = zhHansTerminology.audio;
const { forms: dockTerms } = zhHansTerminology.dock;
const { forms: customThemeTerms } = zhHansTerminology.customTheme;
const { forms: definitionNamespaceTerms } = zhHansTerminology.definitionNamespace;
const { forms: followTerms } = zhHansTerminology.follow;
const { forms: postTerms } = zhHansTerminology.post;
const { forms: realmTerms } = zhHansTerminology.realm;
const { forms: tagPathTerms } = zhHansTerminology.tagPath;
const { forms: unitSlugTerms } = zhHansTerminology.unitSlug;
const { forms: zoneTerms } = zhHansTerminology.zone;
const { forms: entityTerms } = zhHansTerminology.entity;

export default {
	GovernanceDeliveryCapacityExceeded: "通知发送队列繁忙，请稍后重试此治理操作。",
	CatalogSourceRequestLimited: "该来源收到的请求过多，请稍后重试。",
	CatalogSourceUnavailable: "该来源暂时不可用，请稍后重试。",
	RouteNotFound: "未找到请求的页面或资源。",
	UnitGovernanceLookupInvalid: "请输入精确标识符，或填写地址名称及其确切的地址作用域。",
	OrganizationMembershipConflict: "邀请或成员状态已改变，请刷新后重试。",
	OrganizationMembershipNotFound: "找不到此邀请或成员记录。",
	OrganizationMembershipCapacityExceeded: "待处理邀请过多，请先处理部分邀请再发送。",
	CatalogReferenceNotFound: "找不到这条记录。",
	CatalogRevisionConflict: "这条记录已更改，请刷新后重试。",
	FavoriteNotFound: "找不到这条收藏。",
	FavoriteRevisionConflict: "你的收藏已更改，请刷新后重试。",
	ParticipationDenied: "你已无权使用此身份执行操作。",

	MalformedRequestBody: "无法解析提交内容的格式。",
	ValidationError: "提交的内容不符合要求。",
	InternalError: "服务暂时不可用，请稍后重试。",
	AuthenticationRequired: "请先登录再继续。",
	ApiTokenPermissionRequired: `这个 ${verbatimTerms.api.value} 令牌没有运行该操作所需的权限。`,
	ApiTokenRateLimitExceeded: `这个 ${verbatimTerms.api.value} 令牌的请求太频繁，请稍后再试。`,
	ApiQuotaExceeded: `${verbatimTerms.api.value} 配额已用尽，请在可重试时间后再试。`,
	VoteHotKeyBusy: "此投票目标当前繁忙，请稍后重试。",
	ApiQuotaPolicyNotFound: `找不到指定的 ${verbatimTerms.api.value} 配额策略。`,
	ApiQuotaPolicyKeyConflict: `已有 ${verbatimTerms.api.value} 配额策略使用此标识符。`,
	ApiQuotaPolicyInvalid: `${verbatimTerms.api.value} 配额策略配置无效。`,
	ApiQuotaPolicyRevisionConflict: `${verbatimTerms.api.value} 配额策略已更改，请重新加载后再保存。`,
	ApiAccountQuotaRevisionConflict: `账户 ${verbatimTerms.api.value} 配额已更改，请重新加载后再保存。`,
	ApiTokenQuotaRevisionConflict: `${verbatimTerms.api.value} 令牌配额分配已更改，请重新加载后再保存。`,
	ApiTokenLimitReached: `此账户已达到 ${verbatimTerms.api.value} 令牌数量上限。`,
	ApiTokenQuotaOverrideInvalid: `${verbatimTerms.api.value} 令牌的配额覆盖配置无效。`,
	ApiTokenQuotaOverrideRevisionConflict: `${verbatimTerms.api.value} 令牌的配额覆盖已更改，请重新加载后再保存。`,
	InteractiveSessionRequired: "请通过交互式登录后再管理凭证。",
	FreshSessionRequired: "运行敏感管理操作前请重新登录。",
	EmailVerificationRequired: "请先验证电子邮件再继续。",
	AccountSuspended: "该账户目前已停用。",
	AccountClosed: "该账户已关闭。",
	UserAccountStateRevisionConflict: "账户状态已变更，请重新加载后再试。",
	UserSelfStatusChangeForbidden: "不能停用或关闭自己的账户。",
	PlatformUserManagerRequired: "平台必须保留至少一位处于使用状态的用户管理员。",
	UserAccountStateExpiryInvalid: "停用期限必须晚于当前时间。",
	SessionNotFound: "找不到该会话。",
	AccountRestricted: "当前账户受到限制，无法运行该操作。",
	UnitNotFound: "找不到这个内容。",
	UnitPermissionForbidden: "你没有操作这个内容所需的权限。",
	UnitAccessRestricted: "你对这个内容范围的访问受到限制。",
	UnitLicenseGrantForbidden: `只有个人拥有的作品可以向 ${verbatimTerms.rezics.value} 授权。`,
	UnitLicenseNotApplicable: "这份授权不能用在这类内容。",
	UnitLicenseOfferingEndForbidden: "这份授权目前不能结束对外提供。",
	UnitLicenseGrantConflict: "这项内容已有一份尚未结束的相同授权提供。",
	UnitChanged: "内容已变更，请刷新后再试一次。",
	UnitContentLanguageSupportInvalid: "无法保存内容消费语言。请检查语言和支持渠道后再试一次。",
	VideoAudioTrackInvalid: `无法保存所选的替代${audioTerms.inline}。请检查选择后再试一次。`,
	UnitRevisionConflict: "修订基础已变更，请刷新后再试一次。",
	RevisionContributionActorRequired: "请指定这次修订的主要贡献者。",
	RevisionCreditEntityInvalid: "所选的修订贡献者无效，请重新选择。",
	ContentStructureRevisionConflict: "内容结构已在其他地方变更，请刷新后再试一次。",
	CollectionStructureRevisionConflict: "条目结构已在其他地方变更，请刷新后再试一次。",
	DockRevisionConflict: `${dockTerms.label}已在其他地方变更，请刷新后再试一次。`,
	DockNotFound: `找不到这个${dockTerms.inline}。`,
	DockNotSupported: `这个内容不支持该${dockTerms.inline}。`,
	DockDocumentInvalid: `${dockTerms.label}文档无效。`,
	ApiTokenNotFound: `找不到仍然有效的 ${verbatimTerms.api.value} 令牌。`,
	InvalidSearch: "搜索条件不符合要求。",
	SearchUnavailable: "搜索服务暂时无法使用，请稍后再试。",
	RealmCapabilityRequired: `你没有所需的${realmTerms.inline}权限。`,
	RealmRulesAcceptanceRequired: `请先同意当前的${realmTerms.inline}规则。`,
	RealmRuleRevisionChanged: "服务器上的规则已更改，请检查最新版本后重试。",
	PlatformCapabilityRequired: "此操作需要平台权限。",
	PlatformAccessManagerRequired: "平台必须保留至少一位无期限的访问权管理者。",
	PlatformAccessRevisionConflict: "平台权限已由其他变更更新，请重新加载后再试一次。",
	PlatformAccessConfigurationInvalid: "请求的平台访问权限超出了唯一性或容量限制。",
	CollectionOwnershipRequired: "这个收藏集不属于你。",
	ProfileNotFound: "找不到这个用户资料。",
	ProfileChanged: "用户资料已变更，请刷新后再试一次。",
	PreferencesNotFound: "找不到这些偏好设置。",
	UserNotFound: "找不到这个用户。",
	UserSelfFollowForbidden: `不能${followTerms.action}自己。`,
	UserFollowBlocked: `存在封锁关系时不能${followTerms.action}。`,
	FollowingTargetKindMismatch: `${followTerms.collectionLabel}的类型已变更，请重新加载设置。`,
	UserSelfBlockForbidden: "不能封锁自己。",
	SoftwareSystemRequirementSourceInvalid: `系统需求来源必须属于这个${zhHansTerminology.software.forms.label}。`,
	SeriesReleaseNotFound: "找不到这个系列发行项目。",
	ZonePageNotFound: `找不到这个${zoneTerms.inline}页面。`,
	ZonePageInUse: `该${zoneTerms.inline}页面仍被区块或导航文档引用。`,
	ZoneNavigationNotFound: `找不到这个${zoneTerms.inline}导航。`,
	ZoneNavigationInUse: `该${zoneTerms.inline}导航仍被区块文档引用。`,
	ZoneDocumentInvalid: `${zoneTerms.label}的区块或导航文档无效。`,
	ZoneTimeRangeInvalid: `${zoneTerms.label}的结束时间必须晚于开始时间。`,
	ZoneRuleRealmInvalid: `所选${realmTerms.label}没有包含至少一条规则的当前规则版本。`,
	SoftwareNotFound: `找不到这个${zhHansTerminology.software.forms.label}。`,
	SystemRequirementNotFound: "找不到这项系统需求。",
	PollOptionsDuplicated: "投票选项不能重复。",
	PollNotFound: "找不到这个投票。",
	PollClosed: "这个投票已经关闭。",
	PollSingleChoiceInvalid: "该投票必须且只能选择一个选项。",
	PollOptionInvalid: "选择的投票选项无效。",
	PollAlreadyClosed: "这个投票已经关闭。",
	ContentStructureInvalid: "这个内容结构不符合其用途规则。",
	ContentStructureNotFound: "找不到这个内容结构。",
	ContentStructureNodeNotFound: "找不到这个内容结构节点。",
	ReviewNotFound: "找不到这篇评论。",
	ContentGovernanceTargetNotFound: "找不到内容治理操作的目标。",
	ContentReviewRealmMissing: `该内容审查案件缺少所属${realmTerms.inline}。`,
	ContentReviewCaseNotFound: "找不到这个内容审查案件。",
	ContentGovernanceReversedActionInvalid: "要撤销的操作不属于这个案件。",
	ContentGovernanceActionIncompatible: "该内容治理操作不适用于当前目标。",
	ContentGovernanceTransitionInvalid: "当前目标无法进行这项内容治理状态变更。",
	ContentGovernanceActionNoEffect: "该内容治理操作不会改变目标。",
	ContentGovernanceReversalUnavailable: "已无法安全撤销这项内容治理操作。",
	ContentGovernanceIdempotencyConflict: "该重试键已用于另一项内容治理请求。",
	GovernanceNoteRoleDuplicate: "内部注解和公开通知均最多添加一则。",
	ReportAlreadySubmitted: "你已为当前案件举报过这个条目。",
	ReportTargetRevisionUnavailable: "此条目目前没有可供举报的修订。",
	ReportRuleUnavailable: "所选治理范围目前没有可用规则。",
	ReportRuleChanged: "所选规则已更新，请重新选择后再提交。",
	ReportRuleSourceForbidden: `举报只能引用当前所在${realmTerms.inline}及官方规则。`,
	GovernanceRuleSourceForbidden: `所选规则${realmTerms.label}不属于此治理权限范围。`,
	GovernanceRuleChanged: "所选规则已不在当前规则版本中，请重新选择。",
	GovernanceReversalUnavailable: "这项治理决定已被撤销，或与当前治理对象不符。",
	EnforcementExpiryInvalid: "管理措施的过期时间必须晚于当前时间。",
	EnforcementNotFound: "找不到这项管理措施。",
	EnforcementAlreadyRevoked: "这项管理措施已被撤销。",
	EnforcementChanged: "管理措施已变更，请刷新后再试一次。",
	RealmMemberNotFound: `找不到这个有效的${realmTerms.inline}成员。`,
	CapabilityGrantExpiryInvalid: "授权过期时间必须晚于当前时间。",
	CapabilityGrantNotFound: "找不到这项有效授权。",
	UnitAccessExpiryInvalid: "条目访问授权的过期时间必须晚于当前时间。",
	UnitAccessInvitationNotFound: "找不到这项条目访问邀请。",
	UnitAccessInvitationConflict: "这项条目访问邀请已无法继续处理。",
	UnitAccessInvitationExpired: "这项条目访问邀请已过期。",
	UnitAccessInvitationSelfForbidden: "不能邀请自己访问条目。",
	UnitOwnerRestrictionForbidden: "不能限制条目所有者。",
	UnitAccessConfigurationInvalid: "这组条目权限设置无效或超出你可授予的范围。",
	UnitOwnershipChanged: "条目所有者已变更，请重新加载后再试。",
	UnitOwnershipTargetIneligible: "所选人员当前不符合接收条目所有权的条件。",
	UnitOwnershipRelinquishmentForbidden: "不能放弃由社区持有的条目所有权。",
	UnitOwnershipOverrideConfirmationInvalid: "输入的条目标识符与强制转移目标不符。",
	UnitOwnershipClaimUnavailable: "只有仍由社区持有的受支持类型公开条目可以提出所有权认领。",
	UnitOwnershipClaimAlreadyPending: "你已对这个条目提出尚待审核的所有权认领。",
	UnitOwnershipClaimNotFound: "找不到这项条目所有权认领。",
	UnitOwnershipClaimChanged: "这项所有权认领或原有所有权已变更，请重新加载后再试。",
	UnitOwnershipClaimConfirmationInvalid: "输入的认领标识符与审核目标不符。",
	UnitOwnershipClaimSelfDecisionForbidden: "申请者不能审核自己的条目所有权认领。",
	UnitLifecycleConfirmationInvalid: "输入的条目标识符与操作目标不符。",
	UnitLifecycleChanged: "条目已变更，请重新加载后再试。",
	UnitLifecycleProtected: "这个受保护的条目不能软删除。",
	UnitAlreadyDeleted: "这个条目已经软删除。",
	UnitNotDeleted: "这个条目当前不是软删除状态。",
	UnitMergeNotFound: "找不到此条目合并申请。",
	UnitMergeConfirmationInvalid: "来源或目标确认值不匹配。",
	UnitMergeKindIneligible: "此类型的条目不能合并。",
	UnitMergeKindMismatch: "来源与目标条目必须是相同类型。",
	UnitMergeRequestConflict: "来源条目已有进行中或已完成的合并。",
	UnitMergeMeasurementConflict:
		"此次合并无法完整保留所有测量数据和来源信息。请先整理来源和目标的测量数据，然后重试。",
	UnitMergeIdempotencyConflict: "此重试键已用于其他合并。",
	UnitMergeManifestStale: "来源、目标或变体关系图已变更，请重新执行预检。",
	UnitMergeReviewSelfForbidden: "你不能审核自己提出的条目合并。",
	UnitMergeReviewDuplicate: "你已经审核过此条目合并。",
	UnitMergeReviewFingerprintMismatch: "此审核所依据的是较旧的合并规格。",
	UnitMergeRequestNotPending: "此条目合并已不再等待审核。",
	UnitMergeRequestExpired: "此条目合并申请已过期。",
	UnitMergeRetryUnavailable: "当前状态下无法重试此条目合并。",
	InvalidPaginationCursor: "该分页链接无效或已过期。",
	BookNotFound: "找不到这本书。",
	MediaNotFound: "找不到这个媒体项目。",
	ChapterNotFound: `找不到这个${zhHansTerminology.chapter.forms.label}。`,
	ChapterLanguageNotFound: `找不到该${zhHansTerminology.chapter.forms.label}的语言版本。`,
	ReportRealmMismatch: `被举报的条目不属于这个${realmTerms.inline}。`,
	PostNotFound: `找不到这篇${postTerms.inline}。`,
	PostLocalizationNotFound: `找不到该${postTerms.inline}的语言版本。`,
	PostTargetingLocked: `此对象当前不接受新的${postTerms.plural}指向。`,
	ReplyPostNotFound: `找不到这篇回复${postTerms.inline}。`,
	ParentReplyNotFound: `在该讨论串中找不到父回复${postTerms.inline}。`,
	ReplyDepthExceeded: "该回复超过讨论串的最大层级。",
	InvalidNotificationCursor: "该通知分页链接无效或已过期。",
	NotificationNotFound: "找不到这则通知。",
	EntityEntryNotFound: `找不到这个${entityTerms.label}条目。`,
	EntityAssociationRestricted: `该${entityTerms.label}不接受这种结构化关联。`,
	AssociationProposalNotFound: "找不到这项关联提案。",
	AssociationProposalConflict: "这项关联提案已无法继续处理。",
	AssociationProposalExpired: "这项关联提案已过期。",
	AssociationProposalExpiryInvalid: "关联提案的过期时间必须晚于当前时间。",
	AssociationProposalRoleInvalid: "所选的关联角色不适用于这种关联类型。",
	CreditAttributionNotFound: "找不到这项署名归属。",
	CreditAttributionRoleInvalid: "所选的署名角色不适用于这种类型的条目。",
	CreditAttributionRequestConfirmationRequired: "发送署名邀请前需要确认。",
	SubjectAssociationNotFound: "找不到这项主题关联。",
	AliasNotFound: "找不到这个别名。",
	TagApplicationNotFound: "找不到这笔标签应用记录。",
	TagApplicationHasJudgments: "仍有判断记录依赖这笔标签应用，因此不能移除。",
	UnitTagCurationChanged: "这笔标签精选已在其他地方变更；已加载最新顺序，请再试一次。",
	TagNotFound: "找不到这个标签。",
	TagNotDirectlyApplicable: "这个标签不能直接应用。请选择更具体的标签或路径。",
	ContentLabelApplicationInvalid: "这个内容标签不能应用到该条目。",
	ContentLabelUnitMergeForbidden: "内置内容标签不能合并。",
	ContentLabelJudgmentForbidden: "内容标签是直接设置的，不接受社区判断。",
	ContentLabelPlatformApplyForbidden:
		"只能通过已授权且留有审计记录的平台纠正操作应用这个内容标签。",
	ContentLabelPlatformIdentityImmutable: "平台应用的内容标签不能直接改成另一个标签。",
	ContentLabelPlatformRemovalForbidden:
		"只能通过已授权且留有审计记录的平台纠正操作移除这个由平台应用的内容标签。",
	UnitExternalLinkNotFound: "找不到这个作品的外部链接。",
	UnitReferenceCurationChanged: "这笔参考资料精选已在其他地方变更；已加载最新顺序，请再试一次。",
	UnitReferenceLimitReached: "这个作品的有效参考资料已达上限。",
	UnitReferencePinnedLimitReached: "这个作品的置顶参考资料已达上限。",
	UnitReferenceWithdrawn: "这笔参考资料已撤回。",
	UnitVariantKindMismatch: "变体与主版本必须使用相同且受支持的条目类型。",
	UnitVariantTargetIsVariant: "变体必须直接指向主版本。",
	UnitVariantSourceHasVariants: "已有变体的主版本不能成为变体。",
	UnitVariantGroupLimitReached: "该主版本的变体数量已达上限。",
	UnitVariantChanged: "该条目的主版本关系已变更，请刷新后再试一次。",
	UnitVariantMainUnavailable: "当前变体状态无法使用该主版本。",
	InvalidMessageCursor: "该消息分页链接无效或已过期。",
	ConversationNotFound: "找不到这个对话。",
	ConversationParticipantsInvalid: "私人消息对话必须包含两名用户。",
	DirectMessageBlocked: "存在封锁关系时不能发送私人消息。",
	MessageNotFound: "找不到这则消息。",
	CollectionNotFound: "找不到这个收藏集。",
	RealmNotFound: `找不到这个${realmTerms.inline}。`,
	RealmMembershipNotFound: `找不到这个${realmTerms.inline}成员关系。`,
	RealmOwnerLeaveForbidden: `${realmTerms.label}所有者不能离开${realmTerms.inline}。`,
	RealmUnitNotFound: `这个条目尚未挂载至该${realmTerms.inline}。`,
	WikiNavigationNotFound: "找不到这个百科导航。",
	WikiNavigationInUse: "这个百科导航仍在使用中。",
	WikiNavigationDocumentInvalid: "百科导航文档无效。",
	FavoritesEditForbidden: "不能编辑默认收藏集。",
	FavoritesDeleteForbidden: "不能删除默认收藏集。",
	InvalidFeedCursor: "该动态分页链接无效或已过期。",
	InvalidFeedFilter: "这组动态筛选条件无效。",
	InvalidHistoryCursor: "该历史分页链接无效或已过期。",
	UnitRevisionNotFound: "找不到这个条目修订。",
	CurrentRevisionContentVisibilityForbidden: "请先恢复其他修订，再隐藏当前修订内容。",
	ImageAssetNotFound: "找不到这个图片资源。",
	ImageAssetUploadNotFound: "找不到已上传的图片对象。",
	ImageAssetUnsupportedType: "不支持这种图片格式。",
	ImageAssetInvalidSize: "图片大小无效。",
	ImageAssetContentMismatch: "图片内容与上传声明不一致。",
	ImageAssetInvalidState: "图片资源当前的状态不允许这项操作。",
	ImageAssetInvalidPresentation: "这张图片的显示范围不符合指定用途。",
	ImageAssetInUse: "正在使用的图片资源不能删除。",
	UnitLocalizationOrderChanged: "内容语言顺序已在其他地方变更，请重新加载后再试一次。",
	UnitLocalizationOrderInvalid: "语言顺序必须恰好包含所有现有的内容语言。",
	UnitLocalizationNotFound: "这个内容语言已不存在。",
	UnitLastLocalizationRemovalForbidden: "作品必须保留至少一种内容语言。",
	InvalidSlug: `${unitSlugTerms.label}必须是 1 至 63 个字符的小写 ${verbatimTerms.ascii.value} 短横线标签。`,
	SlugTaken: `这个条目${definitionNamespaceTerms.label}中的${unitSlugTerms.inline}已被使用。`,
	SlugReserved: `这个${unitSlugTerms.inline}是平台保留名称，无法使用。`,
	ProfileSlugChangeUnavailable: `个人资料的${unitSlugTerms.inline}设置后目前无法更改。`,
	SlugScopeNotFound: `找不到${unitSlugTerms.inline}所属的条目${definitionNamespaceTerms.label}。`,
	SlugScopeUnavailable: `没有地址或已删除的条目，不能作为${unitSlugTerms.inline}的标准${definitionNamespaceTerms.label}。`,
	SlugScopeCycle: `这次移动会形成${unitSlugTerms.inline}${definitionNamespaceTerms.label}循环。`,
	SlugDepthExceeded: `由${unitSlugTerms.plural}组成的条目路径超过最大深度。`,
	UnitAddressMutationForbidden: `无法对这个条目的${unitSlugTerms.inline}地址运行这项操作。`,
	SlugRedirectNotFound: `找不到${unitSlugTerms.inline}重定向。`,
	UnitSlugAddressNotFound: `这个条目没有标准${unitSlugTerms.inline}地址。`,
	GovernanceNoteNotFound: "找不到这则治理说明。",
	PostScoreDuplicate: `一篇${postTerms.inline}不能重复显示同一个评分。`,
	PostScoreNotFound: "找不到所选的评分。",
	RealmScoreContextPostNotMounted: `评分规则说明${postTerms.inline}必须挂载至该${realmTerms.inline}。`,
	RealmScoreContextPostKindInvalid: `评分规则说明必须使用普通${postTerms.label}或百科文章。`,
	RealmTagContextNotFound: `找不到这个${realmTerms.inline}标签说明。`,
	RealmTagContextInUse: `仍有标签判断依赖这个${realmTerms.inline}标签说明，因此不能移除。`,
	RealmTagContextPostNotMounted: `作为标签说明的${postTerms.inline}必须在该${realmTerms.inline}中可见。`,
	RealmTagContextAlreadyExists: `此${realmTerms.inline}已有这个标签的说明。`,
	RealmTagContextPostAlreadyUsed: `该${postTerms.inline}已用于说明另一个${realmTerms.inline}标签。`,
	RealmTagVotingDisabled: `该${realmTerms.inline}尚未开启标签投票。`,
	RealmTagContextRequired: `该${realmTerms.inline}必须先为此标签创建当前可见的正式说明。`,
	RealmTagSelfReferenceForbidden: "不能将标签应用到该标签本身。",
	SharedSearchQueryNotFound: "找不到这个共享搜索查询。",
	InvalidTagPath: `这个${tagPathTerms.inline}无效。`,
	TagPathNotFound: `找不到这个${tagPathTerms.inline}。`,
	TagPathApplicationNotFound: `这个内容没有应用该${tagPathTerms.inline}。`,
	TagPathChanged: `这个${tagPathTerms.inline}已在其他地方变更，请刷新后再试一次。`,
	TagPathDefinitionConflict: `已有内容完全相同的${tagPathTerms.inline}。`,
	InvalidTagPathMerge: `这个${tagPathTerms.inline}合并方案无效。`,
	TagPathMergeNotFound: `找不到这个${tagPathTerms.inline}合并提案。`,
	AssociationContextPostInvalid: `关联脉络必须是知识型${postTerms.inline}。`,
	ProgressEntryNotFound: "找不到这条进度事件。",
	PostTagMentionVoteConflict: `这个标签提及与你对该${postTerms.inline}的既有负票冲突。`,
	UnitRealmPublicationNotFound: `找不到这项内容与该${realmTerms.label}的发布关系。`,
	UnitRealmPublicationAlreadyExists: `这项内容已与该${realmTerms.label}建立发布关系。`,
	UnitRealmPublicationTransitionInvalid: "这项发布关系已经处于要求的状态。",
	CustomThemeExternalLiveAccessSelfMutationForbidden:
		"你不能授予、更改或撤销自己对外部资源主题的访问权限。",
	CustomThemeNotFound: `找不到这个${customThemeTerms.inline}。`,
	CustomThemeRevisionNotFound: `找不到这个${customThemeTerms.inline}修订。`,
	CustomThemePackageInvalid: `这个${customThemeTerms.inline}包无效。`,
	CustomThemeSubmissionBackpressure: `这个${customThemeTerms.inline}的提交已暂时暂停，待积压的审核处理完毕后再开放。`,
	CustomThemeExternalResourceInvalid: `无法安全审核这个${customThemeTerms.inline}使用的外部资源。`,
	CustomThemeReviewEvidenceInvalid: `这个${customThemeTerms.inline}的审核凭证不完整。`,
	CustomThemeRevisionStateConflict: `这个${customThemeTerms.inline}修订已进入其他审核状态，请重新加载后再继续。`,
	CustomThemeReviewerSeparationRequired: `提交这个${customThemeTerms.inline}修订的人不能决定其审核结果。`,
	CustomThemeInstallationInvalid: `请选择当前已获准用于此宿主的${customThemeTerms.inline}修订。`,
	UnitPresentationHostUnsupported: "这种条目不支持自定义外观。",
	UnitPresentationRevisionConflict: "条目外观已在其他位置更改，请重新加载后再保存。",
	StudioRealmSubjectLimitExceeded: "你所属的团队过多，工作空间暂时无法列出团队委派内容。",
} satisfies typeof import("../zh-Hant/errorCodes").default;
