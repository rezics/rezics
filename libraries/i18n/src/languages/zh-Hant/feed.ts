import { insert } from "native-i18n";

import { zhHantTerminology } from "@rezics/i18n/terminology/zh-Hant";

const { forms: followTerms } = zhHantTerminology.follow;
const { forms: postTerms } = zhHantTerminology.post;
const { forms: realmTerms } = zhHantTerminology.realm;
const { forms: zoneTerms } = zhHantTerminology.zone;

export default {
	title: "動態",
	subtitle: "作品因討論而被看見",
	personalized: "為你推薦",
	sortLabel: "動態排序",
	sort: { best: "最佳", hot: "熱門", new: "最新", top: "高評價", rising: "熱度上升" },
	filtersLabel: "動態篩選",
	contentFilterLabel: "內容篩選",
	contentFilterPlaceholder: "內容",
	content: {
		all: "全部",
		clear: "清除",
		allSelected: "全部內容",
		noneSelected: "未選擇內容",
		selectedCount: insert("已選 {{count}} 種", { count: Number }),
		unitGroup: "單元",
		postGroup: postTerms.pluralLabel,
		kinds: {
			"unit:profile": "個人檔案",
			"unit:book": "書籍",
			"unit:software": "軟體",
			"unit:media": "媒體",
			"unit:release": "發行版本",
			"unit:entity": "目錄條目",
			"unit:tag": "標籤",
			"unit:series": "系列",
			"unit:zone": zoneTerms.pluralLabel,
			"unit:collection": "收藏集",
			"unit:poll": "投票",
			"unit:realm": realmTerms.pluralLabel,
			"post:post": `主題${postTerms.pluralLabel}`,
			"post:reply": "回覆",
			"post:review": "評論",
			"post:chapter": "章節",
			"post:chapter_group": "章節群組",
			"post:wiki": "百科文章",
			"post:picture": `圖片${postTerms.pluralLabel}`,
		},
		postDescription: "由社群成員發起的主題內容",
		replyDescription: "討論中的回覆與延伸內容",
	},
	discoverWorks: "發現值得投入的作品",
	emptyTitle: "廣場還很安靜",
	emptyBody: "成為第一個分享作品與見解的人。",
	reason: {
		followedPublisher: `因為你${followTerms.followed}了發布者`,
		followedRealm: `因為你${followTerms.followed}了${realmTerms.inline}`,
		basedOnActivity: "根據你的近期活動",
		relatedSubject: "與正在瀏覽的內容相關",
		popularNow: "近期熱度較高",
		newAndRelevant: "新近收錄，可能適合你",
	},
	recommendationMenu: "推薦選項",
	moreActions: "更多操作",
	notInterested: "不感興趣",
	actions: {
		voteGroup: "內容評價",
		comments: insert("{{count}} 則回覆", { count: Number }),
		shareTitle: "分享內容",
		shareDescription: "使用裝置的分享功能，或複製內容連結。",
		shareNative: "分享至其他應用程式",
		copyLink: "複製連結",
		linkCopied: "已複製連結",
		shareFailed: "無法分享，請稍後再試。",
		saved: "已儲存",
		addToCollection: "加入收藏集",
		collectionPickerTitle: "加入收藏集",
		collectionPickerDescription: "選擇要收錄這項內容的收藏集。",
		collectionAdded: "已加入收藏集",
		noOwnedCollections: "你還沒有可使用的收藏集。",
		manageCollections: "管理收藏集",
	},
	fixture: {
		canvasLabel: "內容動態元件展示",
		postTitle: "為什麼御坂網絡是學園都市最特別的群體意識？",
		postBody:
			"御坂網絡並非由個體思維的簡單疊加，而是以電磁場為媒介形成的群體意識。它既超越個人能力的邊界，也保留個體之間的差異。",
		collectionTitle: "科學與魔法的交會點",
		collectionBody: "整理系列中值得反覆閱讀的篇章、評論與世界觀資料。",
		publisher: "海豚號讀書會",
		realm: "魔法禁書目錄",
		recommendation: `因為你${followTerms.actionLabel}了這個發布者`,
		timestamp: "2 小時前",
	},
	replyingIn: "回覆自",
	relatedPosts: "相關推薦討論",
	relatedWorks: "相似作品",
	trending: "正在熱議",
	activeRealms: `活躍${realmTerms.pluralLabel}`,
	continueReading: "繼續閱讀",
	viewAll: "檢視全部",
	relatedWork: "討論關聯作品",
	myRealms: `我的${realmTerms.pluralLabel}`,
	realmContext: insert("位於 {{realm}}", { realm: String }),
	realmContextWithMore: insert(`位於 {{realm}} 及其他 {{count}} 個${realmTerms.plural}`, {
		realm: String,
		count: Number,
	}),
	showRealmList: insert(`{{summary}}。顯示${realmTerms.inline}清單`, { summary: String }),
	publishedInOneRealm: insert(`發布於 {{count}} 個${realmTerms.inline}`, { count: Number }),
	publishedInRealms: insert(`發布於 {{count}} 個${realmTerms.plural}`, { count: Number }),
};
