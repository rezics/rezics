...
export default {
	...
	progressByType: {
		...
		generic: {
			...
			summaryTitle: "我的進度",
			progressLabel: "完成度",
			dialogDescription: "記錄狀態與進度；完成本次時，完成次數會增加一次。",
			...
			totalMinutes: "累計分鐘",
			history: {
				completion: insert("於 {{date}} 完成了這次進度。", { date: String }),
				update: insert("於 {{date}} 達到 {{percent}}%。", {
					date: String,
					percent: Number,
				}),
			},
			...
			updateAction: "更新進度",
			resumeAction: "繼續",
			startAgainAction: "再開始一次",
			restartAction: "重新開始",
			completeAction: "完成本次進度",
			completedFeedback: insert("已完成第 {{count}} 次", { count: Number }),
			statistics: {
				active: insert("有 {{count}} 人正在進行", { count: String }),
				activeOne: "有 1 人正在進行",
				backlog: insert("有 {{count}} 人想開始", { count: String }),
				backlogOne: "有 1 人想開始",
			},
			statuses: {
				backlog: "想開始",
