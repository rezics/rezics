...
export default {
	memberships: {
		...
		acceptPrompt: "接受後，目前登入的帳戶將以此組織的私人成員身分加入。",
		removePrompt: "移除此成員會結束其成員關係，依賴此關係的存取權也會失效。",
		leavePrompt: "退出會結束你的成員關係，依賴此關係的存取權也會失效。",
		recipientHint: `請輸入要邀請的${zhHantTerminology.entity.forms.label}的 ${verbatimTerms.id.value}。`,
		states: {
			pending: "待處理",
			accepted: "已接受",
			declined: "已拒絕",
	...
	customThemes: "顯示自訂空間主題",
	customThemesHint: "關閉後，所有空間都會改用平台預設主題；內容與版面配置不受影響。",
	account: "帳戶",
	...
	tokens: {
		...
		limitRanges: insert(
			"允許範圍：每分鐘要求數 {{requestsMinimum}} 至 {{requestsMaximum}}；突發容量 {{burstMinimum}} 至 {{burstMaximum}}；同時執行要求數 {{concurrentMinimum}} 至 {{concurrentMaximum}}；每日成本單位 {{dailyMinimum}} 至 {{dailyMaximum}}。",
			{
