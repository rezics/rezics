import type { PlatformCapability, UnitPermission } from "@rezics/access";
import { insert } from "native-i18n";

import { verbatimTerms } from "@rezics/i18n/verbatim-terms";
import { zhHantTerminology } from "@rezics/i18n/terminology/zh-Hant";

const { forms: realmTerms } = zhHantTerminology.realm;
const { forms: entityTerms } = zhHantTerminology.entity;
const { forms: postTerms } = zhHantTerminology.post;
const { forms: zoneTerms } = zhHantTerminology.zone;
const { forms: customThemeTerms } = zhHantTerminology.customTheme;
const { forms: definitionNamespaceTerms } = zhHantTerminology.definitionNamespace;

export default {
	title: "條目治理",
	open: "開啟治理設定",
	platformOpen: "開啟平台治理",
	ruleBasis: {
		label: "適用規則",
		description: `請從此治理權限可用的規則${realmTerms.label}選擇至少一條規則，作為不可變更的決策依據。`,
		sourceAriaLabel: `規則${realmTerms.label}`,
		chooseSource: `選擇規則${realmTerms.label}`,
		sources: {
			platform: "平台官方規則",
			realm: `${realmTerms.label}規則`,
			local: "本地規則",
		},
		rulesAriaLabel: "適用規則",
		chooseRules: "選擇規則",
		clear: "清除所選規則",
		selectedCount: insert("已選擇 {{count}} 條規則", { count: Number }),
		loading: "正在載入規則…",
		loadError: "無法載入可用規則，請稍後再試。",
	},
	accessInvitations: "存取邀請",
	receivedInvitations: "收到的存取邀請",
	accessInvitationDescription: "接受邀請前不會產生任何存取權限。",
	invitedAccount: "受邀帳戶",
	invitedBy: "邀請人",
	unitId: `條目 ${verbatimTerms.id.value}`,
	scope: "權限範圍",
	scopeHint: "留空代表整個條目；多級範圍用 / 分隔。",
	invitationExpiry: "邀請有效期限",
	accessExpiry: "存取權限有效期限（選填）",
	invite: "傳送邀請",
	accept: "接受",
	decline: "拒絕",
	cancel: "撤銷",
	noInvitations: "目前沒有邀請。",
	creditAssociations: "署名關聯",
	publisherAttributions: `${zhHantTerminology.publisher.forms.label}標示`,
	publisherAttributionDescription: `若對方的個人檔案允許，可直接加入${zhHantTerminology.publisher.forms.label}；否則請在下方提出${zhHantTerminology.publisher.forms.label}關聯請求。`,
	publisherProfile: `${zhHantTerminology.publisher.forms.label}個人檔案`,
	addPublisher: `加入${zhHantTerminology.publisher.forms.label}`,
	removePublisher: `移除${zhHantTerminology.publisher.forms.label}`,
	noPublishers: `尚未標示${zhHantTerminology.publisher.forms.label}。`,
	subjectAssociations: "主題關聯",
	associationProposals: "關聯提案",
	associationProposalDescription: "提案經另一方接受後才會建立實際關聯。",
	targetEntity: `目標${entityTerms.label}`,
	targetUnit: "署名對象",
	sourceUnit: "來源條目",
	associationRole: "關聯角色",
	proposalExpiry: "提案有效期限",
	contextWikiPost: `選填脈絡知識${postTerms.label}`,
	requestAssociation: "請求關聯",
	inviteAssociation: "邀請關聯",
	noProposals: "目前沒有關聯提案。",
	capabilities: {
		"catalog.definition.manage": "管理目錄術語定義與驗證規則",
		"platform.access.read": "檢視平台權限指派",
		"platform.access.manage": "管理平台權限指派",
		"platform.audit.read": "檢視平台稽核紀錄",
		"platform.user.read": "檢視平台使用者",
		"platform.user.status.update": "管理使用者帳號狀態",
		"platform.session.read": "檢視使用者工作階段",
		"platform.session.revoke": "撤銷使用者工作階段",
		"entity.associations.override": `略過${entityTerms.inline}關聯權限`,
		"unit.edit": "編輯所有條目",
		"platform.development_preview.access": "使用尚未發布的開發預覽功能",
		"unit.governance.read": "檢視所有條目的治理資料",
		"unit.merge.propose": "提出條目合併",
		"unit.merge.review": "審核條目合併",
		"unit.merge": "管理條目合併",
		"unit.ownership.override": "強制移轉任意條目的所有權",
		"unit.license.manage": `使條目${zhHantTerminology.license.forms.label}授予失效或恢復`,
		"unit.delete": "軟刪除任意條目",
		"unit.restore": "還原已軟刪除的條目",
		"unit.slug.manage": "管理條目公開網址",
		"unit.slug.namespace.manage": `管理網址${definitionNamespaceTerms.label}`,
		"unit.slug.redirect.release": "釋出保留的網址重新導向",
		"platform.api_quota_policy.read": `檢視 ${verbatimTerms.api.value} 配額政策`,
		"platform.api_quota_policy.update": `更新 ${verbatimTerms.api.value} 配額政策`,
		"platform.user.api_quota.read": `檢視使用者 ${verbatimTerms.api.value} 配額`,
		"platform.user.api_quota.update": `管理使用者 ${verbatimTerms.api.value} 配額`,
		"platform.user.api_token.api_quota.read": `檢視使用者 ${verbatimTerms.api.value} 權杖配額`,
		"platform.user.api_token.api_quota.update": `管理使用者 ${verbatimTerms.api.value} 權杖配額`,
		"platform.moderate": "執行平台內容治理",
		"platform.suppress": "隱藏平台內容與修訂",
		"platform.custom_theme.external_live.access": `在${customThemeTerms.label}中使用外部即時資源`,
		"platform.custom_theme.external_live.access.manage": `管理${customThemeTerms.label}的外部即時資源權限`,
		"platform.custom_theme.review": `審查${customThemeTerms.label}`,
		"platform.custom_theme.kill": `緊急停用已核准的${customThemeTerms.label}`,
		"realm.contribute": `參與所有${realmTerms.pluralLabel}`,
		"realm.units.create": `在所有${realmTerms.pluralLabel}建立條目`,
		"realm.post.replies.create": `在所有${realmTerms.pluralLabel}建立回覆`,
		"realm.settings.update": `更新${realmTerms.label}設定`,
		"realm.members.read": `檢視${realmTerms.label}成員`,
		"realm.members.manage": `管理${realmTerms.label}成員`,
		"realm.rules.update": `更新${realmTerms.label}規則`,
		"realm.pins.manage": `管理${realmTerms.label}置頂內容`,
		"realm.tags.manage": `管理${realmTerms.label}標籤`,
		"realm.tag-voting.update": `更新${realmTerms.label}標籤投票`,
		"realm.tag-contexts.manage": `管理${realmTerms.label}標籤脈絡關聯`,
		"realm.units.moderate": `治理${realmTerms.label}內容`,
	} satisfies Record<PlatformCapability, string>,
	direction: {
		request: "請求",
		invitation: "邀請",
	},
	states: {
		pending: "待處理",
		expired: "已過期",
		accepted: "已接受",
		declined: "已拒絕",
		cancelled: "已撤銷",
	},
	access: {
		scopeSelectorLabel: "權限範圍",
		scopeOptions: {
			root: "整個內容單元",
			creditAttributions: "來源：貢獻署名",
			subjectAssociations: "來源：主題關聯",
			creditTargets: `${entityTerms.label}目標：貢獻署名`,
			subjectTargets: `${entityTerms.label}目標：主題關聯`,
		},
		publicTitle: "公共權限",
		publicDescription: `直接授予所有已登入 ${verbatimTerms.rezics.value} 使用者的權限。這是全域基線，因此與個別${zhHantTerminology.license.forms.label}對象分開管理。`,
		authenticatedLabel: "所有已登入使用者",
		currentRealmMembersLabel: `此${realmTerms.label}的所有有效成員`,
		realmMembersLabel: insert("{{realm}}的有效成員", { realm: String }),
		realmAccessManagersLabel: insert("{{realm}}的存取權管理員", { realm: String }),
		builtInAudience: "內建對象",
		subjectsTitle: `${zhHantTerminology.license.forms.label}對象`,
		subjectsDescription: `管理使用者與${realmTerms.label}的直接${zhHantTerminology.license.forms.label}或限制；限制優先於${zhHantTerminology.license.forms.label}。`,
		addSubject: `新增${zhHantTerminology.license.forms.label}對象`,
		addSubjectDescription: `搜尋一位使用者或一個${realmTerms.label}，再設定可使用或受限制的權限。`,
		searchSubjects: `搜尋${zhHantTerminology.license.forms.label}對象……`,
		searchCandidates: `搜尋使用者或${realmTerms.label}……`,
		filterLabel: `${zhHantTerminology.license.forms.label}對象類型篩選`,
		filters: {
			all: "全部",
			auth: "帳戶",
			realm: realmTerms.label,
		},
		alreadyAdded: "已加入",
		noCandidates: `找不到符合條件的${zhHantTerminology.license.forms.label}對象。`,
		owner: "條目擁有者",
		transferOwnership: "移轉擁有權",
		relinquishOwnership: "放棄擁有權",
		transferOwnershipDescription:
			"只能選擇已接受協作邀請，且目前在整個條目具有永久編輯權限的個人檔案。",
		searchOwnershipCandidates: "搜尋可接收擁有權的人員…",
		ownershipCandidates: "可接收擁有權的人員",
		selectOwnershipCandidate: "選取",
		noOwnershipCandidates: "沒有符合條件的協作者。",
		confirmTransferTitle: "確認移轉擁有權",
		confirmTransferDescription: insert("要將「{{unit}}」的擁有權移轉給 {{profile}} 嗎？", {
			unit: String,
			profile: String,
		}),
		untitledOwnershipUnit: "未命名條目",
		transferOwnershipWarning:
			"完成後，你會立即失去擁有者權限。新的擁有者必須自行再次移轉，才能撤回這項操作。",
		backToOwnershipCandidates: "返回人員清單",
		confirmTransfer: "確認移轉",
		relinquishOwnershipTitle: "確認放棄擁有權",
		relinquishOwnershipDescription:
			"條目將立即歸屬 community@rezics.com；完成後，你會失去擁有者權限。",
		confirmRelinquishment: "放棄並交給社群",
		editorTitle: insert("編輯「{{subject}}」的權限", { subject: String }),
		editorDescription: "逐項授予或限制權限；未選取的項目不建立直接規則。",
		save: "儲存權限",
		saving: "正在儲存……",
		cancel: "取消",
		inheritedDescription: "這項權限另有繼承來源；直接限制仍然優先。",
		subjectKinds: {
			auth: "帳戶",
			realm: realmTerms.label,
			authenticated: "所有已登入使用者",
		},
		subjectRelations: {
			member: `${realmTerms.label}成員`,
			access_manager: `${realmTerms.label}存取權管理員`,
		},
		table: {
			subject: `${zhHantTerminology.license.forms.label}對象`,
			kind: "類型",
			rules: "直接規則",
			ruleCount: insert("{{count}} 項", { count: Number }),
			empty: `目前沒有符合條件的${zhHantTerminology.license.forms.label}對象。`,
		},
		matrix: {
			templates: "權限範本",
			permissions: "權限",
			searchPlaceholder: "搜尋權限群組……",
			clear: "全部取消選取",
			selected: insert("已選 {{selected}} / {{total}}", {
				selected: Number,
				total: Number,
			}),
			categorySelected: insert("已選 {{selected}} 項", { selected: Number }),
			required: "必要",
			empty: "沒有符合搜尋條件的權限。",
			grant: "授予",
			restrict: "限制",
		},
		permissionCategories: {
			unit: "條目",
			realm: realmTerms.label,
			entity: `${entityTerms.label}關聯`,
		},
		permissions: {
			"unit.read": "檢視",
			"unit.update": "編輯",
			"unit.metadata-only.update": "變更完整內容可用性",
			"unit.status.update": "變更狀態",
			"unit.history.restore": "還原歷史",
			"unit.access.manage": "管理權限",
			"unit.ownership.transfer": "移轉擁有權",
			"unit.association.manage": "管理關聯",
			"unit.tag-curation.manage": "管理標籤精選",
			"unit.reference-curation.manage": "管理參考資料精選",
			"unit.realm-publication.manage": `管理${realmTerms.label}發布`,
			"zone.pages.manage": `管理${zoneTerms.label}頁面與導覽`,
			"zone.theme.manage": `管理${zoneTerms.label}外觀`,
			"realm.contribute": `參與${realmTerms.label}`,
			"realm.units.create": "建立條目",
			"realm.post.replies.create": "建立回覆",
			"realm.settings.update": `編輯${realmTerms.label}設定`,
			"realm.members.read": "檢視成員",
			"realm.members.manage": "管理成員",
			"realm.rules.update": "更新規則",
			"realm.pins.manage": "管理置頂",
			"realm.tags.manage": `管理${realmTerms.label}標籤`,
			"realm.tag-voting.update": "更新標籤投票",
			"realm.tag-contexts.manage": "管理標籤脈絡關聯",
			"realm.units.moderate": "治理內容",
			"entity.association.credit.request": "申請署名關聯",
			"entity.association.credit.direct": "直接建立署名關聯",
			"entity.association.subject.request": "申請主題關聯",
			"entity.association.subject.direct": "直接建立主題關聯",
		} satisfies Record<UnitPermission, string>,
	},
};
