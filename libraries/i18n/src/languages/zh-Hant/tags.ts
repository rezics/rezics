...
export default {
	...
	picker: {
		open: insert("查看待加入標籤「{{tag}}」", { tag: String }),
		close: "關閉標籤詳細資料",
		path: tagPathTerms.label,
		remove: insert("從待加入項目移除「{{tag}}」", { tag: String }),
		searchMore: "繼續搜尋標籤",
		loading: "正在搜尋標籤……",
		searchError: "無法搜尋標籤，請再試一次。",
		directResults: "標籤",
		pathResults: `${tagPathTerms.label}語意`,
		noResults: `沒有相符的標籤或${tagPathTerms.label}。`,
		addSelected: insert("加入已選標籤（{{count}}）", { count: Number }),
		selectedCount: insert("已選取 {{count}} 項", { count: Number }),
		commitError: "部分標籤未能加入；失敗項目已保留，可直接重試。",
	},
	basic: {
		title: "基本標籤",
	...
	create: {
		...
		duplicateTitle: "檢查既有標籤",
		duplicateDescription: "編輯標題時會自動顯示相近的既有標籤，避免重複建立。",
		duplicateSearching: "正在檢查既有標籤……",
		duplicateSearchError: "目前無法檢查既有標籤。請修改標題或稍後再試。",
		duplicateMatches: "可能相同的標籤",
		duplicateNoMatches: "找不到相近的既有標籤。",
		useExisting: "使用這個標籤",
		viewExisting: "查看標籤",
		exactDuplicateBlocked: "已有同名標籤。請使用既有標籤，不能建立重複項目。",
		duplicateConfirmed: "你已確認這些標籤都不是所需標籤。",
		continueDistinct: "都不相符，繼續建立",
		useExistingError: "無法套用既有標籤，請再試一次。",
	},
	global: {
		title: "全域情境",
