...
export default {
	...
	progressByType: {
		...
		generic: {
			title: "Fortschrittsprotokoll",
			summaryTitle: "Mein Fortschritt",
			progressLabel: "Abschluss",
			dialogDescription:
				"Erfasse deinen Status und Fortschritt. Beim Abschließen erhöht sich die Anzahl deiner Abschlüsse um eins.",
			listSummary: insert("{{percent}} % abgeschlossen · {{count}} Abschlüsse", {
				percent: Number,
				count: Number,
			}),
			status: "Status",
			...
			history: {
				completion: insert("Am {{date}} abgeschlossen.", { date: String }),
				update: insert("Am {{date}} {{percent}} % erreicht.", {
					date: String,
					percent: Number,
				}),
			},
			scoreAction: "Bewerten",
			recordAction: "Fortschritt erfassen",
			startAction: "Beginnen",
			updateAction: "Fortschritt aktualisieren",
			resumeAction: "Fortsetzen",
			startAgainAction: "Erneut beginnen",
			restartAction: "Von vorne beginnen",
			completeAction: "Diesen Durchgang abschließen",
			completedFeedback: insert("Abschluss {{count}}", { count: Number }),
			statistics: {
				active: insert("{{count}} Personen sind gerade dabei", {
					count: String,
				}),
				activeOne: "1 Person ist gerade dabei",
				backlog: insert("{{count}} Personen möchten dies beginnen", { count: String }),
				backlogOne: "1 Person möchte dies beginnen",
			},
			statuses: {
				backlog: "Möchte ich beginnen",
