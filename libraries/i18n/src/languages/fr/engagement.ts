...
export default {
	...
	progressByType: {
		...
		generic: {
			...
			summaryTitle: "Ma progression",
			progressLabel: "Achèvement",
			dialogDescription:
				"Enregistrez votre état et votre progression. Terminer ceci ajoute une unité à votre nombre de fois.",
			listSummary: insert("{{percent}} % terminé · {{count}} fois", {
				percent: Number,
				count: Number,
			}),
			status: "État",
			progress: "Progression",
			completedCount: "Nombre de fois",
			totalMinutes: "Minutes totales",
			history: {
				completion: insert("Terminé le {{date}}.", { date: String }),
				update: insert("Progression portée à {{percent}} % le {{date}}.", {
					date: String,
					percent: Number,
				}),
			...
			scoreAction: "Noter",
			recordAction: "Enregistrer la progression",
			startAction: "Commencer",
			updateAction: "Mettre à jour la progression",
			resumeAction: "Reprendre",
			startAgainAction: "Recommencer",
			restartAction: "Recommencer depuis le début",
			completeAction: "Terminer cette progression",
			completedFeedback: insert("Fois {{count}}", { count: Number }),
			statistics: {
				active: insert("{{count}} personnes ont actuellement ceci en cours", {
					count: String,
				}),
				activeOne: "1 personne a actuellement ceci en cours",
				backlog: insert("{{count}} personnes souhaitent commencer ceci", {
					count: String,
				}),
				backlogOne: "1 personne souhaite commencer ceci",
			},
			statuses: {
				backlog: "À commencer",
				active: "En cours",
