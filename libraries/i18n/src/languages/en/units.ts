import { verbatimTerms } from "@rezics/i18n/verbatim-terms";
import { enTerminology } from "@rezics/i18n/terminology/en";
import { insert } from "native-i18n";

const { forms: licenseTerms } = enTerminology.license;
const { forms: postTerms } = enTerminology.post;
const { forms: videoTerms } = enTerminology.video;
const { forms: audioTerms } = enTerminology.audio;
const { forms: realmTerms } = enTerminology.realm;
const { forms: followTerms } = enTerminology.follow;
const { forms: metadataTerms } = enTerminology.metadata;
const { forms: entityTerms } = enTerminology.entity;

export default {
	nativeEditorial: {
		title: "Description and images",
		edit: "Edit description and images",
		language: "Language tag",
		openLanguage: "Open language",
		invalidLanguage: "Enter a valid language tag.",
		withdraw: "Clear current description and images",
		name: "Name",
		primaryName: "Use as the primary name for this language",
		addName: "Add name",
	},
	nativeGrouping: {
		classes: "Classifications",
		orders: "Named orders",
		orderName: "Order name",
		membership: "Membership",
		sourcePosition: "Source position",
		moveEarlier: "Move earlier",
		removeOrderEntry: "Remove from this order",
		addMembership: "Place in this order",
		editMemberships: "Edit membership relations",
		reapply: "Reapply this change",
		operations: {
			"class.assign": "Classification assigned",
			"class.remove": "Classification removed",
			"order.create": "Order created",
			"order.rename": "Order renamed",
			"order.set": "Membership positioned",
			"order.remove": "Membership removed from order",
		},
	},
	nativePublishing: {
		work: enTerminology.publishingWork.forms.label,
		text_version: enTerminology.textVersion.forms.label,
		publication: enTerminology.publication.forms.label,
		serialization: enTerminology.serialization.forms.label,
		works: enTerminology.publishingWork.forms.pluralLabel,
		textVersions: enTerminology.textVersion.forms.pluralLabel,
		publications: enTerminology.publication.forms.pluralLabel,
		serializations: enTerminology.serialization.forms.pluralLabel,
		coverage: enTerminology.publishingCoverage.forms.label,
		installments: enTerminology.publishingInstallment.forms.pluralLabel,
		publisher: enTerminology.publisher.forms.label,
		pageCount: "Pages",
		pagination: "Pagination statement",
		editTitle: "Edit title",
		removed: "Removed",
		noSubparts: "No nested parts.",
		dateText: "Date as written",
		events: `Dates and ${enTerminology.publisher.forms.plural}`,
	},
	nativeSemantics: {
		authorQualifier: "Write a qualifier value",
		reuseQualifier: "Reuse an existing attribute",
		includeInactive: "Show withdrawn and superseded decisions",
		title: "Attributes and relations",
		facts: "Attributes",
		relations: "Relations",
		openRecord: "Open entry",
		addFact: "Add attribute",
		addRelation: "Add relation",
		noFacts: "No attributes match the current spoiler setting.",
		noRelations: "No relations match the current spoiler setting.",
		value: "Value",
		item: "Item",
		unknown: "Unknown",
		trueValue: "Yes",
		falseValue: "No",
		chooseValue: "Choose a value",
		add: "Add",
		remove: "Remove",
		chooseVocabularyValue: "Choose a vocabulary value",
		valueSelected: "Value selected",
		unnamedDefinition: "Unnamed definition",
		unnamedTarget: "Unnamed entry",
		unavailableTarget: "This participant is not available to you.",
		property: "Attribute definition",
		predicate: "Relation definition",
		spoiler: "Spoiler level",
		spoilerVisibility: "Show spoilers",
		spoilerNone: "No spoilers",
		spoilerMinor: "Minor spoilers",
		spoilerMajor: "All spoilers",
		invalid: "Check the value, participant roles, required counts, and allowed targets.",
		saveFact: "Save attribute",
		saveRelation: "Save relation",
		largeEdit: "This value exceeds the editor’s bounded command size. You can inspect it in pages.",
		incompleteValue: "The complete value could not be loaded. Reload before editing.",
		participant: "Participant",
		participants: "Participants",
		role: "Role",
		target: "Target entry",
		chooseTarget: "Find an existing entry",
		creditedAs: "Credited as",
		addParticipant: "Add participant",
		addQualifier: "Add qualifier",
		qualifiers: "Qualifiers",
		inspectValue: "Inspect value",
		useValue: "Use this value",
		noMatchingValues:
			"No existing value matches this definition and spoiler setting. Add the attribute first.",
		history: "Revision history",
		restore: "Restore as a new revision",
		revise: "Revise",
		list: "List",
		record: "Record",
		states: {
			active: "Active",
			disputed: "Disputed",
			withdrawn: "Withdrawn",
			superseded: "Superseded",
		},
		transitions: {
			disputed: "Dispute",
			withdrawn: "Withdraw",
			superseded: "Mark as superseded",
		},
	},
	nativeSources: {
		releaseTypes: {
			vndb: enTerminology.softwareRelease.forms.label,
			musicbrainz: enTerminology.musicRelease.forms.label,
		},
		previewProfile: "Compare source data",
		rawProfile: "Original response",
		nativeProfile: "Mapped source view",
		archiveDigest: "Archive digest",
		profileCaptureNotice:
			"These responses were captured separately. Their timestamps and hashes identify the exact evidence; they do not guarantee a single simultaneous upstream state.",
		capturedAt: "Captured at",
		captureRequest: "Request address",
		derivedFrom: "Derived from",
		title: "Sources and updates",
		intake: "Import from a source",
		privateDraft:
			"New entries are private drafts. Existing entries require a reviewed update before source changes are adopted.",
		provider: "Source",
		objectType: "Record type",
		externalId: "Source record identifier",
		invalidInput: "Check the source, record type, and identifier.",
		import: "Import",
		openRecord: "Open entry",
		job: "Import progress",
		preparedDependencies: "Related records prepared",
		outcome: "Result code",
		refresh: "Refresh progress",
		resume: "Resume",
		pause: "Pause",
		noBindings: "No source is linked to this entry.",
		history: "Update history",
		bindingSettings: "Source settings",
		checkSource: "Check for updates",
		propose: "Prepare update for review",
		bindingState: "Link status",
		mode: "Update mode",
		reason: "Reason",
		withdrawBindingNotice:
			"Stops future updates from this source. Previously adopted information remains.",
		save: "Save",
		noProposals: "No updates are awaiting review or recorded in this history.",
		review: "Review changes",
		reviewWithdrawal: "Review withdrawal",
		reject: "Reject update",
		supersede: "Mark as superseded",
		sourcePreviewNotice:
			"These are changes in the source record. Applying them runs the selected mapping; this preview is not an exact list of changes to the entry.",
		reviewDetails: "Review details",
		mappingVersion: "Mapping version",
		targetRevision: "Expected entry revision",
		bindingRevision: "Expected link revision",
		sourceDigest: "Source digest",
		noChanges: "No source values differ in this comparison.",
		withdrawProposalNotice:
			"Withdrawal removes the adopted contribution under the current ownership rules. Review the source values and provide a reason before continuing.",
		apply: "Apply update",
		withdraw: "Withdraw contribution",
		before: "Before",
		after: "After",
		readValue: "Read full value",
		absent: "Absent",
		changedEvidence: "The evidence differs from the selected preview. Reload before deciding.",
		providers: {
			vndb: verbatimTerms.vndb.value,
			musicbrainz: verbatimTerms.musicbrainz.value,
			bangumi: verbatimTerms.bangumi.value,
			openlibrary: verbatimTerms.openlibrary.value,
		},
		bindingStates: {
			active: "Active",
			paused: "Paused",
			withdrawn: "Withdrawn",
		},
		modes: {
			review: "Review updates",
			manual: "Manual updates",
		},
		intakeStates: {
			created: "Private draft created",
			unchanged: "Already up to date",
			paused: "Updates are paused",
			review_required: "Update needs review",
		},
		proposeStates: {
			paused: "Updates are paused",
			unchanged: "No new update to review",
			proposed: "Update ready for review",
		},
		proposalStates: {
			pending: "Awaiting review",
			applied: "Applied",
			rejected: "Rejected",
			superseded: "Superseded",
			withdrawn: "Withdrawn",
		},
		jobStates: {
			queued: "Queued",
			prepared: "Prepared",
			paused: "Paused",
			succeeded: "Completed",
			superseded: "Superseded",
			blocked: "Needs attention",
			failed: "Failed",
		},
		objectTypes: {
			staff: "Staff member",
			producer: "Producer",
			trait: "Trait",
			quote: "Quotation",
			work: "Work",
			artist: "Artist",
			series: "Series",
			genre: "Genre",
			url: "Web address",
			subject: "Subject",
			index: "List",
			author: "Author",
			vn: enTerminology.visualNovel.forms.label,
			recording: enTerminology.musicRecording.forms.label,
			release_group: enTerminology.musicReleaseGroup.forms.label,
			person: enTerminology.entity.forms.personLabel,
			character: enTerminology.entity.forms.characterLabel,
			label: enTerminology.entity.forms.labelLabel,
			area: enTerminology.referenceCatalog.forms.areaLabel,
			place: enTerminology.referenceCatalog.forms.placeLabel,
			event: enTerminology.referenceCatalog.forms.eventLabel,
			instrument: enTerminology.referenceCatalog.forms.instrumentLabel,
			episode: enTerminology.program.forms.episodeLabel,
			edition: enTerminology.publication.forms.label,
			tag: "Tag",
		},
	},
	nativeDefinitions: {
		title: `Definitions for ${enTerminology.catalogEntry.forms.plural}`,
		newDefinition: "New definition",
		revise: "Create new revision",
		revisionNotice: "Existing facts keep the meaning of the revision they were recorded with.",
		kind: "Definition type",
		namespace: enTerminology.definitionNamespace.forms.label,
		key: "Stable key",
		labels: "Localized names",
		language: "Language",
		label: "Name",
		description: "Description",
		reason: "Reason for this meaning",
		empty: "No definitions match these filters.",
		noReview: "No review recorded",
		reviewed: "Reviewed",
		choose: "Choose definition",
		selected: "Selected definition",
		constraints: "Validation rules",
		valueKind: "Value type",
		nullable: "Allow an unknown value",
		integer: "Whole numbers only",
		minimum: "Minimum value",
		maximum: "Maximum value",
		minLength: "Minimum text length",
		maxLength: "Maximum text length",
		unit: "Measurement unit",
		allowedValues: "Allowed values",
		targets: "Allowed targets",
		shapes: "Allowed shapes",
		slots: "Usage slots",
		roles: "Participant roles",
		role: "Participant role",
		minParticipants: "Minimum participants",
		maxParticipants: "Maximum participants",
		qualifiers: "Allowed qualifiers",
		members: "Vocabulary members",
		vocabulary: "Controlled vocabulary",
		rules: "Value structure",
		parent: "Parent rule",
		memberKey: "Field name",
		root: "Root value",
		invalid: "Check the required fields, bounds, and rule relationships.",
		save: "Save reviewed meaning",
		history: "Revision history",
		details: "Definition details",
		add: "Add",
		remove: "Remove",
		any: "All types",
		namespaceHint: `Use the exact ${enTerminology.definitionNamespace.forms.inline} to narrow the list.`,
		noCurrent: "This definition has no revision.",
		kinds: {
			class: "Class",
			property: "Property",
			predicate: "Relationship",
			role: "Participant role",
			vocabulary: "Vocabulary",
		},
		valueKinds: {
			null: "Empty",
			string: "Text",
			number: "Number",
			boolean: "Yes or no",
			object: "Record",
			array: "List",
		},
	},
	nativeMusic: {
		music: enTerminology.music.forms.label,
		recording: enTerminology.musicRecording.forms.label,
		track: enTerminology.musicTrack.forms.label,
		medium: enTerminology.musicMedium.forms.label,
		release: enTerminology.musicRelease.forms.label,
		release_group: enTerminology.musicReleaseGroup.forms.label,
		work: enTerminology.musicalWork.forms.label,
		release_candidate: `Unmatched ${enTerminology.musicRelease.forms.inline}`,
		number: "Number",
		position: "Position",
		duration: "Duration (milliseconds)",
		barcode: "Barcode",
		language: "Language",
		script: "Script",
		history: "History",
		restore: "Restore this version",
		remove: "Remove",
		addMedium: `Add ${enTerminology.musicMedium.forms.inline}`,
		addTrack: `Add ${enTerminology.musicTrack.forms.inline}`,
		emptyMedia: `No ${enTerminology.musicMedium.forms.plural} yet`,
		emptyTracks: `No ${enTerminology.musicTrack.forms.plural} yet`,
		deleted: "Deleted",
		current: "Current",
		video: `${enTerminology.video.forms.label}`,
		addName: "Add name",
	},
	nativeCatalog: { names: "Names", identifiers: "Identifiers" },
	nativeDomain: {
		any: "Any",
		yes: "Yes",
		no: "No",

		duration: "Duration (milliseconds)",
		details: metadataTerms.label,
		history: "Revision history",
		restore: "Restore revision",
		cancel: "Cancel",
		language: "Language",
		development: "Development status",
		unknown: "Not specified",
		inDevelopment: "In development",
		finished: "Completed",
		cancelled: "Cancelled",
		notes: "Notes",
		evidence: "What distinguishes this version",
		minimumAge: "Minimum age",
		productCode: "Product code",
		year: "Year",
		month: "Month",
		day: "Day",
		mainEpisodes: "Main episodes",
		totalEpisodes: "Total episodes",
		number: "Number",
		episodeNumber: `${enTerminology.program.forms.episodeLabel} number`,
		orderNumber: "Order number",
		releases: "Available releases",
		languages: "Languages",
		machineTranslated: "Machine-translated",
		episodes: "Episodes",
		emptyReleases: "No matching releases are recorded.",
	},
	types: {
		book: "Book",
		software: `${enTerminology.software.forms.label}`,
		media: "Media",
		series: "Series",
		video: videoTerms.label,
		audio: audioTerms.label,
		release: "Release",
	},
	creation: {
		workForm: "Work form",
		commonCategories: "Common categories",
		commonTopics: "Common topics",
		workOwnershipLabel: "Work ownership",
		ownedWork: "Owned work",
		ownedWorkDescription:
			"Your Profile owns and governs this work. You can share edit access later in permissions.",
		publicWork: "Public work",
		publicWorkDescription:
			"Community-owned and editable by every signed-in user; the creator receives no special access.",
		publicWorkLearnMore: "What is a public work?",
		publicWorkHelpTitle: "What is a public work?",
		publicWorkHelpOwnership:
			"Public works are owned and maintained by the community. Creating one does not give you ownership or additional access; every signed-in user edits with the same shared permissions.",
		publicWorkHelpScope: `Choose this when the community, rather than one Profile, should maintain the work. Ownership does not determine whether ${enTerminology.chapter.forms.label} bodies are shown.`,
		publicWorkHelpAlternative:
			"If you hold the copyright, or want to govern a work while opening collaboration to selected people, choose “Owned work” and grant the appropriate edit access in permissions after creation.",
		entryOwnershipLabel: "Entry ownership",
		ownedEntry: "My entry",
		ownedEntryDescription: "Your Profile owns and governs this entry.",
		publicEntry: "Public entry",
		publicEntryDescription:
			"Community-owned and editable by every signed-in user; the creator receives no special access.",
		creditAttributionsTitle: "Credits",
		creditEntityLabel: insert(`${entityTerms.label} {{number}}`, { number: Number }),
		creditRoleLabel: insert("Credit role {{number}}", { number: Number }),
		addCreditAttribution: "Add credit",
		removeCreditAttribution: insert("Remove credit {{number}}", { number: Number }),
		creditEntityRequired: `Choose an ${entityTerms.inline}.`,
		creditRoleRequired: "Choose a role.",
		creditDuplicate: `This ${entityTerms.inline} already has this role.`,
		creditRequestConfirmationTitle: "Confirm credit invitations",
		creditRequestConfirmationDescription: `Some selected ${entityTerms.plural} require consent before their credits can appear. Confirm to create this work and send the invitations.`,
		creditRequestConfirmationCancel: "Cancel",
		creditRequestConfirmationConfirm: "Create and send invitations",
		metadataOnlyConfirmationTitle: "Provide the full content?",
		metadataOnlyConfirmationDescription: `Turning off “${metadataTerms.label} only” means ${verbatimTerms.rezics.value} may present the work’s full content. This may be appropriate under an open ${licenseTerms.inline}, in the public domain, or with direct authorization. This setting and any selected ${licenseTerms.label} are declarations only; ${verbatimTerms.rezics.value} has not verified that the person making them has the required rights.`,
		metadataOnlyConfirmationCancel: `Keep ${metadataTerms.inline} only`,
		metadataOnlyConfirmationConfirm: "Provide full content",
		versionRole: "Version type",
		mainVersion: "Main version",
		variantVersion: "Variant version",
		mainVersionEntity: "Related Main version",
	},
	ownershipClaim: {
		action: "Claim this entry",
		pendingAction: "Claim under review",
		title: "Claim entry ownership",
		description:
			"Submit a claim if you are entitled to represent the person, organization, brand, or work described by this entry.",
		detailsLabel: "Basis for your claim",
		detailsPlaceholder:
			"Explain your relationship to this entry and how the platform can verify it.",
		detailsHint:
			"Provide public sources that establish the representative or rights relationship. Do not submit confidential personal information.",
		effectNotice: `Approval transfers management ownership of this ${verbatimTerms.rezics.value} entry to your Profile. It is not identity certification or a determination of copyright or other legal rights.`,
		submit: "Submit claim",
		cancel: "Cancel",
		submitted: "Claim submitted",
		submittedDescription: "You will be notified after platform governance reviews it.",
		pendingTitle: "Claim under review",
		pendingDescription: "Platform governance has not yet decided this entry ownership claim.",
		submittedDetails: "Your submitted basis",
		withdraw: "Withdraw claim",
		withdrawn: "Claim withdrawn",
		withdrawnDescription: "This entry ownership claim is no longer awaiting review.",
	},
	attributionRoles: {
		author: "Author",
		"co-author": "Co-author",
		translator: "Translator",
		illustrator: "Illustrator",
		editor: "Editor",
		publisher: `${enTerminology.publisher.forms.label}`,
		letterer: "Letterer",
		colorist: "Colorist",
		developer: "Developer",
		composer: "Composer",
		designer: "Designer",
		director: "Director",
		producer: "Producer",
		writer: "Writer",
		actor: "Actor",
		narrator: "Narrator",
		studio: verbatimTerms.studio.value,
		distributor: "Distributor",
	},
	subjectAssociationRoles: {
		primary_character: "Primary character",
		featured_character: "Featured character",
		appears: "Appears",
		about: "About",
		setting: "Setting",
		source_work: "Source work",
		canonical_wiki_page: "Canonical knowledge page",
		related_subject: "Related subject",
	},
	workspace: {
		title: "Manage unit",
		description: `Edit content, ${metadataTerms.inline}, Unit relationships, access, and revision history.`,
		backToUnit: "Back to unit",
		backToOverview: "Back to settings",
		navigation: "Unit management navigation",
		overview: "All management tools",
		sections: {
			content: {
				label: "Content",
				description: "Edit titles, summaries, descriptions, and covers in each content language.",
			},
			metadata: {
				label: metadataTerms.label,
				description: `Manage lifecycle, visibility, rating, ${enTerminology.license.forms.inline}, and type-specific ${metadataTerms.inline}.`,
			},
			relationships: {
				label: "Unit relationships",
				description: "Manage credits, subjects, and variant relationships.",
			},
			tags: {
				label: "Tags",
				description: "Feature global Tags and arrange the order in which they appear first.",
			},
			realms: {
				label: `${realmTerms.label} publishing`,
				description: `Manage publishing, withdrawal, and republishing for each ${realmTerms.label}.`,
			},
			contentStructure: {
				label: "Content structure",
				description: `Manage Book ${enTerminology.chapter.forms.plural} or Media items in a collapsible, indented tree editor.`,
				developmentBadge: "In development",
			},
			releases: {
				label: "Series releases",
				description: "Manage the units, order, and release dates within this Series.",
			},
			access: {
				label: "Access",
				description: "Manage authorization subjects, public permissions, and restrictions.",
			},
			history: {
				label: "Revision history",
				description: "Review, compare, undo, or restore content revisions.",
			},
		},
	},
	realmPublications: {
		title: `${realmTerms.label} publishing`,
		description: `Manage the permanent inclusion relationships between this content and ${realmTerms.pluralLabel}. Withdrawing stops publishing without deleting a relationship.`,
		addTitle: `Publish to a ${realmTerms.label}`,
		addDescription: `Add one ${realmTerms.label} at a time so its publishing permission and rules are checked independently.`,
		realmLabel: realmTerms.label,
		add: "Publish",
		publicationStateFilter: "Publishing state",
		realmStatusFilter: `${realmTerms.label} governance state`,
		all: "All",
		current: "Current governance states",
		publicationStates: { active: "Published", withdrawn: "Withdrawn" },
		realmStatuses: {
			pending: "Pending review",
			visible: "Approved",
			hidden: "Hidden",
			removed: "Removed",
		},
		effectivelyVisible: `Currently visible in the ${realmTerms.label}`,
		notEffectivelyVisible: `Not currently visible in the ${realmTerms.label}`,
		empty: "No publishing relationships match the current filters.",
		withdraw: "Withdraw",
		republish: "Republish",
		loadMore: "Load more",
		unnamedRealm: `Unnamed ${realmTerms.label}`,
		governanceAction: insert("Governance action: {{action}}", { action: String }),
	},
	series: {
		kind: "Series kind",
		releases: "Series content",
		addRelease: "Add work",
		releaseUnit: "Work",
		releasedOn: "Release date",
		removeRelease: "Remove from series",
		removeReleaseConfirm: insert(
			"Remove {{title}} from this series? The work itself will not be deleted.",
			{ title: String },
		),
		noReleases: "This series has no works yet.",
		moveEarlier: "Move earlier",
		moveLater: "Move later",
		viewAllReleases: "View all series content",
	},
	fields: {
		isbn13: verbatimTerms.isbn13.value,
		metadataOnly: `${metadataTerms.label} only`,
		metadataOnlyDescription: {
			book: `Keep the work details, contents, progress, and discussions, but do not present hosted ${enTerminology.chapter.forms.label} bodies. Existing bodies are not deleted.`,
			software: `Keep the ${enTerminology.software.forms.inline} details, versions, progress, and discussions, but do not present hosted packages, binaries, or source archives. Existing files are not deleted.`,
			media:
				"Keep the media details, contents, progress, and discussions, but do not present hosted media content. Existing content is not deleted.",
		},
		publicationDate: `${enTerminology.publication.forms.label} date`,
		pageCount: "Page count",
		releaseStatus: "Work release status",
		versionLabel: "Version label",
		parentUnit: "Parent Unit",
		mediaKind: "Media kind",
		releaseDate: "Release date",
		runtimeMinutes: "Runtime (minutes)",
		episodeCount: `${enTerminology.program.forms.episodeLabel} count`,
		seasonCount: `${enTerminology.program.forms.seasonLabel} count`,
		durationSeconds: "Duration (seconds)",
		adaptedAudio: `Adapted ${audioTerms.label}`,
		adaptedAudioDescription: `Select ${audioTerms.inline} that can replace this ${videoTerms.inline}'s original sound.`,
		removeAdaptedAudio: `Remove adapted ${audioTerms.inline}`,
		yes: "Yes",
		no: "No",
	},
	releaseStatuses: {
		ongoing: "Ongoing",
		hiatus: "On hiatus",
		completed: "Completed",
		cancelled: "Cancelled",
	},
	detail: {
		sections: "Unit sections",
		tabs: {
			book: {
				overview: "Book details",
				contents: "Contents",
				tags: "Tags",
				associations: "Associations",
				reviews: "Reader reviews",
				collections: "Collections",
				discussion: "Discussion",
			},
			media: {
				overview: "Media details",
				contents: "Contents",
				tags: "Tags",
				associations: "Associations",
				reviews: "Audience reviews",
				collections: "Collections",
				discussion: "Discussion",
			},
			software: {
				overview: `${enTerminology.software.forms.label} information`,
				requirements: "System requirements",
				tags: "Tags",
				associations: "Associations",
				reviews: "User reviews",
				collections: "Collections",
				discussion: "Discussion",
			},
			series: {
				overview: "Series details",
				releases: "Series content",
				tags: "Tags",
				associations: "Associations",
				reviews: "Series reviews",
				collections: "Collections",
				discussion: "Discussion",
			},
		},
		sectionDescriptions: {
			book: {
				overview: `Book description, ${enTerminology.publication.forms.inline} information, and reading state.`,
				contents: `Browse the ${enTerminology.chapter.forms.inline} structure and continue reading.`,
				tags: "Review global Tags and contextual judgments from your selected sources.",
				associations: "Review the Book's subjects, Series, and Main–Variant relationships.",
				reviews: "Read or write reader reviews with an optional Score.",
				collections: "Explore public Collections that include this Book.",
				discussion: `Browse ${postTerms.pluralLabel} that use this Book as their subject.`,
			},
			media: {
				overview: "Media description, release information, and viewing state.",
				contents: `Browse the ${videoTerms.inline} and ${audioTerms.inline} structure and track watching or listening progress.`,
				tags: "Review global Tags and contextual judgments from your selected sources.",
				associations: "Review this work's subjects, Series, and Main–Variant relationships.",
				reviews: "Read or write audience reviews.",
				collections: "Explore public Collections that include this work.",
				discussion: `Browse ${postTerms.pluralLabel} that use this work as their subject.`,
			},
			software: {
				overview: `${enTerminology.software.forms.label} description, version information, and usage state.`,
				requirements: "See system requirements by platform and tier.",
				tags: "Review global Tags and contextual judgments from your selected sources.",
				associations: `Review the ${enTerminology.software.forms.label}'s subjects, Series, and Main–Variant relationships.`,
				reviews: `Read or write ${enTerminology.software.forms.label} user reviews.`,
				collections: `Explore public Collections that include this ${enTerminology.software.forms.label}.`,
				discussion: `Browse ${postTerms.pluralLabel} that use this ${enTerminology.software.forms.label} as their subject.`,
			},
			series: {
				overview: "Series description, information, and ratings.",
				releases: "Browse the works in this series in their intended order.",
				tags: "Review global Tags and contextual judgments from your selected sources.",
				associations: "Review the subjects associated with this series.",
				reviews: "Read or write reviews of the series as a whole.",
				collections: "Explore public Collections that include this series.",
				discussion: `Browse ${postTerms.pluralLabel} that use this series as their subject.`,
			},
		},
		information: "Unit information",
		contentLanguages: "Content languages",
		credits: "Credits",
		aboutAuthor: "About the author",
		authorStatistics: {
			bookOne: "1 book",
			books: insert("{{count}} books", { count: String }),
			followerOne: `1 ${followTerms.follower}`,
			followers: insert(`{{count}} ${followTerms.follower}s`, { count: String }),
		},
		viewAllCredits: "View all credits",
		backToOverview: "Back to overview",
		backToDiscussion: "Back to discussion",
		subjectAssociations: "Subjects",
		externalLinks: "External links",
		tags: "Tags",
		versions: "Versions",
		variants: "Variants",
		variantsDescription: "Direct Main and Variant relationships within the same work.",
		viewAssociations: "View all associations",
		aliases: "Aliases",
		releasedOn: "Release date",
		license: licenseTerms.label,
		aiDisclosure: `${verbatimTerms.ai.value} disclosure`,
		type: "Type",
		updatedAt: "Last updated",
		primary: "Primary",
		main: "Main",
		version: "Variant",
		noVersions: "No other versions are available.",
		noVariants: "No other Variants are available.",
		noSeriesMemberships: "This work is not part of a Series.",
		noRequirements: "No system requirements are available.",
		requirementTier: "Requirement tier",
		requirementPlatform: "Platform identifier",
		requirementSource: "External link identifier",
		requirementNotSpecified: "Not specified",
	},
	rating: {
		general: "General",
		r15: verbatimTerms.r15.value,
		r18: verbatimTerms.r18.value,
		r18g: verbatimTerms.r18g.value,
	},
	aiDisclosure: {
		unknown: "Unknown",
		none: "None",
		ai_assisted: `${verbatimTerms.ai.value}-assisted`,
		ai_originated: `${verbatimTerms.ai.value}-originated`,
		machine_generated: "Machine-generated",
	},
	editor: {
		title: "Edit unit",
		settings: "Settings",
		languageCode: "Language code",
		relationships: "Unit relationships",
		creditRole: "Credit role",
		subjectRole: "Subject role",
		externalUrl: `External ${verbatimTerms.url.value}`,
		credit: "Add credit",
		subjectAssociation: "Add subject association",
		contextWikiPost: `Optional context wiki ${postTerms.label}`,
		externalLink: "Add external link",
		tag: "Add tag",
		version: "Attach version",
		canonicalUnit: "Primary unit",
		saveSettings: "Save settings",
		bookDraftScope: "Book takedown scope",
		bookDraftScopeDescription: `${enTerminology.chapter.forms.pluralLabel} are independent ${postTerms.pluralLabel}. Choose explicitly whether attached published ${enTerminology.chapter.forms.pluralLabel} should also become drafts.`,
		bookDraftChoose: "Choose the takedown scope",
		bookDraftBookOnly: "Draft the Book only",
		bookDraftBookAndChapters: `Draft the Book and manageable published ${enTerminology.chapter.forms.pluralLabel}`,
		bookChapterDraftQueued: `${enTerminology.chapter.forms.label} takedown is queued; ${enTerminology.chapter.forms.pluralLabel} you can manage will be processed in batches.`,
	},
	relationshipManagement: {
		credits: "Credits",
		creditsDescription: "People and organizations credited for creating or publishing this work.",
		subjects: "Subjects",
		subjectsDescription: "People, organizations, and topics depicted in or discussed by this work.",
		externalLinks: "External links",
		externalLinksDescription: "External pages and reference material connected to this work.",
		variants: "Main and variants",
		variantsDescription: "Manage the direct Main–Variant structure for this work.",
		addCredit: "Add credit",
		addCreditDescription: "Choose who to credit and the role they performed.",
		creditedEntity: "Credited person or organization",
		addSubject: "Add subject",
		addSubjectDescription: "Choose a subject, its relationship, and an optional context page.",
		subjectEntity: "Subject",
		addExternalLink: "Add external link",
		addExternalLinkDescription: `Add an external link and optionally identify its source ${entityTerms.inline}.`,
		sourceEntity: "Source",
		urlPlaceholder: `${verbatimTerms.https.value}://`,
		cancel: "Cancel",
		confirmRemove: "Remove",
		noCredits: "No credits yet",
		noCreditsDescription: "Add the people or organizations that contributed to this work.",
		noSubjects: "No subjects yet",
		noSubjectsDescription: "Add people, organizations, or topics represented by this work.",
		noExternalLinks: "No external links yet",
		noExternalLinksDescription: "Add a reliable public page that supports this work's information.",
		noVariants: "No variants yet",
		noVariantsDescription: "This work is the Main and has no connected Variants.",
		pendingRequests: "Pending consent",
		unavailableUnit: "Unavailable entry",
		contextPost: insert("Context: {{title}}", { title: String }),
		removeCreditTitle: "Remove credit?",
		removeCreditDescription: insert("Remove {{name}} from this work's credits?", {
			name: String,
		}),
		removeCreditLabel: insert("Remove credit for {{name}}", { name: String }),
		removeSubjectTitle: "Remove subject?",
		removeSubjectDescription: insert("Remove {{name}} from this work's subjects?", {
			name: String,
		}),
		removeSubjectLabel: insert("Remove subject {{name}}", { name: String }),
		removeExternalLinkTitle: "Remove external link?",
		removeExternalLinkDescription: insert("Remove the external link from {{name}}?", {
			name: String,
		}),
		removeExternalLinkLabel: insert("Remove external link from {{name}}", { name: String }),
		openExternalLinkLabel: insert("Open external link from {{name}}", { name: String }),
		creditAdded: "Credit added",
		subjectAdded: "Subject added",
		externalLinkAdded: "External link added",
		creditRemoved: "Credit removed",
		subjectRemoved: "Subject removed",
		externalLinkRemoved: "External link removed",
		requestConfirmationTitle: "Consent is required",
		requestConfirmationDescription:
			"This entry controls incoming associations. Send a request so its managers can review the relationship.",
		sendRequest: "Send request",
		requestSent: "Request sent",
		requestSentDescription: "The relationship will appear after the other side accepts it.",
		main: "Main",
		variant: "Variant",
		standalone: "Standalone work",
		standaloneDescription: "This work is not connected to a Main and has no Variants.",
		attachMain: "Choose Main",
		changeMain: "Change Main",
		changeMainDescription:
			"Choose a Main of the same work type. The current state is checked when saving.",
		mainUnit: "Main work",
		saveMain: "Save Main",
		mainUpdated: "Main updated",
		detachMain: "Detach from Main",
		detachMainTitle: "Detach from Main?",
		detachMainDescription:
			"This work will become standalone. The Main and its other Variants are unchanged.",
		mainDetached: "Main detached",
		mainUnavailable: "Main unavailable",
		mainUnavailableDescription:
			"The current Main is not readable. It cannot be safely changed from this screen.",
		promote: "Promote",
		promoteTitle: "Promote Variant to Main?",
		promoteDescription: insert(
			"Make {{name}} the Main? This work and the other Variants will move under it.",
			{ name: String },
		),
		promoteLabel: insert("Promote {{name}} to Main", { name: String }),
		promoted: "Variant promoted to Main",
	},
	references: {
		title: "Community references",
		description:
			"Propose and vote on search aliases and external links. Pinned references stay first; the rest are ranked by community votes.",
		aliasesTitle: "Aliases",
		aliasesDescription: "Alternative names that can help people find this Unit.",
		externalLinksTitle: "External links",
		externalLinksDescription: "Public pages that support information about this Unit.",
		aliasTerm: "Alias",
		aliasPlaceholder: "Enter an alternative name",
		proposeAlias: "Propose alias",
		proposeExternalLink: "Propose external link",
		sourceEntity: `Source ${entityTerms.inline}`,
		sourcePlaceholder: `Choose a source ${entityTerms.inline}`,
		externalUrl: `External ${verbatimTerms.url.value}`,
		urlPlaceholder: `${verbatimTerms.https.value}://`,
		pinned: "Pinned",
		voteSummary: insert("Score {{score}} · {{count}} votes", {
			score: String,
			count: String,
		}),
		support: "Support",
		oppose: "Oppose",
		pin: "Pin",
		unpin: "Unpin",
		moveEarlier: "Move earlier",
		moveLater: "Move later",
		withdraw: "Withdraw reference",
		withdrawConfirm: "Withdraw this reference? Its vote history will be retained.",
		aliasProposed: "Alias proposed",
		externalLinkProposed: "External link proposed",
		noAliases: "No aliases yet.",
		noExternalLinks: "No external links yet.",
	},
	contentLanguageSupport: {
		title: "Content consumption languages",
		description:
			"Specify the languages in which the content can be read, watched, heard, or operated. This does not affect access to the entry, community, reviews, or other features.",
		language: "Language",
		languagePlaceholder: "Select a language",
		addLanguage: "Add language",
		removeLanguage: "Remove language",
		channelLabel: "Supported channels",
		unqualifiedChannels: "Channels not specified",
		channels: {
			text: "Text",
			audio: audioTerms.label,
			subtitle: "Subtitles",
			interface: "Interface",
		},
		evidenceTitle: "Languages on related entries",
		evidenceDescription:
			"Review saved values from directly related entries. They are added to this field only when you adopt them.",
		loadEvidence: "View related entries",
		loadMoreEvidence: "Load more",
		noEvidence: "No directly related entries provide language information.",
		adoptEvidence: "Adopt",
		evidenceSources: {
			parent: "Parent Unit",
			main: "Main version",
			variant: "Variant",
			release: "Release",
			occurrence: "Content item",
			adapted_audio: `Adapted ${audioTerms.inline}`,
		},
	},
	contentLanguages: {
		controlLabel: "Content language",
		settings: "Language settings",
		dialogTitle: "Content language settings",
		dialogDescription:
			"Set the fallback display order used when a reader's preferred languages are unavailable.",
		add: "Add language",
		addDescription: "Choose a language, then save its required content to add it to this Unit.",
		pending: "Not added yet",
		drag: "Drag to reorder",
		moveFirst: "Move to first",
		moveUp: "Move up",
		moveDown: "Move down",
		moveLast: "Move to last",
		remove: "Remove language",
		removeConfirm: insert("Remove {{language}} and all of its content from this Unit?", {
			language: String,
		}),
		saveOrder: "Save order",
		cancel: "Cancel",
		noMoreLanguages: "All supported languages have already been added.",
		draftConflictTitle: "The server content changed",
		draftConflictDescription:
			"This unsaved draft was kept, but it was based on an older version. Review it before saving, or discard it to load the current server content.",
		discardDraft: "Discard draft",
		instructions:
			"Press Space to pick up a language. Use the arrow keys to move it, then press Space again to drop it.",
		moved: insert("Moved {{language}} to position {{position}} of {{count}}.", {
			language: String,
			position: Number,
			count: Number,
		}),
	},
	content: {
		title: "Contents",
		edit: "Edit contents",
		chapter: `${enTerminology.chapter.forms.label}`,
		label: "Label",
		structure: "Structure",
		root: "(Root)",
		structureSummary: insert(
			`{{books}} ${enTerminology.textVersion.forms.plural} · {{chapters}} ${enTerminology.chapter.forms.plural} · {{labels}} labels`,
			{
				books: Number,
				chapters: Number,
				labels: Number,
			},
		),
		mediaStructureSummary: insert(
			`{{media}} ${enTerminology.program.forms.plural} · {{videos}} ${videoTerms.plural} · {{audios}} ${audioTerms.inline} items · {{labels}} labels`,
			{ media: Number, videos: Number, audios: Number, labels: Number },
		),
		childCount: insert("{{count}} children", { count: Number }),
		totalWordCount: insert("{{count}} words total", { count: Number }),
		totalCharacterCount: insert("{{count}} characters total", { count: Number }),
		views: "Views",
		actions: "Content actions",
		selectionActions: "Selected item actions",
		rename: "Rename",
		moveToFirst: "Move to first at this level",
		moveToLast: "Move to last at this level",
		move: "Move",
		moveDescription: `Choose a label to move to the end of its children, or a ${enTerminology.chapter.forms.inline} to move directly below it.`,
		mediaMoveDescription:
			"Choose a label to move the selection to the end of its children, or a media item to move directly below it.",
		addChapter: `Add ${enTerminology.chapter.forms.inline}`,
		addBook: `Add ${enTerminology.textVersion.forms.inline}`,
		addMedia: `Add ${enTerminology.program.forms.inline}`,
		addLabel: "Add label",
		addVideo: `Add ${videoTerms.inline}`,
		addAudio: `Add ${audioTerms.inline}`,
		addChapterDescription: `Create a new ${enTerminology.chapter.forms.inline} or search for an existing ${enTerminology.chapter.forms.inline} to add. Completing this action also saves the content structure.`,
		chapterOwnership: `${enTerminology.chapter.forms.label} ownership`,
		addBookDescription: `The linked ${enTerminology.textVersion.forms.inline} keeps its own content and editing permissions.`,
		addMediaDescription: `The linked ${enTerminology.program.forms.inline} keeps its own content and editing permissions.`,
		profileOwnedChapter: `Profile-owned ${enTerminology.chapter.forms.label}`,
		communityOwnedChapter: `Community-owned ${enTerminology.chapter.forms.label}`,
		addLabelDescription:
			"Create a new label or search for an existing label to add. Completing this action also saves the content structure.",
		addVideoDescription: `Create a new ${videoTerms.inline} or search for an existing one to add. Completing this action also saves the content structure.`,
		addAudioDescription: `Create a new ${audioTerms.inline} item or search for an existing one to add. Completing this action also saves the content structure.`,
		addMode: "How to add",
		createMode: "Create",
		attachMode: "Add existing",
		existingChapter: `Existing ${enTerminology.chapter.forms.inline}`,
		existingBook: `Existing ${enTerminology.textVersion.forms.inline}`,
		existingMedia: `Existing ${enTerminology.program.forms.inline}`,
		existingLabel: "Existing label",
		existingMediaItem: "Existing media item",
		searchExistingChapter: `Search existing ${enTerminology.chapter.forms.plural}`,
		searchExistingBook: `Search existing ${enTerminology.textVersion.forms.plural}`,
		searchExistingMedia: `Search existing ${enTerminology.program.forms.plural}`,
		searchExistingLabel: "Search existing labels",
		searchExistingMediaItem: "Search existing media items",
		mediaKindFilter: "Media type",
		allMediaKinds: "All",
		createChapterAndSave: `Create ${enTerminology.chapter.forms.inline} and save`,
		createLabelAndSave: "Create label and save",
		createMediaItemAndSave: "Create media item and save",
		attachChapterAndSave: `Add ${enTerminology.chapter.forms.inline} and save`,
		attachBookAndSave: `Add ${enTerminology.textVersion.forms.inline} and save`,
		attachMediaAndSave: `Add ${enTerminology.program.forms.inline} and save`,
		attachLabelAndSave: "Add label and save",
		attachMediaItemAndSave: "Add media item and save",
		saveCurrentChangesNotice:
			"This action also saves the content structure changes that are currently unsaved.",
		choosePosition: "Choose structure position",
		choosePositionDescription: `Choose a label to append to its children, or a ${enTerminology.chapter.forms.inline} to insert directly below it.`,
		mediaChoosePositionDescription:
			"Choose a label to append to its children, or a media item to insert directly below it.",
		newChapter: `New ${enTerminology.chapter.forms.inline}`,
		newBook: `Add ${enTerminology.textVersion.forms.inline}`,
		newLabel: "New label",
		newMediaItem: "Add media item",
		newVideo: `New ${videoTerms.inline}`,
		newAudio: `New ${audioTerms.inline}`,
		newChapterAfter: `New ${enTerminology.chapter.forms.inline} below`,
		newLabelAfter: "New label below",
		newVideoAfter: `New ${videoTerms.inline} below`,
		newAudioAfter: `New ${audioTerms.inline} below`,
		expand: "Expand",
		collapse: "Collapse",
		multiSelect: "Multi-select",
		selectedCount: insert("{{count}} selected", { count: Number }),
		expandAll: "Expand all",
		collapseAll: "Collapse all",
		dragHandle: "Drag to move",
		noContent: `There are no ${enTerminology.chapter.forms.plural} yet.`,
		noMediaContent: `There are no ${videoTerms.plural} or ${audioTerms.inline} items yet.`,
		durationUnknown: "Duration not provided",
		markMediaItemComplete: "Mark complete",
		markMediaItemIncomplete: "Mark incomplete",
		mediaItemComplete: "Complete",
		mediaItemIncomplete: "Incomplete",
		saveDraft: "Save content structure",
		discardDraft: "Discard draft changes",
		unsavedDraft: "There are unsaved changes",
		draftHint:
			"Renaming and moving stay in the draft until saved. Creating or adding content saves every current change.",
		development: "The content structure editor is in development",
		developmentDescription: `Content structure is not available for this unit type yet. Preview ${verbatimTerms.api.value} access requires the development preview capability.`,
	},
	chapter: {
		title: `Edit ${enTerminology.chapter.forms.inline}`,
		backToStructure: "Back to content structure",
		language: `${enTerminology.chapter.forms.label} language`,
		useLanguage: "Switch language",
		save: `Save ${enTerminology.chapter.forms.inline}`,
		communityUnitContentPolicyTitle: `Public works do not provide ${enTerminology.chapter.forms.inline} body editing`,
		communityUnitContentPolicyDescription: `This Book is maintained by the community. You can organize its content structure or attach an existing ${postTerms.inline}, but this editor does not provide a ${postTerms.inline} body editor.`,
		wordCount: insert("{{count}} words", { count: Number }),
		characterCount: insert("{{count}} characters", { count: Number }),
	},
	reader: {
		backToContents: "Back to contents",
		settings: "Reader settings",
		fontSize: "Font size",
		chapterLanguage: `${enTerminology.chapter.forms.label} language`,
		automaticLanguage: "Automatic",
		markChapterComplete: `Mark ${enTerminology.chapter.forms.inline} complete`,
		markChapterIncomplete: `Mark ${enTerminology.chapter.forms.inline} incomplete`,
		read: "Read",
		unread: "Unread",
	},
} satisfies typeof import("../zh-Hant/units").default;
