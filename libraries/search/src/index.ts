...
const SearchRequestBase = {
	...
	cursor: Type.Optional(Type.String({ maxLength: 32, pattern: "^s_[0-9a-z]+$" })),
};
...
/** Cursor contents are deliberately an implementation detail of Search. */
export function createSearchCursor(offset: number): string {
	if (!Number.isSafeInteger(offset) || offset < 0)
		throw new TypeError("Search cursor offset must be a non-negative safe integer");
	return `s_${offset.toString(36)}`;
}

export function parseSearchCursor(value: string | undefined): number {
	if (value === undefined) return 0;
	if (!/^s_[0-9a-z]+$/.test(value)) throw new TypeError("Invalid Search cursor");
	const offset = Number.parseInt(value.slice(2), 36);
	if (!Number.isSafeInteger(offset) || createSearchCursor(offset) !== value)
