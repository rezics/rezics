...
The canonical visual reference is the **`@rezics/ui` Storybook** (`bun -F @rezics/ui storybook`, port 6007). Foundation pages (Tokens: Colors / Typography / Spacing / Radius / Elevation / Motion / Iconography; Patterns: Density / State Layer / Depth Without Shadow / Inverse Surface / Layout & Breakpoints / Voice) render live tokens. Every shadcn primitive and rezics primitive/composite has a story; density opt-in primitives also expose a `WithDensity` story rendering compact / comfortable / spacious side-by-side. The Storybook toolbar exposes a global **Density** switch (`compact` / `comfortable` / `spacious`) alongside the Light/Dark theme switch — both are class-toggles on `<html>` (`density-compact` / `density-spacious`, `dark`).
