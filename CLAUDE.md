...
The canonical visual reference is the **`@rezics/ui` Storybook** (`bun -F @rezics/ui storybook`, port 6007). Foundation pages (Tokens: Colors / Typography / Spacing / Radius / Elevation / Motion / Iconography; Patterns: Density / State Layer / Depth Without Shadow / Inverse Surface / Layout & Breakpoints / Voice) render live tokens and document the canonical patterns. Every shadcn primitive and rezics primitive/composite has a story. Density is documented at `Foundation/Patterns/Density` as an intrinsic component vocabulary backed by the fixed `--padding-*` tokens; it is not a compact/comfortable/spacious runtime toolbar. The Storybook toolbar exposes Light/Dark only.
