...
- `slug-validation`: extend reserved-word rules to a two-layer model — per-scope reserved words (one list per `SlugScope`) plus per-owner reserved words (`profile`, `settings`, `shelf`, `post`, etc. under a user / realm owner). Add typed-error rejection for `ENTITY` slug writes (gated until `entity-slug-activation`).
