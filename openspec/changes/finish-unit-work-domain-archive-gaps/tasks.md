...
- [x] 4.4 Convert internal app/api imports that do not need book-specific semantics to generic names.
- [x] 4.5 Keep `targetUnitId` reserved for interactions; content-structure node identity must remain `contentUnitId`.

## 5. Seeds, Factories, And Cross-Domain Consumers

- [x] 5.1 Update book factories/seeds to write generic content-structure rows.
- [x] 5.2 Provide helper APIs for game/media/series fixtures to create content-structure nodes for release parts or series members.
- [x] 5.3 Update seed reports and fixture docs to use `ownerUnitId` / `contentUnitId` terminology.
...
- [x] 6.2 Add job-runner handlers that process membership moves, legacy release sync, optional metadata copy, and repair enqueueing in resumable batches.
- [x] 6.3 Persist item-level progress and enough before-state for revert as each batch completes.
- [x] 6.4 Ensure operation status reflects queued/running/completed/failed states after repair commands are enqueued.
- [x] 6.5 Keep dry-run preview synchronous, but make active merge execution async/resumable.

## 7. Verification

- [x] 7.1 Run targeted contract tests for generic content-structure DTOs and compatibility aliases.
- [x] 7.2 Run targeted server tests for generic tree read/write/path resolution, book wrappers, chapter materialization, and history events.
- [x] 7.3 Run migration/parity tests for existing book content-structure rows.
- [x] 7.4 Run job-runner/server tests for queued admin work merge execution and resumability.
