...
- [ ] 0.1 Confirm `complete-rezics-token-system`, `migrate-to-theme-config-classes`, and `unify-tokens-single-source` are landed: `bun run check:convention` passes (R9 active); `package/ui/src/config/tokens.css` does not exist; `--colors-*` / `--font-*` / `--radius-*` / `--shadow-*` / `--duration-*` / `--easing-*` / `--state-*-opacity` are all emitted by the preflight in `package/ui/src/config/uno-config.ts`.
- [ ] 0.2 Confirm `package/ui/src/docs/tokens/_gallery.tsx` exports: `Grid`, `Swatch`, `Row`, `SpacingRuler`, `RadiusSample`, `ElevationSample`, `TypeSample`, `MotionSample`, `Do`, `Dont`, `Compare`. (Already verified during proposal review on 2026-05-04.) Phase 3 extends this file with four new exports.
- [ ] 0.3 Confirm Storybook 10 runs on the configured ports per `design-system-storybook` Requirement-2: `bun -F @rezics/ui storybook` starts successfully.
- [ ] 0.4 Confirm the existing 6 Foundation MDX pages render: `Foundation/Tokens/Colors`, `…/Typography`, `…/Spacing`, `…/Radius`, `…/Elevation`, `…/Motion`. Each shows live tokens. (No edits in this phase; verification only.)
- [ ] 0.5 Confirm the existing 2 patterns MDX pages render: `Foundation/Voice` (`docs/voice.mdx`) and `Foundation/Patterns` (`docs/patterns.mdx`).
...
- [ ] 1.1 Extend `package/ui/src/config/uno-config.ts` with a `DENSITY_STEP` constant and emit `--density-step` from the preflight:
...
- [ ] 1.2 In the same file, define and emit the component-tier `--padding-*` tokens for the opt-in list. Each is `calc(<base> + var(--density-step))`. Final list (final base values pinned during 4.1 visual review):
...
- [ ] 1.4 Run `bun -F @rezics/ui storybook` and confirm the new toolbar entry appears alongside the Light/Dark toggle.
...
- [ ] 2.1 Author `package/ui/src/docs/tokens/iconography.mdx`:
...
- [ ] 2.3 (Per the Colors-page contrast-badge addition in proposal `What Changes`.) Extend `_gallery.tsx`'s `Swatch` to compute and display contrast against the closest `on-*` role; apply to the Colors page swatches. Defer if the existing `Swatch` already shows contrast badges.
...
- [ ] 3.1 Extend `package/ui/src/docs/tokens/_gallery.tsx` with the four new Patterns helpers:
...
- [ ] 3.3 Author `package/ui/src/docs/patterns/state-layer.mdx` (`<Meta title="Foundation/Patterns/State Layer" />`):
...
- [ ] 3.4 Author `package/ui/src/docs/patterns/depth-without-shadow.mdx` (`<Meta title="Foundation/Patterns/Depth Without Shadow" />`):
...
- [ ] 3.5 Author `package/ui/src/docs/patterns/inverse-surface.mdx` (`<Meta title="Foundation/Patterns/Inverse Surface" />`):
...
- [ ] 3.6 Author `package/ui/src/docs/patterns/layout-and-breakpoints.mdx` (`<Meta title="Foundation/Patterns/Layout & Breakpoints" />`):
...
- [ ] 4.1 For each composite/primitive in the opt-in list, update the source to consume the component-tier token. Files to touch (verify each; some may already use a CSS variable):
...
- [ ] 4.2 For each opt-in composite/primitive, add a `WithDensity` story rendering compact / comfortable / spacious side-by-side via `<DensityDemo>`. Tick alongside the corresponding Phase 5 entry (e.g. `Phase 5.X` for shadcn primitives).
...
- [ ] 5.1 Alert (`alert.tsx`)
- [ ] 5.2 Avatar (`avatar.tsx`)
- [ ] 5.3 Badge (`badge.tsx`)
- [ ] 5.4 Breadcrumb (`breadcrumb.tsx`) — also `WithDensity`
- [ ] 5.5 Button (`button.tsx`)
- [ ] 5.6 Card (`card.tsx`)
- [ ] 5.7 Carousel (`carousel.tsx`)
- [ ] 5.8 Chart (`chart.tsx`)
- [ ] 5.9 Checkbox (`checkbox.tsx`)
- [ ] 5.10 Collapsible (`collapsible.tsx`)
- [ ] 5.11 Command (`command.tsx`) — also `WithDensity`
- [ ] 5.12 ContextMenu (`context-menu.tsx`) — also `WithDensity`
- [ ] 5.13 Dialog (`dialog.tsx`)
- [ ] 5.14 Drawer (`drawer.tsx`)
- [ ] 5.15 DropdownMenu (`dropdown-menu.tsx`) — also `WithDensity`
- [ ] 5.16 Input (`input.tsx`) — also `WithDensity`
- [ ] 5.17 Label (`label.tsx`)
- [ ] 5.18 Popover (`popover.tsx`)
- [ ] 5.19 Select (`select.tsx`) — also `WithDensity`
- [ ] 5.20 Separator (`separator.tsx`)
- [ ] 5.21 Sheet (`sheet.tsx`)
- [ ] 5.22 Sidebar (`sidebar.tsx`) — also `WithDensity`
- [ ] 5.23 Skeleton (`skeleton.tsx`)
- [ ] 5.24 Sonner (`sonner.tsx`)
- [ ] 5.25 Table (`table.tsx`) — also `WithDensity`
- [ ] 5.26 Tabs (`tabs.tsx`) — also `WithDensity`
- [ ] 5.27 ThemeSwitch (`theme-switch.tsx`) — rezics-custom; covers light/dark toggle behaviour.
- [ ] 5.28 Toggle (`toggle.tsx`)
- [ ] 5.29 ToggleGroup (`toggle-group.tsx`)
- [ ] 5.30 Tooltip (`tooltip.tsx`)
- [ ] 5.31 (rezics primitive) `package/ui/src/primitive/button/TextButton.stories.tsx` — the missing 14th primitive story identified by the audit.
...
- [ ] 6.1 `bun -F @rezics/ui run build-storybook` — `storybook-static/index.json` contains:
  ...
  - The 13 existing rezics-primitive entries and the 21 composite entries all still register.
...
- [ ] 6.4 `bun run check:tokens` passes (contrast script — should be unaffected by this change but verify).
- [ ] 6.5 `bun run check:convention` passes — **R9 must not regress**: no `--rezics-*` references introduced; `package/ui/src/config/tokens.css` does not appear.
