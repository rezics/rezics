...
A composed React component that renders a fully configured markdown editor with toolbar, preview, and all standard formatting commands. It wraps the generic `<Editor>` with `markdownFull()` plugins preconfigured.
...
A composed React component that renders a fully configured JSON editor with syntax highlighting, format button with icon, and keyboard shortcut support.
...
A composed React component that renders a plain text/code editor with no language-specific plugins.
