...
- [x] 1.1 Add `scripts/i18n/analyze-duplicates.ts` that reads
...
- [x] 1.2 Run the analyzer and review the output. Annotate each group
...
- [x] 1.3 Commit `scripts/i18n/analyze-duplicates.ts` and the annotated
  `dedup-report.json`. No source files are modified in this commit.

## 2. Namespace map and split (non-breaking)

- [x] 2.1 Add `scripts/i18n/namespace-map.ts` exporting a typed
...
- [x] 2.2 Add `scripts/i18n/split-locales.ts` that reads
...
- [x] 2.3 Add `scripts/i18n/key-map.json` emitted by the split:
...
- [x] 2.4 Run the splitter. Verify by reloading the JSON output and
  ...
  delta.
- [x] 2.5 Commit `scripts/i18n/{namespace-map.ts,split-locales.ts,key-map.json}`
...
- [x] 3.1 Add runtime dependencies to `package/i18n/package.json`:
  `i18next`, `react-i18next`, `i18next-http-backend`,
  `i18next-browser-languagedetector`. Keep `@inlang/paraglide-js` for
  now.
- [x] 3.2 Create `package/i18n/src/runtime.ts` that exports an
...
- [x] 3.3 Create `package/i18n/src/react.tsx` that re-exports
...
- [x] 3.4 Add a `ResourceNamespaceMap` declaration in
...
- [x] 4.2 Add an HTTP cache header strategy: long max-age on the
...
- [x] 4.3 Add a Storybook locale-provider shim in
...
- [x] 5.1 Add `scripts/i18n/codemod-paraglide-to-i18next.ts` (jscodeshift
...
- [x] 5.2 Run the codemod against `package/app/src/`,
...
- [x] 5.3 Wrap each app's root in `RezicsI18nProvider`. In
...
- [x] 5.4 Replace any remaining usages of `useMessage` from
  `@rezics/i18n/react` with `useTranslation`. Delete the
  `useMessage` export.
- [x] 5.5 Run `bun run check:convention`, `bun run format:check`,
...
- [x] 6.1 Create `package/ui/locales/en.ts` etc. by reading the seven
  ...
  message keys.
- [x] 6.2 Add `package/ui/src/i18n/register.ts` exporting
...
- [x] 6.5 Update the Storybook locale shim from task 4.3 to also call
  `registerUiLocale`.
- [x] 6.6 Delete `package/ui/messages/`, `package/ui/project.inlang/`,
  `package/ui/src/paraglide/`, and the `i18n:compile` script in
  `package/ui/package.json`.

## 7. `check:i18n` script

- [x] 7.1 Add `scripts/i18n/check-i18n.ts` that:
...
- [x] 8.4 Update `package/i18n/package.json` exports to remove
...
- [x] 8.5 Run `bun install` to confirm the lockfile updates and no
  Paraglide-related warnings remain.

## 9. Validation

- [x] 9.1 Run `bun run format:check`, `bun run check:convention`,
  `bun run check:tokens`, `bun run check:i18n`, and `bun run knip`
  from the repo root; all SHALL pass.
- [x] 9.2 Run `bun --filter=@rezics/{app,admin,ui,i18n,storybook-config}
...
- [x] 9.3 Run `bun --filter=@rezics/app run build` and
...
- [x] 9.4 Manual verification under `bun run dev`:
  ...
  - Switch language via LangToggle, observe parallel refetches of
    ...
    component remount.
  - Repeat for admin.
- [x] 9.5 Confirm `localStorage.getItem('rezics-locale')` persists the
  user's selected locale across reloads.

## 10. Documentation and cleanup

- [x] 10.1 Update `CONTRIBUTING.md` (or equivalent) section on i18n:
...
- [x] 10.2 Update `AGENTS.md` if it contains any Paraglide-specific
  guidance.
- [x] 10.3 Update the `rezics-design` skill (if it references i18n
  patterns) to use `t('<ns>:<key>')` in examples.
- [x] 10.4 Delete obsolete helper scripts under `scripts/i18n/` that
