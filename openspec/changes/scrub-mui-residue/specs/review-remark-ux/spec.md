...
**Reason**: Superseded by "Review creation page" (preserved above as a MODIFIED requirement). The obsolete version specified MUI `<Rating>`; the active version specifies `<RatingInput>` from `@rezics/ui`. MUI is permanently removed from the project, so the prior contract cannot be implemented.

**Migration**: None at the codebase level — the review creation form already uses `<RatingInput>`.

### Requirement: Review editing

**Reason**: Superseded by "Review edit page uses RatingInput" (preserved above). The obsolete version specified MUI `<Rating>` in the edit form scenarios; the active version specifies `<RatingInput>`.

**Migration**: None at the codebase level — the review edit page already uses `<RatingInput>`.

### Requirement: Remark inline creation form

**Reason**: Superseded by "Inline remark form on review tab" (preserved above). The obsolete version specified MUI `<Rating>` for the score input on the book detail review tab; the active version specifies `<RatingInput>`.

**Migration**: None at the codebase level — the remark inline form already uses `<RatingInput>`.

### Requirement: Remark edit dialog

**Reason**: Superseded by "Remark edit dialog uses RatingInput" (preserved above). The obsolete version specified MUI `<Rating>` in the edit dialog; the active version specifies `<RatingInput>`.

**Migration**: None at the codebase level — the remark edit dialog already uses `<RatingInput>`.
