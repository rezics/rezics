...
**Reason**: Superseded by "shadcn-or-custom component policy" (preserved above as a MODIFIED requirement). MUI is permanently removed from the project; the active component-selection rule is shadcn-default with custom-primitive fallback. The skill file `mui-vs-shadcn.md` it references is deleted in this change. Keeping the obsolete MUI-first requirement creates two contradictory statements about which library to pick.

**Migration**: Delete `.claude/skills/rezics-design/mui-vs-shadcn.md`. The replacement decision flow lives in `.claude/skills/rezics-design/component-selection.md`, which is rewritten in this change to drop any MUI naming. No code-level migration is needed; no source file imports from MUI today.
