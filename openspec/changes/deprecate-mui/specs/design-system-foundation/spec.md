...
### Requirement: MUI theme imports tokens, not literals

**Reason:** The MUI theme adapter (`package/ui/src/config/theme.ts`) is removed by this change. Tokens no longer project to a MUI theme object; they project only to CSS custom properties (consumed via UnoCSS classes and `var(--rezics-*)` references) and a small TypeScript breakpoints export.

**Migration:** Every previous consumer of `getTheme(mode)` or `getDynamicTheme(...)` is migrated to either (a) UnoCSS classes, (b) inline `var(--rezics-*)` references in CSS, or (c) a TypeScript constant from `package/ui/src/config/breakpoints.ts` for the rare JS-side breakpoint need. No spec consumer remains for the MUI projection.

### Requirement: MUI theme exposes light and dark themes

**Reason:** `@rezics/ui` no longer exports `lightTheme`, `darkTheme`, `getTheme`, or `getDynamicTheme`. Theme is expressed entirely through the `[data-theme]` attribute switching the `--rezics-*` CSS custom-property cascade. No MUI theme object exists in the project.

**Migration:** Consumers that previously imported `getTheme` (Storybook preview, app bootstrap) have been migrated to no-op — the layers.css cascade plus the `[data-theme]` attribute deliver the same light/dark behavior without a JavaScript-side theme object. Dynamic theme generation (the cover-image-driven palette) has been removed; the affected composite falls back to a static gradient using `--rezics-color-brand-fill` and `--rezics-color-surface-elevated`. Reintroducing dynamic theming requires a future change with a non-Material-3 color-extraction backend.

## ADDED Requirements

### Requirement: Theme switches via `[data-theme]` attribute alone

The light/dark theme SHALL switch via the `[data-theme]` attribute on the `<html>` element. The `--rezics-*` CSS custom-property cascade SHALL deliver all mode-sensitive token values. There SHALL NOT be a JavaScript-side theme object, a React context provider, or a MUI `ThemeProvider` / `StyledEngineProvider` in the runtime path.
