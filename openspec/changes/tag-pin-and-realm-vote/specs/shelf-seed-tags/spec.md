...
**Reason**: The "set score ≥ 1000 to make this tag prominent" pattern conflated editorial intent with community signal. Seed tags now achieve prominence through the orthogonal `pinned = true` axis introduced by this change, leaving `score` to reflect genuine community voting.

**Migration**: The seed installer SHALL be updated to omit the legacy score boost and instead set `pinned = true` with a low deterministic `position`. Existing seed-created UnitTag rows whose `score ≥ 1000` SHALL be migrated by the global migration step defined in the `tag-scoring` capability change (set `pinned = true`, assign `position`, reset `score` to `Σ TagVote.value`, reset `voteCount` to `count(TagVote)`).
