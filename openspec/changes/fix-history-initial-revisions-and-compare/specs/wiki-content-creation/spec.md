...
Wiki-capable creation paths that create content in editorial content-history
scope SHALL emit an initial content-history revision in the same transaction as
canonical creation. The creation actor SHALL be recorded as the history actor
even when wiki mode assigns ownership to the seeded `rezics-wiki` User.
