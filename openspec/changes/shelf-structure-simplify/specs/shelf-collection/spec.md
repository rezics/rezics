...
**Reason**: The `keywords: String[]` field on `ShelfItem` is deleted. Per-item tagging is now expressed via `ShelfUnit` rows with `role = 'tag'` (unit-id references to Tag units), managed separately from the collection flow. The `User.keywords` vocabulary field is also deleted as it no longer has any consumer.

**Migration**: Existing `keywords` values that resolve to an existing Tag unit SHALL be migrated to `ShelfUnit` rows with `role = 'tag'` during the data migration. Unresolvable free-text keywords SHALL be dropped. `User.keywords` SHALL be dropped without replacement.

### Requirement: Collection status includes review attachment via ShelfItemReview

**Reason**: `ShelfItemReview` is deleted. Review attachments live in `ShelfUnit` rows with `role = 'review'`, indexed by `@@index([unitId, role])`.

**Migration**: The collection status endpoint SHALL check `ShelfUnit` with `WHERE unitId = reviewId AND role = 'review'` instead of querying `ShelfItemReview`.
