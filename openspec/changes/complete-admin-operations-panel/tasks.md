...
- [ ] 3.1 Add cross-type Unit search/list filters by id, slug, title, type, owner, status, visibility, and drift flags.
- [ ] 3.2 Normalize entity/tag/realm/shelf/source-site pages into shared content operation patterns.
- [ ] 3.3 Add work/release grouping and repair surfaces when `introduce-unit-work-domain` is active.
...
- [ ] 4.1 Add admin APIs and clients for dry-run drift checks and repair jobs.
...
- [ ] 5.1 Integrate system status summary into the dashboard while preserving full status page behavior.
- [ ] 5.2 Expand Meili observability with index health, counts, settings, last sync, drift, and repair links.
- [ ] 5.3 Add job-runner queue and failed-job panels.
