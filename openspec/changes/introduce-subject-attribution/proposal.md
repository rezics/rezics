...
- Re-scope the current `Attribution` concept as creator/production credits. In implementation this becomes `CreditAttribution` in code/API/Prisma naming, with the physical table renamed in the migration.
...
- `attribution`: Existing Attribution semantics are narrowed to credit attribution and cut over to `CreditAttribution` naming.
...
- Database impact: add a new `SubjectAttribution` table. Rename the existing Attribution table/model to `CreditAttribution` in a development-stage cutover.
- API impact: add subject attribution link/unlink/list endpoints and contract DTOs; rename existing attribution credit DTOs and APIs to credit-specific names.
