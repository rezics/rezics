...
The route `/realm/:realmId/manage` SHALL render a management page accessible only to users with owner or admin role in the realm. The page SHALL allow editing realm metadata (name, description, public/private setting), managing member roles, and deleting the realm (owner only).
...
- **WHEN** a user without owner or admin role navigates to `/realm/:realmId/manage`
- **THEN** the page SHALL deny access and redirect the user to the realm detail page
...
The realm frontend SHALL conditionally display management controls based on the authenticated user's role within the realm. Moderator and above SHALL see tag management controls on the Tags tab. Admin and above SHALL see member role management controls on the Members tab and access to the management page. Only the owner SHALL see the realm deletion option.

#### Scenario: Moderator sees tag management controls
- **WHEN** a user with moderator role views the realm detail page
- **THEN** the Tags tab SHALL display controls for adding and removing tag-unit associations
- **AND** the Members tab SHALL NOT display role management controls

#### Scenario: Admin sees member role controls and management page link
- **WHEN** a user with admin role views the realm detail page
- **THEN** the Tags tab SHALL display tag management controls
- **AND** the Members tab SHALL display role management controls
- **AND** a link to the management page SHALL be visible

#### Scenario: Owner sees all controls including deletion
- **WHEN** a user with owner role views the realm detail page
- **THEN** all management controls SHALL be visible including tag management, role management, and a link to the management page
- **AND** the management page SHALL include the realm deletion option

#### Scenario: Regular member sees no management controls
- **WHEN** a user with member role views the realm detail page
- **THEN** no tag management controls, role management controls, or management page link SHALL be visible

#### Scenario: Non-member sees no management controls
- **WHEN** a non-member user views a public realm detail page
- **THEN** no management controls SHALL be visible
