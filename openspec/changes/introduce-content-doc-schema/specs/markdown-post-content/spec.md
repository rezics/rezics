...
Post and chapter content SHALL be read from `Post.content.main` using the shared `ContentDoc` schema. When `content.main.type = "markdown"`, the renderer SHALL render `content.main.source` via `MarkdownContent` and `createRezicsRenderer`. `Post.body` SHALL NOT exist as a canonical DTO or database field after this change.
...
The inline post form (`InlinePostForm`) SHALL use `RezicsMarkdownEditor` for content input. On submit, the editor output SHALL be stored as `ContentDoc.main.source` with `main.type = "markdown"` rather than as `Post.body`.
...
- **THEN** the markdown source text is stored in `Post.content.main.source`
