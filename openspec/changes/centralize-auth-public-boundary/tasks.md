...
- [ ] 1.1 Add or rename server/auth environment variables for auth internal base URL, public auth base URL, and public issuer URL.
- [ ] 1.2 Update runtime env validation in `package/server` and `package/auth` for the new auth URL split.
- [ ] 1.3 Update development `.env` examples or docs so browser-facing auth URLs point to main `/auth/*`.
- [ ] 1.4 Add a public route classification table in server code or docs for auth-domain, main-domain, and mixed `/auth/*` routes.

## 2. Main Auth Public Proxy

- [ ] 2.1 Implement main server `/auth/*` routing that maps eligible public paths to auth internal `/api/auth/*`.
- [ ] 2.2 Preserve request method, query string, body, selected headers, and auth session cookies when proxying auth-owned routes.
- [ ] 2.3 Normalize proxied `Set-Cookie` headers so auth session cookies use public `Path=/auth`.
- [ ] 2.4 Normalize proxied redirect `Location` headers so browser-visible redirects use main public URLs.
- [ ] 2.5 Block or omit public exposure of `GET /auth/token` and any equivalent auth session JWT acquisition path.
...
- [ ] 3.1 Implement main-owned `POST /auth/session/refresh`.
- [ ] 3.2 Validate the opaque auth session cookie by calling auth internally instead of parsing it in main.
- [ ] 3.3 Provision or verify the corresponding main user and readiness state during refresh.
- [ ] 3.4 Issue or refresh `rezics-session-token` as an httpOnly cookie with production-safe attributes.
- [ ] 3.5 Stop returning browser-storable main session JWTs from normal web refresh/exchange flows.
- [ ] 3.6 Add CSRF and origin checks for `/auth/session/refresh`, sign-out, and other cookie-authenticated mutating auth routes.
- [ ] 3.7 Ensure sign-out clears the main `rezics-session-token` cookie and delegates auth session invalidation to auth.
...
- [ ] 4.1 Update auth OAuth/OIDC configuration so public issuer and endpoint URLs are generated from public main config.
- [ ] 4.2 Ensure public OAuth routes are reachable through main `/auth/oauth/*` and callbacks through `/auth/callback/:provider`.
- [ ] 4.3 Preserve standards-based public `POST /auth/oauth/token` while keeping auth session JWT acquisition blocked.
- [ ] 4.4 Publish auth/OIDC JWKS through an auth-scoped public path such as `/auth/session/jwks`.
- [ ] 4.5 Keep main `/.well-known/jwks.json` reserved for `rezics-session-token` verification.
- [ ] 4.6 Add or update discovery metadata so it advertises public main URLs and never internal auth service URLs.
...
- [ ] 6.1 Update `@rezics/contract` session/token schemas so `rezics-session-token.sub` and explicit actor fields represent `userId`.
- [ ] 6.2 Remove `unitId` as the trusted actor identity from main session claim contracts.
- [ ] 6.3 Update server token signing and verification code to issue and validate actor `userId` claims.
...
- [ ] 7.1 Replace browser usage of direct auth host env variables with main-relative `/auth/*` routes in `package/api`, `package/app`, and `package/admin`.
...
- [ ] 7.4 Update API client request handling to rely on credentials-included cookies and refresh through `/auth/session/refresh`.
