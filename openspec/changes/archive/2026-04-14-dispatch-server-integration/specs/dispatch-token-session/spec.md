...
- **THEN** the server authenticates the token, looks up the owner's `unitId` and role, calls `signRezicsSessionToken({ unitId, role })`, and returns `{ token: "<rezics-session-token>" }` with status 200
...
The `rezics-session-token` issued by `POST /token/session` SHALL have the same claims schema as tokens issued by `POST /session/exchange`: `sub`, `unitId`, `permission.role`, `iss` ("rezics-server"), `exp`, `iat`. The TTL SHALL use the same `MAIN_SESSION_JWT_TTL_SECONDS` configuration (default 900s).

#### Scenario: Token claims match standard schema

- **WHEN** a session JWT is issued via `POST /token/session`
- **THEN** the token contains `sub` (unitId), `unitId`, `permission.role` (current DB role), `iss` ("rezics-server"), and valid `exp`/`iat` timestamps
