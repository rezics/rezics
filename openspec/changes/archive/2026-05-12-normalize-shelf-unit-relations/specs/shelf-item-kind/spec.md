...
**Reason**: `ShelfItem` is replaced by `ShelfUnit`.

**Migration**: Move the same write-time discriminator semantics to `ShelfUnit.kind`.

### Requirement: ShelfItem kind value set

**Reason**: The canonical contract type is renamed.

**Migration**: Replace `ShelfItemKind` with `ShelfUnitKind` in shelf APIs and app code.

### Requirement: Kind derived at write time only

**Reason**: Replaced by the `ShelfUnit kind is stored at write time` requirement.

**Migration**: Preserve write-time derivation while renaming the model and type.
