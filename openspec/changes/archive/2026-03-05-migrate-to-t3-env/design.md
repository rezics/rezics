...
- Unify schema validation by removing `zod` and standardizing on TypeBox (`import { t } from '@package/contract'`).
...
- **Use App-Specific `env.ts` Files over a Global Shared Module:**
  ...
  - *Alternatives Considered:* A monorepo-wide shared `@package/env` module. Rejected because it complicates managing client vs. server environment scopes with `t3-env`.
...
- **Risk: Frontend Bundling of Server Code.** Importing `t` from `@package/contract` (which re-exports `elysia`) might inadvertently bundle Elysia server code into the Vite frontend.
  - *Mitigation:* We should ensure that `@package/contract` uses `import { t } from 'elysia'` in a way that is tree-shakeable, or directly re-export TypeBox from `@sinclair/typebox` if bundling issues arise.
