...
- [x] 2.1 Update `@package/contract/src/schema.ts` to export Elysia `t` securely for both frontend and backend
- [x] 2.2 Verify `@package/contract` tree-shaking to ensure Elysia server dependencies do not leak into Vite bundles

## 3. Server Validation (Elysia)

- [x] 3.1 Create `package/server/src/env.ts` using `t3-env` and `t` from `@package/contract` to validate `process.env` (e.g. `JWT_SECRET`)
...
- [x] 5.1 Refactor `package/ui/src/shadcn/section/data-table.tsx` to replace `zod` schema with `t` from `@package/contract` (migrating `z.infer` to typebox `Static`)
