...
The homepage SHALL include four content preview sections below the library cards: a featured books carousel, trending shelves, recent reviews, and active realms. Each section SHALL display a compact preview of 3 to 5 items.

#### Scenario: Featured books carousel displays items
- **WHEN** the homepage is loaded
- **THEN** a "Featured from Book Library" section SHALL display a carousel of 3 to 5 book items
...
Each content preview section on the homepage SHALL include a "See all" link that navigates to the dedicated browse page for that content type. Featured books SHALL link to `/book`, trending shelves to `/shelf`, recent reviews to `/review`, and active realms to `/realm`.
...
### Requirement: Hero section with unified search bar

The homepage SHALL display a hero section at the top containing the platform brand identity and a unified search bar. The search bar SHALL allow users to search across all content types (books, shelves, reviews, realms).

#### Scenario: Hero section is visible on homepage
- **WHEN** a user navigates to the homepage `/`
- **THEN** the hero section with brand identity and a search bar SHALL be displayed at the top of the page
...
Book-specific editorial sections (rankings, editor picks, author spotlight, tag explore, wiki) SHALL NOT appear on the homepage. These sections SHALL be located on the Book Library landing page at `/book`.

#### Scenario: Homepage does not contain book-specific editorial sections
- **WHEN** a user views the homepage `/`
- **THEN** the page SHALL NOT display rankings, editor picks, author spotlight, tag explore, or wiki sections

#### Scenario: Book Library landing page contains book-specific sections
- **WHEN** a user navigates to `/book`
- **THEN** the page SHALL display book-specific editorial content such as rankings, editor picks, author spotlight, tag explore, and wiki sections
