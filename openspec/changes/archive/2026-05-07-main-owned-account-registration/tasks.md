...
- [x] 1.5 Update `package/contract/src/auth/*` to remove auth profile/organization surfaces and add pending-registration, verification error, and setup-state schemas.
...
- [x] 2.5 Implement auth internal stale-cleanup/delete support for temporary unverified registration accounts.
...
- [x] 2.9 Add focused tests in `package/auth` for pending account re-entry, stale cleanup, OTP resend, and mailer failure paths.
...
- [x] 3.6 Use normal sign-out for non-destructive registration pause instead of exposing a destructive registration endpoint.
...
- [x] 3.10 Add targeted server tests for setup success, unverified rejection, slug conflict, duplicate setup, sign-out pause behavior, refresh without main user, and cookie-backed member API access.

## 4. Frontend API and State

- [x] 4.1 Update `package/api/src/auth/auth.api.ts`, keys, queries, and mutations for pending-registration state, verification resend errors, and main account setup.
...
- [x] 4.5 Add API/state tests for pending re-entry, refresh rejection, and member-ready transition.
...
- [x] 5.5 Add a pause-registration affordance with confirmation, loading, error, sign-out, and anonymous-state cleanup.
...
- [x] 5.9 Update localization strings and accessibility labels for verification lock, resend failure, pause registration, setup slug conflict, and provider email messaging.
...
- [x] 8.4 Run targeted `bun test` suites for `package/app` registration routing, complete-registration page state, OAuth redirect helpers, and pause registration.
- [x] 8.5 Run formatting/typecheck/build commands required by affected packages.
- [ ] 8.6 Manually verify browser flows: email registration, resend OTP, invalid OTP, pause registration, login re-entry before verification, verified setup with slug conflict, OAuth trusted email setup, sign-out, and session refresh after reload.
