...
**Reason**: Replaced by runtime i18next + JSON file loading. The repository
no longer compiles messages into typed ESM functions.

**Migration**: All `m.<key>()` call sites are rewritten to
`t('<ns>:<key>')`. The `@inlang/paraglide-js` dependency and
`project.inlang/` configuration are removed.

### Requirement: React UI copy resolves through useMessage

**Reason**: Replaced by `useTranslation` from `react-i18next`. The
`useMessage(messageBag)` hook is removed.

**Migration**: Components migrate from `const m = useMessage({ common_save })`
to `const { t } = useTranslation('common')` and call sites change from
`m.common_save()` to `t('common:save')`.

### Requirement: React message imports are explicit

**Reason**: There are no longer generated message functions to import. The
tree-shaking concern is replaced by namespace-level code splitting via
`i18next-http-backend`.

**Migration**: Named imports of generated message functions are removed.
Components import `useTranslation` from `react-i18next` and reference keys
by string.

### Requirement: React-i18next is removed from frontend runtime usage

**Reason**: This change reintroduces `react-i18next` as the runtime. The
prior prohibition was specific to the Paraglide-based design and is
reversed.

**Migration**: Frontend packages add `react-i18next` and `i18next` as
runtime dependencies via re-exports from `@rezics/i18n`.

### Requirement: Generated files are not edited manually

**Reason**: There are no longer generated files in the i18n stack.

**Migration**: Translation edits happen directly in
`public/locales/{lng}/{ns}.json`.

### Requirement: Dynamic Paraglide message lookup is forbidden

**Reason**: Superseded by the new `Dynamic translation keys are typed maps`
requirement above, which expresses the same intent for i18next's `t()`
API.

**Migration**: None. Existing typed slug-to-function maps become typed
slug-to-key-string maps.

### Requirement: Paraglide output is generated before TypeScript checks

**Reason**: There is no longer a Paraglide compile step. TypeScript runs
directly against source.

**Migration**: `postinstall` and `dev`/`build` scripts drop the
`i18n:compile` invocation.

### Requirement: Legacy frontend translation API is removed

**Reason**: The "legacy translation API" being removed by the prior
requirement is now the desired runtime API. The prohibition is reversed.

**Migration**: Components use `useTranslation` and `t()` as the
canonical translation API.

### Requirement: Fallback string parameters are forbidden

**Reason**: Replaced by a clearer requirement in the new
`Canonical locale behavior is preserved` requirement above (no
fallback strings, achieved via complete catalogs and `check:i18n`).

**Migration**: None. The prohibition is preserved; the wording is
updated.

### Requirement: UI message catalogs have complete locale parity

**Reason**: Replaced by the more general per-locale gap detection in
`check:i18n`, which covers every namespace not only `package/i18n/messages`
and `package/ui/messages`.

**Migration**: `check:i18n` enforces parity across all namespace JSON
...
**Reason**: Covered by `Canonical locale behavior is preserved` above.

**Migration**: None. The prohibition carries over.

### Requirement: Locale and message checks run with repository conventions

**Reason**: Superseded by `check:i18n validates keys statically` above,
which describes the new validation tool covering the same concerns.

**Migration**: None.

### Requirement: Slug-to-function maps are typed

**Reason**: Reformulated as `Dynamic translation keys are typed maps`
above, with namespace-qualified key strings replacing message functions.

**Migration**: Existing slug-to-function maps become slug-to-key-string
maps with the same exhaustiveness constraint.
