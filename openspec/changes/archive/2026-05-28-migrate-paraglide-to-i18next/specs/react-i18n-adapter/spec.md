...
**Reason**: Replaced by `Translation binding via useTranslation` above.
With JSON-loaded namespaces, tree shaking happens at the namespace
level (via HTTP-backend lazy loading), not at the message-function
level.

**Migration**: Components switch from
`const m = useMessage({ common_save })` and `m.common_save()` to
`const { t } = useTranslation('common')` and `t('common:save')`.

### Requirement: Runtime registration synchronizes Paraglide runtimes

**Reason**: Replaced by `UI locale bundles register via addResourceBundle`
above. There is no longer a separate Paraglide runtime per package; the
shared i18next instance receives resources via `addResourceBundle`.

**Migration**: App/admin bootstrap calls
`registerUiLocale(i18n, locale)` instead of registering a Paraglide
runtime. The UI package no longer exposes a `setLocale` function.

### Requirement: React UI callsites use the m-dot form

**Reason**: Replaced by `Translation binding via useTranslation` above.
The `m.<key>()` callsite shape is replaced by `t('<ns>:<key>')`.

**Migration**: Codemod rewrites every `m.<key>()` callsite to its
namespace-qualified `t()` equivalent according to the namespace map.
