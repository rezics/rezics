...
**Reason**: This pattern conflated editorial intent (an admin's choice to make a tag prominent) with community signal (the score), making both signals unreadable. The replacement is the orthogonal `pinned` + `position` axis defined in this change. Editorial promotion now sets `pinned = true` with a `position` that places the row at the desired location among pinned rows; the score remains a pure community signal computed solely from TagVote rows.

**Migration**: For each UnitTag row whose existing `score ≥ 1000` (the legacy "official boost" sentinel), the migration step SHALL: (a) set `pinned = true` and assign a `position` value preserving the prior relative ordering of these legacy-boosted rows by descending score; (b) reset `score` to `Σ TagVote.value` for that pair (which may be `0` if no community votes exist); (c) reset `voteCount` to `count(TagVote rows)` for that pair. After migration, prominence is preserved (the row remains visually first in the pinned section) and the score reflects only genuine community signal. Consumers that previously inferred "official" status from `score ≥ 1000` MUST switch to checking `pinned`.
