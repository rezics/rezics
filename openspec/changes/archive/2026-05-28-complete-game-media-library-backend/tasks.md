...
- [x] 7.1 Add admin-facing read/list endpoints or service methods for platform Entities, external rating tags, and system requirement rows.
- [x] 7.2 Add source evidence handling so imported requirements can reference `UnitExternalRef` records.
- [x] 7.3 Add drift diagnostics for legacy platform/rating rows, missing platform Entities, missing rating tags, and search projection mismatch.
