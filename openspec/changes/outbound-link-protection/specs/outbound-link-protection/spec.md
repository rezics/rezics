...
The `@rezics/ui` package SHALL export a `<Link>` component that accepts an `href` string plus standard anchor props (`children`, `className`, `aria-label`, `title`). It SHALL classify the href via `classifyUrl` and render:
...
- **WHEN** `<Link href="/book/123">view</Link>` mounts
...
- **WHEN** the user left-clicks `<Link href="https://example.com">go</Link>`
...
- **WHEN** `<Link href="javascript:alert(1)">click</Link>` mounts
...
Each app shell (`package/app`, `package/admin`) SHALL mount exactly one `<ExternalLinkModal>` instance near the root. The modal SHALL be opened by the `<Link>` primitive via a shared store (Jotai atom in `@rezics/ui` or `@rezics/app-shell`), and SHALL display:
...
### Requirement: Markdown and rich-text renderers route through `<Link>`
Every renderer in the codebase that converts markdown `[text](url)` or rich-text anchor nodes to JSX SHALL emit `<Link href={url}>{text}</Link>` rather than a raw `<a>`. This applies to (at minimum):
...
- **THEN** the resulting DOM contains a `<Link>` instance whose classified kind is `external`, and clicking it opens the modal
...
- **WHEN** the user middle-clicks an external `<Link>`
...
- **WHEN** the user right-clicks an external `<Link>` and selects "Copy Link"
...
- **WHEN** a rezics `<Link>` is rendered
