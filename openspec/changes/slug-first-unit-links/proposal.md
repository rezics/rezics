...
- Update `public-short-routes/spec.md` with new requirements codifying both sides of the rule: link builders render short-prefix slug URLs when a slug is known, and API responses that already return slug-bearing unit references should include `slug` whenever the backend can reasonably obtain it.
- Audit backend DTO mappers for embedded slug-bearing references (for example post/review authors, book/realm/shelf owners, tag/entity references, and system shelf owner context). When a mapper already has or can cheaply include the referenced `Unit.slug`, return it alongside `unitId`; when the slug is absent or not reasonably available, keep returning the unitId-only shape and let `unitHref` fall back.
- **Out of scope**: any client-side slug↔unitId cache, reverse lookup from a bare unitId without an embedded slug, chained slug resolution, server-wide DTO-walking middleware, or expensive per-row slug hydration. Those land in a follow-on change `slug-client-resolver`. This change consumes slugs that are present in the DTOs available at the link-build site and asks backend mappers to include known slugs opportunistically and systematically.
...
- `public-short-routes`: adds the link-builder rule (short-prefix slug URL when slug is known; long-prefix unitId URL otherwise; long-prefix URLs never rendered when slug is known) and the API-response rule (slug-bearing unit references should include known slugs when reasonably available). Existing route-resolution requirements are unchanged.
...
- **Affected packages**:
  ...
  - `package/contract` — additive/optional schema clarification only. Existing routing-side schemas remain unchanged; DTO/reference schemas that already expose `slug` keep it optional/nullish rather than making slug mandatory.
  - `package/server` — mapper/include audit for slug-bearing embedded references. The backend should return known `Unit.slug` values next to `unitId` when this can be done from the current query shape or a bounded batch lookup.
...
- **Migration risk**: moderate-low. Link callsite migration is local and mechanical; mapper hydration touches more backend surfaces but remains additive because `slug` stays optional/nullish and clients already fall back to `unitId`.
