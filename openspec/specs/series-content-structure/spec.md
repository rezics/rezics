# series-content-structure Specification

## Purpose

Defines Series member content as direct, release-first content structure. The generic content structure is the source of truth for a Series' direct content, display hierarchy, ordering, grouping, labels, and history-bearing edits. Counted member nodes reference visible release Units; nested Series references are allowed only as non-transitive structural/cross-reference nodes that do not contribute inherited membership or projection.
...
A nested Series reference within a Series content structure SHALL be treated as
a non-counted structural reference. A Series content structure MAY contain
another Series Unit as a structural or cross-reference node where the contract
allows that node kind. That nested Series reference SHALL NOT be counted as
