...
The realm service SHALL trigger an incremental sync when a realm is created or updated. Translation updates on realm units SHALL also trigger a re-sync. Sync SHALL be fire-and-forget.

#### Scenario: Realm creation triggers sync
- **WHEN** a new realm is created via `realm.service.create()`
- **THEN** `syncRealmToMeili(unitId)` SHALL be called

#### Scenario: Realm update triggers sync
- **WHEN** a realm is updated (metadata or translations)
- **THEN** `syncRealmToMeili(unitId)` SHALL be called

#### Scenario: Realm translation update triggers sync
