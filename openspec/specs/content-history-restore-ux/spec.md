# content-history-restore-ux Specification

## Purpose

Defines response tolerance and UI failure handling for history-backed restore editing on the Book edit page. The history service SHALL normalize optional `restoreSource` metadata so legacy or malformed values do not break response validation, and the Book edit page SHALL surface restore revision read failures rather than silently treating restore mode as ready. Ordinary canonical editing SHALL remain independent of history service availability; only workflows that explicitly depend on a requested historical revision are blocked by history read failures.
