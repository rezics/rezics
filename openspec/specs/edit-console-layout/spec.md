# edit-console-layout Specification

## Purpose

Defines the reusable edit console layout in `package/app` shared by Unit-backed
and Post-backed edit surfaces. Covers the layout's configurable contract (a
localized return action, primary edit navigation, optional operational
navigation, optional lower context content, and routed main content), the
capability-driven operational navigation that avoids hardcoded `UnitType` or
`PostKind` switches, the optional lower sidebar context slot below the console
divider, and the design-system, localization, and accessibility constraints the
console must follow.
