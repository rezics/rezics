# server-user-cache Specification

## Purpose

Defines how the server resolves user identity and permission
without an in-memory cache. Owns the rule that `User` lookups key
on `userId` (never the legacy `unitId` column), forbids the old
`package/server/src/middleware/user-cache.ts` Map cache, and splits
permission checks into a fast-path read of
`rezics-session-token.permission.role` and a database verification
for admin/root endpoints.
