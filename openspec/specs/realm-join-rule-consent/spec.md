# realm-join-rule-consent Specification

## Purpose

Defines the frontend-only flow for showing a realm's rule (sourced from `Realm.extra.rule`) before a user joins. When a realm has a rule set, `JoinButton` opens a consent modal rendering the rule Post; the join API is called only after the user clicks "Agree and Join". The system does NOT track per-user-per-rule acknowledgments; the `RealmMember` row is the only evidence of consent.
