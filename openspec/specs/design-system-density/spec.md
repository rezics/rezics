# design-system-density Specification

## Purpose

Codifies density as a per-component-type intrinsic property of the rezics design system, decided at design time and baked into each component's vocabulary. Density is **not** a user-facing toggle: there is no toolbar, settings panel, or `<html>` class that flips compact/comfortable/spacious modes. Same-component runtime density variants are explicitly out of scope.

The system surfaces a closed nine-token `--padding-*` vocabulary emitted by the `uno-config.ts` preflight at fixed values (no `calc()`, no `--density-step`), one per density-bearing rezics-authored component type. The vocabulary applies to rezics-authored primitives and composites only; vendored shadcn primitives consume the spacing values shadcn ships with.

Density affects only spacing dimensions; typography is invariant across density tiers. Opt-in components (repeating-row affordances) consume the vocabulary; opt-out components (bespoke compositions like hero sections, reading view, book covers, dialog content) use ad-hoc fixed spacing utilities.
