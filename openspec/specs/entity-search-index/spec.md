# entity-search-index Specification

## Purpose

Defines the Meilisearch `entities` index that backs Entity identity search, including the EntityPicker and the `entities` category of federated search. Establishes the document shape, searchable/filterable/sortable attributes, the boundary between identity facets and related-Unit lookups, the catalog-vs-personal search semantics, and the bounded sync rules between Entity/CreditAttribution/SubjectAttribution mutations and index documents.
