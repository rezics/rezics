...
Each tag SHALL have an English `UnitTranslation` with the canonical title (e.g., "Book"), `isLanguageNeutral: true`, `status: PUBLISHED`, `visibility: PUBLIC`.
...
- **THEN** the system creates 5 tag Units with database-generated v7 IDs, English translations, and self-tag score boost
...
- **THEN** the system creates the default realm with the root user as owner
