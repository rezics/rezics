# main-email-verification-contracts Specification

## Purpose

Defines main-owned verification records for product email fields such as `user.email` and future `org.email`. Auth owns login email verification; main owns product email verification. Main writes an email value into a domain table only after the corresponding main email verification contract is verified, and the verification flow uses shared `@rezics/email` sender utilities driven by main's own configuration.
