...
Defines email verification using one-time codes (OTP) for the auth service and the frontend `/verify-email` flow. Covers OTP characteristics, Turnstile gating, branded email templates, resend/cooldown behavior for locked registration, and how OTP verification transitions a pending account into the main-owned account-setup step.
