# unit-ranking Specification

## Purpose

Defines the `@rezics/ranking` service that owns Unit-based rank projections
keyed by `unitId`, scope, and rank kind. It turns reactions, comments, scores,
progress, and future view signals into sortable hot/top/trending/quality fields
on the Meilisearch `content` and `posts` indexes, consuming CDC invalidations
through the queue and reading current canonical state rather than treating
webhook payloads as authoritative rank deltas.
