# entity-attribution-batch-editing Specification

## Purpose

Defines a unit-scoped batch commit surface that lets editor UIs save credit and subject attribution edits for a target Unit as one semantic commit. Multi-edit attribution editors (e.g., book metadata authors, character/studio selectors) accumulate local add/remove/reorder gestures, coalesce them into final per-role sets, and submit them through `PATCH /unit/:unitId/entity-attributions/batch`. The server reconciles all requested rows in one transaction, writes at most one editorial history revision when the batch produces an effective change, and rejects the whole batch on validation failure. Existing single link/unlink endpoints remain available for simple immediate actions.
