# history-reference-resolution Specification

## Purpose

Defines batch resolution of `actorUserId` values and Unit ids referenced inside history payloads into display data usable by history timelines, revision detail, and compare surfaces. Provides stable result statuses (`OK`, `DELETED`, `GONE`, `RESTRICTED`) for graceful fallbacks, shared query-cache keys so timeline/detail/compare reuse cached lookups, and the contract that history payloads remain id-only so display names are always resolved at read/render time.
