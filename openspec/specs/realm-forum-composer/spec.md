# realm-forum-composer Specification

## Purpose

Defines the top-level realm-post composer flow: how `ReplyComposer` is extended with realm-post mode (`realmUnitIds` + `tagIds`), how the tag picker hydrates from `Realm.extra.tagTree`, how the resulting submission writes `Post`, `RealmUnit`, and `UnitTag` rows in one transaction, and how the realm detail page exposes a "post in this realm" entry point for members.
