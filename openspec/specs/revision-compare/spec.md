# revision-compare Specification

## Purpose

Defines frontend revision comparison behavior for editorial Unit revisions. Covers selecting base and target revisions, deriving a field-level compare model from canonical payloads, rendering Markdown source diffs with split/unified layouts, CJK-aware inline tokenization via `Intl.Segmenter`, semantic collection diffs (tags, credits, subjects, translations, support languages), and product-safe fallback for unrecognized slots without exposing raw payload content to public viewers.
