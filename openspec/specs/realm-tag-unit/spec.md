...
Removing a RealmUnit record SHALL remove the unit from the realm's content feed. Removal of a RealmUnit SHALL NOT affect any UnitTag records and SHALL NOT affect any RealmTagApplication records that reference the realm and unit; removal of those rows is a separate, deliberate action by an authorized actor.
...
#### Scenario: Removal does not affect existing RealmTagApplication rows

- **GIVEN** RealmUnit `(realm-1, unit-1)` and RealmTagApplication rows like `(realm-1, unit-1, tag-1)` exist
- **WHEN** the RealmUnit record is deleted
- **THEN** RealmTagApplication rows SHALL be retained until separately deleted by an authorized actor
...
The system SHALL support querying units in a realm that have been classified with a specific tag, by joining RealmTagApplication on `realmUnitId` and `tagUnitId`.

#### Scenario: Filter realm units by tag

- GIVEN RealmTagApplication records: `(realm-1, tag-action, unit-1)`, `(realm-1, tag-action, unit-2)`, `(realm-1, tag-comedy, unit-3)`
...
- GIVEN RealmTagApplication records for "realm-1" that do not include "tag-horror"
- WHEN a client queries "realm-1" filtered by "tag-horror"
- THEN the response SHALL return an empty list

### Requirement: Query all tags a realm has applied to a unit

The system SHALL support querying all tags that a specific realm has applied to a specific unit by selecting RealmTagApplication records matching both `realmUnitId` and `unitId`.

#### Scenario: List realm-specific tags for a unit

- GIVEN RealmTagApplication records: `(realm-1, tag-action, unit-1)`, `(realm-1, tag-sci-fi, unit-1)`, `(realm-2, tag-drama, unit-1)`
...
The system SHALL support querying all realms that have applied at least one tag to a given unit by selecting distinct `realmUnitId` values from RealmTagApplication records matching `unitId`.

#### Scenario: List realms that tagged a unit

- GIVEN RealmTagApplication records: `(realm-1, tag-action, unit-1)`, `(realm-2, tag-drama, unit-1)`, `(realm-3, tag-comedy, unit-2)`
...
- GIVEN no RealmTagApplication records for "unit-orphan"
...
- AND "realm-female-traits" applies "long-hair" to "unit-1" via RealmTagApplication
- AND "realm-male-traits" applies "long-hair" to "unit-1" via RealmTagApplication
- WHEN querying RealmTagApplication for "unit-1"
...
- GIVEN "realm-critics" applies tag "slow-burn" to "unit-1" via RealmTagApplication
- AND no `RealmUnit(realm-critics, unit-1)` row exists
- WHEN querying "realm-critics" content feed
- THEN "unit-1" SHALL NOT appear only because it has a RealmTagApplication row
- AND when filtering by `RealmTagApplication(realm-critics, slow-burn, *)`
- THEN "unit-1" SHALL be eligible for the realm-tag filtered result

### Requirement: RealmTagApplication has score, voteCount, pinned, and position fields

RealmTagApplication SHALL contain the following additional fields beyond its `(realmUnitId, tagUnitId, unitId)` composite primary key: `score: integer` (default `0`), `voteCount: integer` (default `0`), `pinned: boolean` (default `false`), `position: string?` (nullable), and timestamp fields `createdAt`, `updatedAt`. The `score` and `voteCount` SHALL be derived from `RealmTagApplicationVote` rows as defined by the `realm-tag-vote` capability. The `pinned` and `position` fields define editorial ordering analogous to UnitTag.

#### Scenario: New RealmTagApplication row has expected default fields

- **WHEN** a RealmTagApplication row is newly created via the standard creation flow
- **THEN** the row SHALL have `score = 1`, `voteCount = 1`, `pinned = false`, `position = null`, and timestamps populated

#### Scenario: RealmTagApplicationDTO contains the new fields

- **GIVEN** the `RealmTagApplicationDTO` type
...
### Requirement: RealmTagApplication display ordering pins-first then by score descending

When retrieving the RealmTagApplication list for a `(realmUnitId, unitId)` pair, the system SHALL emit rows in two sections in order: first all rows where `pinned = true` sorted by `position` ascending (lexicographic string compare), then all rows where `pinned = false` sorted by `score` descending. Ties within either section SHALL maintain a stable order. This ordering rule applies to both realm-context endpoints and any list endpoint that returns realm-scoped tag rows.

#### Scenario: Pinned realm tags lead the realm-context list

- **GIVEN** RealmTagApplication rows for `(realm-1, unit-1)`: `tag-A (pinned=true, position="G")`, `tag-B (pinned=true, position="M")`, `tag-C (pinned=false, score=5)`, `tag-D (pinned=false, score=200)`
- **WHEN** a client requests realm-1's tags for unit-1
- **THEN** the order SHALL be: `tag-A`, `tag-B`, `tag-D`, `tag-C`

### Requirement: Pin and delete authority for RealmTagApplication is restricted to admin or realm owner

Setting `pinned`, mutating `position`, unpinning, and deleting a RealmTagApplication row SHALL be restricted to:
- platform administrators, OR
- the user identified as the realm's owner.

Realm moderators SHALL NOT hold pin or delete authority on RealmTagApplication in this iteration.

#### Scenario: Realm owner pins a realm tag

- **GIVEN** "user-owner" is the owner of "realm-1"
- **AND** RealmTagApplication `(realm-1, unit-1, tag-1)` exists with `pinned = false`
- **WHEN** "user-owner" requests to pin the row at position `"M"`
- **THEN** the row SHALL be updated to `pinned = true, position = "M"`

#### Scenario: Platform admin deletes a RealmTagApplication

- **GIVEN** RealmTagApplication `(realm-1, unit-1, tag-1)` exists
- **WHEN** a platform administrator sends a delete request
- **THEN** the row SHALL be removed
- **AND** all RealmTagApplicationVote rows for `(realm-1, unit-1, tag-1)` SHALL also be removed

#### Scenario: Realm moderator is denied pin authority

- **GIVEN** "user-mod" is a moderator (but not the owner) of "realm-1"
- **WHEN** "user-mod" attempts to pin or delete a RealmTagApplication row in "realm-1"
- **THEN** the system SHALL deny the operation with an authorization error

#### Scenario: Regular member is denied pin authority

- **GIVEN** "user-1" is a regular member of "realm-1"
- **WHEN** "user-1" attempts to pin or delete a RealmTagApplication row in "realm-1"
- **THEN** the system SHALL deny the operation with an authorization error

### Requirement: RealmTagApplication deletion is unconditional for authorized actors

Deletion of a RealmTagApplication row by an authorized actor (platform admin or realm owner) SHALL succeed regardless of the row's current `score`. There SHALL be no minimum-score gate or community-consensus precondition for deletion.

#### Scenario: Delete a high-score realm tag row

- **GIVEN** RealmTagApplication `(realm-1, unit-1, tag-1)` with `score = 50`
- **WHEN** an authorized actor sends a delete request
- **THEN** the row SHALL be removed

#### Scenario: Delete a low-score realm tag row

- **GIVEN** RealmTagApplication `(realm-1, unit-1, tag-1)` with `score = -3`
- **WHEN** an authorized actor sends a delete request
- **THEN** the row SHALL be removed

### Requirement: RealmTagApplication rows with score at or below -100 are hidden from regular users

When a list/search endpoint returns RealmTagApplication rows for a regular caller (any user who is not a platform administrator and not the realm's owner), rows whose `score ≤ -100` SHALL be excluded. When the same endpoint is called by a platform administrator or by the realm's owner, those rows SHALL be included with a flag (e.g. `belowVisibilityThreshold: true`) marking their suppressed status.

#### Scenario: Regular member does not see suppressed realm rows

- **GIVEN** RealmTagApplication `(realm-1, unit-1, tag-1)` has `score = -120`
...
- **GIVEN** "user-owner" is the owner of "realm-1"
- **AND** RealmTagApplication `(realm-1, unit-1, tag-1)` has `score = -120`
...
### Requirement: Admin discovery endpoint for low-score RealmTagApplication rows

The system SHALL expose an admin-only endpoint that lists RealmTagApplication rows with `score` at or below a configurable threshold (default `-100`), to support periodic moderation sweeps. The endpoint SHALL accept `threshold` and optional `realmUnitId` query parameters and SHALL return rows ordered by `score` ascending.

#### Scenario: Admin lists low-score realm rows

- **GIVEN** several RealmTagApplication rows have `score ≤ -100`
...
- **GIVEN** RealmTagApplication `(realm-1, unit-1, tag-1)` and UnitTag `(unit-1, tag-1)` both exist
- **AND** UnitTag `(unit-1, tag-1)` has `score = 7`
- **WHEN** an authorized actor deletes RealmTagApplication `(realm-1, unit-1, tag-1)`
- **THEN** UnitTag `(unit-1, tag-1)` SHALL still exist with `score = 7`

#### Scenario: Deleting a UnitTag does not affect RealmTagApplication

- **GIVEN** UnitTag `(unit-1, tag-1)` and RealmTagApplication `(realm-1, unit-1, tag-1)` both exist
- **AND** RealmTagApplication `(realm-1, unit-1, tag-1)` has `score = 4`
- **WHEN** an authorized actor deletes UnitTag `(unit-1, tag-1)`
- **THEN** RealmTagApplication `(realm-1, unit-1, tag-1)` SHALL still exist with `score = 4`
...
- **WHEN** "user-1" creates `RealmTagApplication(realm-1, tag-1, unit-1)` through the standard endpoint
...
### Requirement: RealmTagApplication and extra.tagTree have independent purposes

`RealmTagApplication` and `Realm.extra.tagTree` SHALL serve distinct purposes that do not constrain each other:

- `RealmTagApplication(realmUnitId, unitId, tagUnitId)` SHALL track which tags have been applied to which units within a realm, with score, voteCount, pinned, and position fields. It is the data layer of realm-scoped tagging.
...
A tag id appearing in `extra.tagTree` SHALL NOT imply any `RealmTagApplication` row exists. A tag id NOT in `extra.tagTree` SHALL NOT be prevented from being applied via `RealmTagApplication`. The two layers SHALL evolve independently.

#### Scenario: tagTree advertises a tag with no RealmTagApplication rows yet

- **GIVEN** realm-1 with `extra.tagTree = [{ tagId: "action" }]`
- **AND** no `RealmTagApplication` rows exist for `(realm-1, *, action)`
- **WHEN** a member opens the composer in realm-post mode for realm-1
- **THEN** the picker SHALL show "action" as a quick-pick chip
- **AND** the absence of RealmTagApplication rows SHALL NOT prevent rendering or selection

#### Scenario: RealmTagApplication exists for a tag not in tagTree

- **GIVEN** realm-1 with `extra.tagTree = []`
- **AND** `RealmTagApplication(realm-1, post-A, "fantasy")` exists from a previous tagging
...
- **AND** the existing RealmTagApplication row SHALL remain unaffected
...
### Requirement: RealmTagApplication is independent from RealmUnit

`RealmTagApplication(realmUnitId, tagUnitId, unitId)` SHALL record that a realm applied an existing global tag to a target Unit. It SHALL NOT require `RealmUnit(realmUnitId, unitId)` to exist, SHALL NOT create `RealmUnit` as a side effect, and SHALL NOT be deleted merely because a related `RealmUnit` row is deleted.
...
- **WHEN** a current member creates `RealmTagApplication(realm-1, tag-1, unit-1)`
...
- **WHEN** a developer inspects the create or query service for `RealmTagApplication`
