...
Defines the federated, scope-aware search experience that collapses the platform's previously fragmented per-index flows (content, posts, realms, users) into a single endpoint and a unified frontend surface. Owns the `SearchScope` and `SearchCategory` contracts in `@rezics/contract`, the route-to-scope resolver consumed by header submit and scoped pages, the `POST /meili/search/federated` endpoint with its three response variants (grouped / ranked / single), the strict-membership scope-to-filter mapping across indexes, configuration of federation weights, the default filters that every sub-query inherits, the `useFederatedSearch` TanStack Query hook in `@rezics/api`, and the `buildSearchPath` helper that constructs scope- and category-aware URLs.
