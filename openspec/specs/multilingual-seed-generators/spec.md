...
Titles and summaries for each language SHALL be drawn from curated text pools stored in `package/shared/src/text/corpus/<lang>.ts` and accessed via `getTitlePool` / `getSummaryPool` / `getDescriptionPool` from `@rezics/shared/text`. Each pool SHALL contain realistic text in the target language's script (Chinese characters for zh-hant/zh-hans, Japanese for ja, German for de, English for en). The generator SHALL pick randomly from the pool.
