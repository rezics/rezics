...
The post service SHALL trigger an incremental sync to Meilisearch when a post is created, updated, or deleted. The sync SHALL be fire-and-forget (non-blocking, errors caught silently).

#### Scenario: Post creation triggers sync
- **WHEN** a new post is created via `post.service.create()`
- **THEN** `syncPostToMeili(unitId)` SHALL be called after the database write

#### Scenario: Post update triggers sync
- **WHEN** a post is updated via `post.service.update()`
- **THEN** `syncPostToMeili(unitId)` SHALL be called after the database write

#### Scenario: Post deletion triggers sync (removal)
