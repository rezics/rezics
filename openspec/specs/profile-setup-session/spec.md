# profile-setup-session Specification

## Purpose

Defines the separate profile setup token/cookie, its route guard, and its relationship to normal member sessions. After registration verification succeeds, main materializes a minimal user and issues `rezics-profile-setup-token` instead of the normal `rezics-session-token`. Profile setup endpoints accept only the setup token; normal member endpoints continue to require the member session token. This avoids broad capability checks on every product handler.
