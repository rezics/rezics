# editor-entry-policy Specification

## Purpose

Defines how the app decides whether a viewer can see or enter an editor surface,
keeping that editor-entry decision separate from concrete field- or
operation-level edit permission. Covers collaborative work surfaces (Book,
future Game/Media work surfaces, Wiki Post) that expose editor entry by
capability for authenticated non-blocked viewers, and ordinary author-owned
surfaces (Review, Remark, Excerpt, reply Post) that remain owner/admin
controlled, while leaving field-level authorization to forms, mutations, server
permission helpers, collaborative authority gates, and field-lock checks.
