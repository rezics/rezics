...
Defines the Account/Profile settings surface that owns the Rezics product email (`server.User.email`), its verification through main email verification contracts, and account deletion. Login email and password belong to Security settings; this section never calls auth login email change APIs.
