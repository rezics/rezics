...
- **THEN** `authSessionStore.capabilityLevel` is `"anonymous"` regardless of `auth-session-token` presence

#### Scenario: Needs verification derived from auth-session-token claims

- **WHEN** the `auth-session-token` JWT contains `email_verified: false`
