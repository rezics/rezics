/**
 * Client-side fractional-index helper for position strings.
 *
 * Mirrors the server's shelf fractional-index utility
 * (`packages/server/src/shelf/fractional-index.ts`) so client and server
 ...
 *
 * Dual-implementation invariant: this file and the server counterpart MUST
 * stay in sync on alphabet, midpoint algorithm, and edge-case behavior.
 * Cross-package import is intentionally avoided so client builds stay free
 * of server dependencies. When updating one side, mirror the change to the
 * other and adjust both test suites in the same change.
 *
 * Consumed by:
 *  - `@rezics/api/tag` for pinned-tag positions
 *  - `@rezics/api/shelf` (and the shelf editor) for item positions
 */
