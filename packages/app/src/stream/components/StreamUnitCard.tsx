import type { StreamUnitRow } from "@rezics/contract/api/stream/stream.types";
import {
...
function unitTypeLabel(type: string, t: (key: string) => string) {
  switch (type) {
    case "BOOK":
    ...
    case "REALM":
      return t("page:home_stream_type_realm");
    case "ZONE":
      return t("page:home_stream_type_zone");
    default:
      return type;
  }
}

function shelfKindForUnit(type: string): ReactionBarPolicy["shelfItemKind"] {
  switch (type) {
    case "BOOK":
      return "book";
    case "GAME":
...
function unitCoverUrl(row: StreamUnitRow): string | null {
  const extra = row.unit.extra;
...
/**
 * Stream unit card：统一 Unit 推荐项。BOOK/GAME/MEDIA 使用 catalog cover
 * contract 固定封面比例，封面框 `self-start` 避免被右侧内容高度拉伸，
 * 图片 `object-fit: contain` 保持原比例；其他 Unit 使用图标/图片框。
 * 正文列 `min-w-0 flex-1` 吸收宽度变化；窄屏截断标题，宽屏由 stream
 * 容器决定最终宽度。
 *
 * Mobile (<640px)
 * +------------------------------+
 ...
 * Ultra-wide (>=1536px)
 * +----------------------------------------------------------+
 * | [cover] Card remains full-width within stream constraints   |
 * +----------------------------------------------------------+
 */
export function StreamUnitCard({ row, className }: StreamUnitCardProps) {
