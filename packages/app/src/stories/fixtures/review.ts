...
// MOCK：Storybook 评论夹具，手工依据 `PostDTO` 编写（评论是 kind=REVIEW 的帖子）。
import { markdownContentDoc, type PostDTO } from "@rezics/contract";
import { userAlice, userBen } from "./user.ts";

type ReviewFixtureOverrides = Partial<PostDTO> & {
  unitId: string;
  contentSource?: string;
};

function makeReview(overrides: ReviewFixtureOverrides): PostDTO {
  ...
  return {
    ...
    content: markdownContentDoc(
      contentSource ??
        "An honest, generous read — flawed in places, but unforgettable.",
    ),
    extra: {
...
export const reviewCJK: PostDTO = makeReview({
  ...
  contentSource:
    "讀第二次才發現結局並非高潮——第十七章才是，作者刻意不提示，這正是它最動人的地方。",
  extra: {
    title: "重讀的喜悅",
    rating: 5,
    book: {
...
export const reviewLatin: PostDTO = makeReview({
  unitId: "review-latin",
  contentSource:
    "What surprised me on the second reading was how the small chapters mirror each other across hundreds of pages. The novel rewards re-reading; once you see the pattern, it never goes away.",
});
