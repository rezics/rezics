...
describe("MarkdownEditor composition", () => {
  it("markdownFull produces plugins with toolbar items", async () => {
    const plugins = await markdownFull();
  ...
  it("applies default icons to all markdown toolbar items", async () => {
    const plugins = await markdownFull();
    const resolved = resolvePlugins(plugins);
  ...
  it("toolbar override icons replace the correct item", async () => {
    const plugins = await markdownFull();
    const resolved = resolvePlugins(plugins);
    const withIcons = applyIconDefaults(resolved.toolbar, markdownIconMap);
  ...
  it("extra plugins are appended to defaults", async () => {
    const mdPlugins = await markdownFull();
