...
export async function createNotifyApp(options: CreateNotifyAppOptions = {}) {
  const port = options.port ?? (env.PORT ? Number(env.PORT) : 3002);
...
export type NotifyApp = Awaited<ReturnType<typeof createNotifyApp>>["app"];
