...
describe("applyUserShelfItemMetadata", () => {
  test("uses patch semantics for user tags", async () => {
    ...
    const { applyUserShelfItemMetadata } = await import(
      "./user-shelf-item.service"
    );
    await applyUserShelfItemMetadata(tx, "user-1", "unit-1", {
      searchText: null,
