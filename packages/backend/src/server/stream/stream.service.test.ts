...
describe("StreamService", () => {
  ...
  test("schedules home recommendation rows from bounded work and shelf rows", async () => {
    ...
    expect(result.rows[4]).toMatchObject({
      ...
      book: {
        ...
        subtitle: "First subtitle",
        creditAttributions: [
          { entityId: "author-1", name: "Author One", role: "author" },
        ],
      },
    });
    expect(result.nextCursor).toEqual({
  ...
  test("maps home recommendation books through the book dto projection", async () => {
    ...
    expect(result.rows[4]).toMatchObject({
      ...
      book: {
        ...
        summary: "內部摘要",
        creditAttributions: [
          { entityId: "author-1", name: "內部作者", role: "author" },
        ],
      },
    });
  });
  ...
  test("passes stream cursors through to the post source", async () => {
    const { StreamService } = await import("./stream.service");
    const service = new StreamService();
  ...
  test("builds realm content rows from realm post list", async () => {
    ...
    expect(bookListMock).not.toHaveBeenCalled();
  });
  ...
  test("builds zone rows from zone filter-backed post list", async () => {
    ...
    expect(result.rows[0]).toMatchObject({
      type: "post",
      rowId: "post:post-1",
      recommendationReason: "zone-stream-activity",
    });
    expect(byRealmMock).not.toHaveBeenCalled();
  });

  test("rejects zone stream without a zone Unit id", async () => {
    const { StreamService } = await import("./stream.service");
    const service = new StreamService();

    await expect(service.list({ scope: "zone" })).rejects.toMatchObject({
      statusCode: 400,
      message: "zoneUnitId is required for zone stream",
    });
  });

  test("rejects missing zone stream targets", async () => {
    zoneResult = null;
    const { StreamService } = await import("./stream.service");
    const service = new StreamService();
  ...
  test("uses targeted book library streams for book community surfaces", async () => {
    const { StreamService } = await import("./stream.service");
    const service = new StreamService();
  ...
  test("rejects unsupported library stream kinds", async () => {
    const { StreamService } = await import("./stream.service");
    const service = new StreamService();

    await expect(
      service.list({ scope: "library", libraryKind: "game" }),
    ).rejects.toThrow("Only book library streams are supported");
  });
