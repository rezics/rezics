...
export const RealmTagApplication = pgTable(
  "RealmTagApplication",
  {
     ...
     * Realm 集体对 (目标 Unit, TAG Unit) 配对的投票态度。
     */
    realmUnitId: uuid()
      .notNull()
...
/**
 * Member vote on a RealmTagApplication.
 * 成员对 RealmTagApplication 的投票。
 */
...
export const UserTagApplication = pgTable(
  "UserTagApplication",
  {
    ...
    /**
     * User tags apply to the resolved interaction target Unit.
     * 用户标签应用于已解析的交互目标 Unit。
     */
    unitId: uuid()
      .notNull()
      .references(() => Unit.id, { onDelete: "cascade", onUpdate: "cascade" }),
