  ...
  Effect.fn(function* (handlers) {
    ...
    const resolveCurrentUserUnitId = () =>
      Effect.gen(function* () {
        ...
        if (!rows[0]) return yield* Effect.die(new Error(`User row missing for auth user ${currentUser.id}`));
    ...
    return handlers
      ...
      .handle("repairRetry", ({ payload }) =>
        Effect.gen(function* () {
          ...
          const updated = {
            ...
            status: "retried",
            updatedAt: now,
            result: { ...((existing.result ?? {}) as object), retriedAt: now.toISOString() },
          };
      ...
      .handle("repairCancel", ({ payload }) =>
        Effect.gen(function* () {
          ...
          const now = new Date();
          const updated = { ...existing, status: "cancelled", updatedAt: now };
          repairJobs.set(updated.id, updated);
          return repairJobToDTO(updated);
        }),
      )
      ...
      .handle("slugResolve", ({ payload }) =>
        Effect.gen(function* () {
          ...
          const conditions: ReturnType<typeof eq>[] = [eq(Unit.slug, payload.slug)];
          if (payload.kind) {
            conditions.push(eq(Unit.type, payload.kind as typeof Unit.type.enumValues[number]));
          }
      ...
      .handle("dispatchResults", ({ payload }) =>
        Effect.gen(function* () {
          return (payload.results as Array<{ id?: string; status?: string; result?: unknown }>).map(
            (r) =>
              new DispatchResult({
        ...
        Effect.gen(function* () {
          ...
          const rows = yield* Effect.orDie(
            db
              ...
              .leftJoin(
                UnitTranslation,
                and(eq(UnitTranslation.unitId, Unit.id), eq(UnitTranslation.language, "en")),
              )
        ...
        Effect.gen(function* () {
          ...
          const existing = yield* Effect.orDie(
            db
              ...
              .where(and(eq(Unit.type, "LABEL"), eq(UnitTranslation.title, payload.name)))
              .limit(1),
          );
          if (existing[0]) return yield* new AdminConflict();
        ...
        Effect.gen(function* () {
          ...
          const rows = yield* Effect.orDie(
            db
              ...
              .where(and(eq(Unit.type, "LINK"), eq(Unit.targetUnitId, params.unitId)))
              .orderBy(desc(Unit.createdAt)),
          );
          ...
          return rows.map(
            (r) =>
              new TokenEntry({
                ...
                name: r.name,
                token: undefined,
                permissions: Object.keys((r.scopes ?? {}) as Record<string, unknown>),
                createdAt: r.createdAt,
      ...
      .handle("tokenCreate", ({ payload }) =>
        Effect.gen(function* () {
          ...
          const hashBuffer = yield* Effect.orDie(
            Effect.promise(() => crypto.subtle.digest("SHA-256", encoder.encode(rawToken))),
          );
      ...
      .handle("tokenUpdate", ({ params, payload }) =>
        Effect.gen(function* () {
          ...
          return new TokenEntry({
            ...
            name: row.name,
            token: undefined,
            permissions: Object.keys((row.scopes ?? {}) as Record<string, unknown>),
            createdAt: row.createdAt,
            expiresAt: row.expiresAt ?? null,
          });
        }),
          ...
          const unitRows = yield* Effect.orDie(
            db
              ...
              .values({
                type: "BOOK",
                slugScope: scopeId,
                status: (payload.status ?? "DRAFT") as typeof Unit.status.enumValues[number],
                visibility: "PUBLIC",
              })
      ...
      .handle("gameSystemRequirementList", ({ query }) =>
        Effect.gen(function* () {
          ...
          const conditions: ReturnType<typeof eq>[] = [];
          if (query.unitId) {
            conditions.push(eq(GameSystemRequirement.gameUnitId, query.unitId));
          }
          const where = conditions.length > 0 ? and(...conditions) : undefined;
