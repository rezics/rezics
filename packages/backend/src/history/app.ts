...
export type CreateHistoryAppOptions = {
  initializeTelemetry?: boolean;
  openApiPath?: string | false;
  port?: number;
};

export async function createHistoryApp(options: CreateHistoryAppOptions = {}) {
  const port = options.port ?? (env.PORT ? Number(env.PORT) : 3004);
...
export type HistoryApp = Awaited<ReturnType<typeof createHistoryApp>>["app"];
