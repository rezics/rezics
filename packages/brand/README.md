...
| App、PWA、桌面快捷方式图标  | `app-icon.svg`、`app-icon.png` 或对应尺寸 PNG |
...
- 不拉伸、不旋转、不改变渐变。
- 所有变体中的 Z 标志本体严格保持原始 `7:5` 比例；方形只指画布，不代表拉伸标志。
...
其中 `app-icon-180.png`、`app-icon-192.png`、`app-icon-512.png` 均由 `app-icon.svg` 生成，供平台按所需尺寸直接使用。
