  ...
  group "backend" {
    ...
    task "migrate-auth" {
      ...
      config {
        ...
        network_mode = "[[ var "network" . ]]"
        entrypoint   = ["/bin/sh", "-c"]
        args         = ["until cd /repo/packages/backend && bunx drizzle-kit migrate --config drizzle.auth.config.ts 2>&1; do echo 'Waiting for auth database...'; sleep 2; done"]
      }
    ...
    task "migrate-notify" {
      ...
      config {
        ...
        network_mode = "[[ var "network" . ]]"
        entrypoint   = ["/bin/sh", "-c"]
        args         = ["until cd /repo/packages/backend && bunx drizzle-kit migrate --config drizzle.notify.config.ts 2>&1; do echo 'Waiting for notify database...'; sleep 2; done"]
      }
    ...
    task "migrate-reaction" {
      ...
      config {
        ...
        network_mode = "[[ var "network" . ]]"
        entrypoint   = ["/bin/sh", "-c"]
        args         = ["until cd /repo/packages/backend && bunx drizzle-kit migrate --config drizzle.reaction.config.ts 2>&1; do echo 'Waiting for reaction database...'; sleep 2; done"]
      }
    ...
    task "migrate-history" {
      ...
      config {
        ...
        network_mode = "[[ var "network" . ]]"
        entrypoint   = ["/bin/sh", "-c"]
        args         = ["until cd /repo/packages/backend && bunx drizzle-kit migrate --config drizzle.history.config.ts 2>&1; do echo 'Waiting for history database...'; sleep 2; done"]
      }
