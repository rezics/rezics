...
The maintainer has authorized complete breaking API/schema replacement. Under the [2026-09-07 baseline](../plan/operational-refactor-20260906/00-source-complete-schema.md#breaking-replacement-baseline), the stopped site's approximately 400k legacy records are a separate offline migration-software input. Do not retain the seven-language storage limit, one-title-per-language model, old routes or payload adapters for compatibility. Legacy conversion is not a prerequisite for the new contract or its acceptance.
...
3. In the separate offline converter, import selected old localization titles as existing editorial names without inventing official/original status; this does not block replacing the language schema. Preserve `zh` when the original script is unknown. Keep existing aliases, votes, history and external-link IDs through explicit mappings. If past normalization erased `cmn` into `zh`, only recover specificity from preserved source evidence; do not infer it from the new parser.
...
6. Normalize API input before hashing, cache keys, idempotency keys, queue serialization and query execution. Version canonicalization/matching and search projections; reject old cursors clearly after a semantic cutover. Regenerate OpenAPI and both generated SDKs through repository workflows, update filter documents and internal clients directly to the new contract. Any legacy saved-search conversion belongs to the separate offline tool.
7. Unify About's locale values with canonical UI tags. Move lowercase/underscore upstream provider formats into source adapters. Emit canonical new locale paths; retaining old v1 URL spellings or redirects is not required for this refactor. New Unit slug semantics remain independent of localized names.
...
9. Validate new language contracts and collision behavior on fresh-target fixtures. The separate offline converter owns legacy collision reports and data reconciliation before its import is accepted. Destructive schema changes remain an explicit, reviewed forward migration or new-deployment cutover within the existing v1+ release history. Do not modify previously released SQL or restore pre-v1 adapters.
