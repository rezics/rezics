...
采用共同的关系身份、定义、修订、参与者、作用域和来源核心。旧接口可以成为按谓词／领域选择的读取或写入适配，而不是继续各自维护含义重叠的事实。
...
3. 实现可引用的关系修订与作用域，迁移业务 Role 定义；保留基础数据约束和必要的兼容映射。
...
每一步都使用新增迁移、可恢复回填、单一权威写入端和版本化契约；不修改已发布迁移，不恢复 pre-v1 兼容层。先形成具体实施 ADR，再更新相关实现状态文档，避免把研究建议写成已实施事实。
