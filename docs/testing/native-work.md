...
Use produced native IDs and multiple principals. Record the external inputs observed in each live run. Exact release/adoption cases record their selected revisions; ordinary chapter cases resolve eligible published heads and retain stable occurrence progress. Check rejected SQL states, competing authoring/adoption and stateful APIs after semantic fixtures establish expected outcomes. External papers and deployments support the [design reasoning](../architecture/database/design-evidence.md), not a passing result for these cases.
