...
GROUPS = {
    ...
    "D02": "access access-identity access-role access-membership access-group access-group-membership access-role-binding auth participation organization-membership account-control",
...
def capacity():
    ...
    lines += ["",
              ...
              "These stored-row and byte counts do not establish a fixed read- or write-cost multiplier. A known definition UUID combined with a subject or target key can support a selective indexed lookup; join count alone does not determine its cost. Definition caching saves repeated resolution, while relation reads depend on selectivity, scanned/returned rows, ordering, pagination and database cache state. Writes depend on changed rows/indexes, constraints, contention, projection maintenance and WAL. Qualify the [relation query access paths](resource-storage.md#relation-query-access-paths) on candidate DDL under representative predicate/degree skew. Search and facet requests use an explicit flattened projection rather than foreground joins across canonical relation tables, so projection query cost is evaluated separately from canonical write amplification and freshness.", "",
              "A compact generic binary edge is a selected physical strategy but has no byte estimate here until candidate DDL and indexes exist. Its native heap-plus-index storage target is no more than 2x the specialized current occurrence for its admitted current-value workload; exceeding that target requires either a narrower layout or promotion to a specialized relation. This target does not specify a latency or throughput ratio.", "",
