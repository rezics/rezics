...
separately requires source-model preservation and queryability before native
mapping. Converter tests target the intended model without preserving old REZICS
...
Every elected source field/object records its pinned source contract, source
...
These are contract scenarios, not executed evidence or new fixtures. Their test
authoring/execution follows the [phase policy](../plan/execution-workflow.md).
Pin original input, model/vocabulary revision, processing options/context bytes,
expected source representation, queries, native disposition and each export.
...
pinned complete artifacts using bounded streaming; deterministic fixtures must
