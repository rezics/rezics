...
| Identity and collaboration | Several private operators share Agents, join Org/Realm groups and use custom roles under scoped representation; ordinary defaults never retarget prepared commands or consents. |
| Connected applications | A third-party local account connects an Agent and an Org installs the same App; user and autonomous REST/MCP access retain separate ceilings, private subjects and live dependencies. |
...
| SYS07 | Change group ancestry, role head and representation while a delegated content/account-management command waits; validate the complete current proof without pooling unrelated actor/Agent powers. |
| SYS08 | Connect an Agent, install an App, approve a narrower resource scope, then revoke consent/installation while a tool call, refresh and webhook are pending; no subsequent effect may use the revoked path. |
...
| SYS10 | Rate a company, model and exact version under standing/daily/per-experience contexts; correct an old observation, submit a new one, switch contexts explicitly, compare latest/history metrics and drill into matching buckets. Revocation, replay, exact/live Post citations and restore preserve context, counts and private accountability. |
| SYS11 | Create an event with company/model/version participants and named-topic bindings; correct a partial occurrence date while querying by Tag/date and viewing a rating timeline. Preserve event/content result identities, cursor generation, actual/planned/recorded time, uncertainty and current disclosure without inferring causation. |

Run [WORK01-WORK18](native-work.md), [COMP01-COMP24](content-composition.md) and [Resource capability cases](foundation.md#resource-capability-contract-acceptance) within these sequences. The generated cross-domain TSV remains a separate reproducible inventory; these authored matrices add qualification obligations without pretending to be executed scenarios.
