...
- Build the remaining mixed identity contracts on the [private subject/scope registry](../../architecture/identity-and-access.md#private-subject-and-scope-values): private AuthPrincipal, many-to-many Entity representation, default/app preferences, typed memberships, Groups, role bindings and assignment ceilings. Integrate the [qualified role definition store](../../architecture/identity-and-access.md#scoped-role-definition-protocol) with current management authority and assignment-impact admission. Remove the unique-Self and account-only grantee assumptions in affected consumers without preserving old wire/schema compatibility.
