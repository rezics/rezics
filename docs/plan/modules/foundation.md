...
- Resolve the external token privacy profile against pinned Better Auth/Bun/Drizzle integration before closing affected protocol design. Qualify actual token/UserInfo/introspection output, OIDC/public-client and MCP compatibility, installation-client mapping and any token-exchange extension; library documentation is not integration acceptance.
