...
This dictionary specifies target ownership, keys, cardinalities, critical fields, write protocols and access paths. It is a logical/relational specification, not executable migration SQL. Existing retained field definitions remain in their source owner until a scoped implementation replaces them. Every current schema module has a disposition in current-schema-map.tsv; repeating every unchanged authentication/provider field here would create an unreliable second schema.
...
| reference_value | PK id; one concrete nullable owner-target FK | CHECK exactly one target; partial UQ for each non-null owner target; immutable mapping; allocate/reuse by unique-conflict retry | PK and selected target UQ; no separate generic target string |
...
| slug_namespace | PK id; concrete scope REF, normalization policy revision | Namespace existence does not grant write authority | scope REF |
...
| --- | --- | --- | --- |
| users, sessions, accounts, verifications, apikeys | Existing authentication PKs; provider identities, token hashes, expiry/revocation | Authentication-only; preserve library-required security fields; no raw credentials in public metadata/history | Account/provider UQ, token hash UQ, expiry cleanup; private erasure |
| auth_entity | PK auth_user_id; UQ self entity; participation revision/state | Private self binding; source import cannot create it | Account and entity exact lookup |
| entity_participation, service_principal | Entity/principal PK, state, recovery epoch | Public attribution and machine principals distinct from login secrets | Principal/entity PK |
| participation_grant, organization_membership, invitations | Principal, acting entity/organization, permission/scope, validity, grantor, state | Explicit accepted delegation; no inferred control from catalog credits; single active membership per declared member role | Entity/member/state; expiry; account erasure |
| entity_recovery_event, participation_grant_event, membership_event | Owner, sequence, immutable reason/evidence/actor receipt | Recovery does not erase previous control history; sensitive evidence is separately suppressible | Owner/sequence |
| unit_ownership, ownership_event | PK target REF; owner principal/entity, epoch; immutable transfers | Exactly one effective ownership selection where the domain requires ownership; catalog provenance is independent | Target PK; owner/target keyset |
| unit_access_grant, invitation, restriction | Resource REF, principal/role, canonical permission, scope, validity, issuer, version/state | @rezics/access vocabulary; restrictions and default grants retain policy meaning; invitations do not grant before acceptance | Target/principal/permission; principal erasure; expiry |
| authority_fence | PK authority-domain key; epoch | Shared lock for dependent mutations; conflicting lock for revocation/transfer; final re-read under current snapshot | PK; no full authority graph scan |
...
| license_claim, license_selection | Target content/asset/occurrence, license contract/version, territorial/time conditions, claimant, evidence | A license description is not a runtime entitlement or proof of authority; selection and claims distinct | Target/scope/current; evidence reverse |
| unit_license_grant / recognition event | Target REF, instrument id/version, immutable declarant/time, write-once offering end, recognition status/history | Multiple instruments coexist; partial UQ open target/instrument including invalidated rows; recognition cannot reopen an ended offering | Target/open; instrument/effective; target/history |
...
| file_asset, file_revision | Logical file identity; exact payload revision, name/media type, origin | Same bytes can be different files/rights; revocable delivery separate |
| asset_use / derivative | Exact owner/content REV and asset REV; purpose/credit/disclosure; derivative transform contract | Thumbnails/transcodes pin inputs; asset use owns context-local caption/credit |
| image_asset, image_object, audio/video representations | Existing media identities and technical image/audio/video metadata, dimensions/duration/tracks | Preserve native identity; reframe only encoded bytes as resources; no semantic reinterpretation by migration |
...
| unit_merge_request/review/operation/reconciliation | Pinned source/target revisions, required independent reviewers, cursor/items | Existing catalog merge restrictions retained until replacement policy adopted; no authorization from prior discussion |
...
| RevokeAuthority | Exclusive affected authority fences, expected grant/ownership version | New effective authority epoch and revocation event | Earlier locked command commits before revoke; later command rechecks |
