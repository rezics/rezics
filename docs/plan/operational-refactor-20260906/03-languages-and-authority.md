...
4. Convert existing localization rows and aliases deterministically; inventory document/localization IDs, history slot keys and content-metric foreign keys. Collision groups preserve all rows and origins instead of last-write-wins. Historical `zh` remains `zh` without evidence.
