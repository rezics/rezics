...
Replace retained code consumers directly; the separate approximately 400k-record
offline converter is not a prerequisite for removing the old model.
...
- Book format uses governed **multi-valued facets** (for example binding and distribution medium), stored as editorial assignments separate from community fit votes. Multiple compatible facets are valid; incompatible choices are rejected per facet definition. Remove opaque `book.format`; its old values belong to the separate offline conversion input, not target compatibility columns.
...
6. Rewrite all retained catalog and non-catalog consumers against the final model and retire the global identity parent, replaced writes and obsolete fields through generated replacement DDL. Prove the fresh target independently of the P11 offline converter. No transferred legacy rows or old-client compatibility are required before removal.
...
P02/P03 consume stable identities and revision references. P04 must use these commands, never ad hoc inserts bypassing invariants. A failed prelaunch target may be rebuilt and reimported offline; after new writes use new-contract recovery. Restoring the old application or reverse-schema compatibility is not required.
