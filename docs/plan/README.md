...
| Phase | `repair`: main TypeScript, 33 focused unit tests, native demo postconditions and aggregate checks pass. Reconcile the obsolete governance projection fixture and rerun the full database check; preserve catalog report revision evidence as the coverage blocker and migrate it with its API/SDK consumers. Private preference/default initialization without Self remains open. |
