...
| Phase | `repair`: native governance projection and the full 60-migration database sequence pass; main TypeScript and the final serial catalog inspector pass. Resolve native catalog report evidence with its persistence/API/SDK consumers and rerun coverage on the rebuilt empty target. Migrate private preference/default initialization without Self. Full product, coverage and retained-web acceptance remain open. |
