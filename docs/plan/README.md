...
| Phase | `repair`: native invitation capacity passes 17 assertions including both exact last-slot races; scoped TypeScript passes. Continue native recovery/expiry qualification, seed writers and private preference/default initialization without implicit Self. Full main integrity retains 31 type errors and the legacy preference bootstrap failure. |
...
| Acceptance | [Partial repair evidence](../testing/database/foundation-integrity-repair-evidence.json) records the passing scoped checks and failed full typecheck. The [native Realm](../testing/database/realm-native-enrollment-evidence.json) and [account/Org](../testing/database/native-account-org-evidence.json) checkpoints qualify only their listed command/HTTP paths. Remaining [native-consumer regressions](../testing/known-failures.md#main-iam-qualification-after-schema-extraction) must close before this integrity scope passes. |
...
| --- | --- | --- | --- | --- | --- |
| M01 | [Foundation](modules/foundation.md) | None | Identity/access and adapter contracts selected | RoleBinding/representation/workload persistence, native subject/path evaluation, assignment ceilings, identity/default-context APIs, scope/Role/Binding/Ceiling management APIs and App declaration/lifecycle APIs implemented; OAuth client/consent/installation/context storage, live readers and bounded credential erasure implemented; private Group lifecycle/selection/roster/discovery/evaluation, independent approvals, original-path recovery and native Org/Realm enrollment APIs/consumers implemented; representation administration, Group-derived Org directory, all-scope representation policy, replacement recovery, remaining protocol integration and consumer/onboarding migration pending | G2 partial; [foundation repair](../testing/database/foundation-integrity-repair-evidence.json) qualifies the current Group/RoleBinding storage fixtures and 136 earlier unit cases; [native Realm enrollment](../testing/database/realm-native-enrollment-evidence.json) adds 187 assertions / 132 HTTP requests and the current 142-test unit slice. [Native account/Org repair](../testing/database/native-account-org-evidence.json) adds 232 erasure and 197 Org assertions (6/70 HTTP requests), and reruns 50 retained private-lifecycle assertions/four races plus the 142-test unit slice. [Account enforcement](../testing/database/account-native-participation-evidence.json) adds 69 assertions/five HTTP requests and retains six observed race orderings. [Native roster](../testing/database/realm-native-roster-evidence.json) adds 433 assertions/211 HTTP requests and 22 schema tests. [Native invitation capacity](../testing/database/membership-native-capacity-evidence.json) adds 17 assertions and two observed last-slot races. The remaining 31 main type errors and full IAM/OAuth/default/ceiling/Org/Realm/API, migration and retained-web qualification remain open. Remaining [foundation](../testing/foundation.md) and [IAM/APP](../testing/identity-and-access.md) qualification pending. |
