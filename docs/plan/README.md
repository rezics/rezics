...
| Phase | `repair`: native recovery passes 36 assertions/seven HTTP requests and scoped TypeScript. Replace the three seed writes through read-only Realm views with native consent/admission/enforcement/rule history, and qualify an empty installed disposable seed target. Then reconcile private preference/default initialization without implicit Self. Full main integrity and the legacy preference bootstrap remain open. |
