...
| Phase | `repair`: native account erasure, Org enrollment and retained private-lifecycle checks pass their scoped verification. Continue private account/preferences consumers, then roster/recovery/capacity and seed reconciliation against the native contracts. Preserve the legacy locale-preference bootstrap failure and 100 remaining main type errors until their owning checks pass. |
