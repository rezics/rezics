...
| --- | --- |
| Scope | Complete the independently consumable vocabulary/schema package and its PostgreSQL persistence contracts. The maintainer selected this scope on 2026-09-17; prior IAM progress below remains pending. |
| Phase | `verification` complete for the selected schema-package scope; broader domain integration remains a separate scope. |
| Owners | `libraries/schema`, [semantic interoperability](../architecture/semantic-interoperability.md), and [database identity contracts](../architecture/database/README.md). |
| Deliverables | Pinned complete vocabulary inputs, a repeatable importer, stable term identity, independent definition/label versions, explicit application profiles, exported Drizzle schema, generated SQL, and necessary persistence commands. Preserve source graphs and provide portable exports, differences, and bounded lookup. |
| Exclusions | Full HTTP/API acceptance, UI, unrelated IAM implementation, wholesale replacement of existing domain services, and production distributed-database qualification. Only APIs/commands needed to qualify these schema contracts are included. |
| Acceptance | Package type checks; complete pinned-vocabulary reconstruction and semantic preservation; repeat/idempotency, translation/version, relation/history, rejected-state and portable-export cases; generated SQL replay and persistence checks on a task-owned PostgreSQL target. No whole-platform G2/G3/G4 claim. |
...
| M02 | [Knowledge and graph](modules/knowledge-and-graph.md) | M01 | Graph, Tag classification, Event temporal facts, source interoperability and verification/acceptance semantics selected | Independent `@rezics/schema` vocabulary/identity/profile and reusable Drizzle relation schema implemented; existing domain integration remains pending | [Standalone schema scope](../../libraries/schema/README.md#verified-scope): TypeScript, 12 semantic tests and 16 PostgreSQL scenarios passed; broader M02/G2 remains pending. |
...
| M07 | [Sources and converters](modules/sources-and-converters.md) | M01/M02 source contracts; M01-M04 native adoption | Schema.org/Wikidata full-index profiles and verification evidence/lineage intake selected alongside catalog sources; field conformance pending | Complete selected vocabulary importer and offline artifacts implemented in `@rezics/schema`; instance-source conversion remains pending | [Selected vocabulary scope](../../libraries/schema/README.md#verified-scope): 11 inputs, complete offline reconstruction and cross-database recovery passed; broader M07 remains pending. |
