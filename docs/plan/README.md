...
| Phase | `repair`: native account enforcement passes 69 assertions/five HTTP requests, retaining explicit authority and old-session denial after restoration. Continue native roster/recovery/capacity and seed reconciliation, plus private preferences/default initialization without implicit Self. Main integrity retains 61 type errors and the legacy preference bootstrap failure. |
