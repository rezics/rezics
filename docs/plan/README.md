...
| Phase | `repair`: the first corrections pass 136 unit cases, Group topology (80 native assertions) and RoleBinding persistence (91). Main TypeScript now reports 124 remaining errors in six legacy membership fixtures and three seed writes. Reconcile those consumers without weakening native authority or storage constraints. |
...
| Acceptance | [Partial repair evidence](../testing/database/foundation-integrity-repair-evidence.json) records the passing scoped checks and failed full typecheck. Remaining [native-consumer regressions](../testing/known-failures.md#main-iam-qualification-after-schema-extraction) must close before this integrity scope passes. |
