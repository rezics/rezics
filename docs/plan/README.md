...
| Acceptance | [Partial repair evidence](../testing/database/foundation-integrity-repair-evidence.json) records the initial scoped checks and earlier failed full typecheck; [native seed repair](../testing/database/native-seed-integrity-evidence.json) later closes that typecheck. [Private preferences](../testing/database/native-account-preferences-evidence.json) now qualify their direct-account contract. The [native Realm](../testing/database/realm-native-enrollment-evidence.json) and [account/Org](../testing/database/native-account-org-evidence.json) checkpoints qualify only their listed command/HTTP paths. Remaining [native-consumer regressions](../testing/known-failures.md#main-iam-qualification-after-schema-extraction) must close before this integrity scope passes. |
...
| --- | --- | --- | --- | --- | --- |
| M01 | [Foundation](modules/foundation.md) | None | Identity/access and adapter contracts selected | RoleBinding/representation/workload persistence, native subject/path evaluation, assignment ceilings, identity/default-context APIs, scope/Role/Binding/Ceiling management APIs and App declaration/lifecycle APIs implemented; OAuth client/consent/installation/context storage, live readers and bounded credential erasure implemented; private Group lifecycle/selection/roster/discovery/evaluation, independent approvals, original-path recovery, native Org/Realm enrollment APIs/consumers and direct private display/privacy preferences implemented; representation administration, Group-derived Org directory, all-scope representation policy, replacement recovery, remaining protocol integration and consumer/onboarding migration pending | G2/G3 partial; [foundation evidence](../testing/foundation.md) records scoped Group/RoleBinding, Org/Realm, erasure, account enforcement, roster and capacity checkpoints. [Native recovery](../testing/database/membership-native-recovery-evidence.json) adds 36 assertions/seven HTTP requests. [Native seed integrity](../testing/database/native-seed-integrity-evidence.json) closes main TypeScript errors and verifies demo/native enrollment, conversation initialization and aggregate checks. [Native governance projection](../testing/database/realm-native-governance-projection-evidence.json) adds 71 assertions and a passing full 60-migration database sequence. [Private preferences](../testing/database/native-account-preferences-evidence.json) add 72 assertions/32 HTTP requests, main/web/SDK checks and two scoped error-state screenshot reviews. Coverage catalog report evidence remains open under the selected [exact component reference](../architecture/content-governance.md#exact-reported-content-evidence); persistence/API/reviewer and empty-target coverage qualification are required. Remaining IAM/OAuth/default/ceiling/API, capacity/recovery, full product runtime and retained-web acceptance remain open. |
