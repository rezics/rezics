...
The [native model](../schema-modeling.md) defines Resource, Agent, generic Entity,
Space and typed addresses. [Field/storage policy](resource-storage.md) governs
retained columns and physical families. Relation names below describe target
responsibilities; the [runtime reference](../../reference/current-implementation.md)
and generated schema inventory record actual current identifiers and gaps.
...
| agent_participation | Native Agent FK; activation/state/control generation | Catalog existence and public identity do not prove participation or control | Agent PK; lifecycle reverse dependencies |
| main_agent_preference, app_agent_preference | Principal, optional client/scope; Agent FK; expected preference revision | Private convenience selection, not ownership or permission; admission validates usability and never retargets prepared commands/consents | Principal/client unique preference; Agent reverse |
| access_subject | Stable private key; exactly one AuthPrincipal FK or Agent FK | Typed grantee/member reference with concrete identity; not a global parent for native objects or a public account directory | Unique per alternative; selective reverse indexes |
| access_scope | Stable authority key; exactly one registered platform root, private AuthPrincipal account FK or canonical resource REF | Org/Agent, Realm and other public roots use the same canonical REF path; no direct-owner aliases. Private accounts never become public Resource references | Unique owner scope; target reverse |
| membership, invitation/application, membership_event | Scope, typed subject, admission generation, state/revision; separate invitation/terms/expiry/evidence | One current enrollment per declared scope/subject; accepting validates current policy. Rejoin creates a new generation. Public Agent participation and private operational membership have explicit owner policies. | Scope/subject and subject/scope keysets; state/expiry; event owner/sequence |
...
| team_profile | Group FK; optional admitted Agent presentation, collaboration metadata/revision | One underlying roster; public Team identity/control is not inferred from membership | Group unique profile; presentation reverse |
...
| ownership, ownership_event | Target scope/REF; protected owning subject or owner set under domain cardinality policy; revision/transfer history | One accountable Agent may have several controllers; multiple owners only where explicitly supported. No silent ownership transfer from membership. | Target/current and owner/target; serialized continuity check |
...
| oauth_consent, external_account_binding | Client, verified issuer/subject context, selected authority subject/Agent, scopes/resources, grant generation, status | Public Agent identity separate from pairwise login identity; no account merge or default-driven retargeting | Client/subject and Agent/binding keysets; revocation/erasure |
...
Representation is many-to-many. A default preference is not the old unique self binding, and an arbitrary acting-Agent value is not proof. Keep concrete foreign keys for every admitted alternative; a discriminator plus unchecked id is insufficient. Public selectors expose authorized Agent presentation or purpose-scoped private handles, not this internal subject directory.
...
| <owner>_name_authority / revision | Exact name REV, authorizing Agent, scope, evidence, active state | Officialness is scoped and revision-specific; changed name does not inherit old approval | Name/scope; authority reverse |
...
| publishing_serialization, publishing_installment | Serialization identity; installment occurrence, issue/order/original number, text/coverage | Installment appearance differs from chapter text; ordinary progress uses stable occurrences; fixed installments retain exact manifests |
...
| review_target | Publication/target slot; subject REF, review context, role, optional exact rating REV or explicit live observation reference | Textual opinion need not include a numeric rating; changing target is explicit history; publication/reposting does not submit a rating |
...
[Space composition](../space-composition.md) selects shared identity and
...
| content_structure, structure_manifest, node/occurrence | Owner/structure/head; manifest; occurrence id, same-manifest parent, typed Post/channel REF or exact REV/citation, rank, coverage, local label/number/credit | Explicit local contents; ordinary chapter uses follow context-eligible published content; fixed selections remain exact; validated navigation is acyclic; no implicit nested expansion |
...
The [composition protocol](content-composition.md) defines planning, staged writes, completeness validation, activation and refresh. Ordinary chapter references resolve eligible Post publication heads; fixed releases and reviewed adoption pin exact content/structure revisions. Child-list cursors bind structure/manifest/parent/order/occurrence; whole export is resumable. Existing 2,048-node and 64-placement guards remain implementation protections until the bounded replacement is qualified, not permanent target cardinalities.
...
| rating_context / context_revision | Context identity/governance scope; exact question, scale, target/population, cadence/calendar/admission and default aggregation policy | [Rating contract](ratings.md): semantic change creates another context; a default-method change versions policy without rewriting observations |
| rating_observation / observation_revision | Observation id, private counting identity, public attribution, target REF, context/slot, evaluation and original submission times; REV value/state/visibility/operation | UQ counting identity/target/context/slot; standing singleton, daily period or per-experience occasion; CAS revision and idempotent receipt; no duplicate votes from persona switching |
| rating_effective_head / rater_summary | Observation/head REV; target/context/counting identity and generation; selected latest observation or sufficient sum/count state | Withdrawal/private latest cannot resurrect an older current score; summaries are rebuildable and current disclosure applies |
| post_score / rating citation | Publication plus exact rating REV or explicitly selected live reference, local order | Citations preserve context/observation/revision meaning; no vote transfer or new observation on publication |
...
| unit_follow, follow_preference, tag subscription | Public Agent/REF pair; Auth preference FK to the exact pair; target/scope, delivery policy | Native self-follow check decodes REF; private erasure is separate from public interest; Follow is not membership, friendship or automatic access grant |
...
| unit_progress, progress_entry, node_progress, post_progress | Account,target, stable occurrence, position/status and observation; optional observed REV/selector or fixed selection | Ordinary reorder/body updates retain read status; replacement/removal and precise-position relocation are explicit; fixed-edition changes require mapped/unknown state |
...
The [complete rating contract](ratings.md) replaces the mutable-only `score` and single Realm rules-post model. A Realm can own several contexts. Defaults are latest-per-rater then equal-rater mean; history means first reduce observations within each rater over an explicit range. Raw observation counts/distributions and per-rater representatives remain separate. Scale, time basis, policy/generation, safe count serialization and contributor drill-down are required query semantics. Imported scores remain D03 `source_statistic`.

Account-private Tag current rows use a canonical target REF. The account/target/Tag key permits independent annotations by different accounts while rejecting duplicate triples. Self-targeting is checked against the reference's derived native identity; content labels and category-only concepts retain their separate applicability rules. Normal private filtering resolves the authenticated account owner; Agent selection does not switch that owner or import another controller's private Tags. A delegated administrative path requires an explicit private target/action policy under D02. Query/index budgets live with the [Filter compiler](../../../services/main/src/services/filter/README.md).

Global, Realm and account applications share the logical Resource target contract, not one scope-free assertion or copied per-kind Tag service. Apply owner state/applicability and feature authorization before allocating a reference or recording a judgment. Effective-Tag and reverse-discovery rows are rebuildable; reference existence never grants disclosure. Converting a target's physical storage must preserve the feature's target, scope and history rather than recreate its votes or memberships.
...
| verification_index / definition_revision | Operator/index identity; immutable coverage, acceptance-policy REV, supported queries and disclosure contract | Operated index resource, not a global Resource parent or Realm placeholder; approved Subscribe mappings affect access only. |
...
| Hosted third-party accounts | Provider/tenant account, credential, recovery, assurance and private setting boundaries when elected | Shared Agent connections and verifiable representation are selected in D02; hosting another platform's account system remains deferred |
...
| Advanced AI/3D/media operations | Weights/mesh artifact, representation, domain-specific license/use and processing operations | Basic company/model/version catalog references and ratings are selected scope; artifact hosting/execution remains independent. Equal bytes and different provenance/rights can remain different asset uses; deployment is not model identity |
...
| ConnectOrInstallApp | Verified client/subject, selected Agent, current representation, resource selection and approval revision | Private connection/consent or scope-owned installation and narrowed credential context | No name/email claim, global Principal leak, hidden consent, default retarget or client-selected installation substitution |
...
| ResolveOrSubmitRating | Explicit context/revision, target, counting identity, action, slot/occasion, expected observation revision and operation key | New observation or same-observation revision/head plus durable projection work | No silent context switch, duplicate-slot overwrite, retry-created experience or new ballot from a revision |
| ManageRatingContext | Context-management authority, expected definition/policy revision and semantic-change classification | Versioned default/presentation change or explicitly new semantic context | Existing observations retain exact question/scale meaning; default changes do not create votes |
| AcceptOrCorrectEventTime | Typed temporal role/value, native occurrence or fact-slot authority, expected revision and source decision | Accepted date revision/effective value and temporal-index invalidation | Preserve actual/planned/recorded distinctions, uncertainty and single writer; stale dates cannot reactivate |
...
| Multilingual Work contents | Work-subject slot, language/variant dimensions, adoption REV/XREV, structure/manifest/occurrence | Multiple official/community and same-language contributions; ordinary chapter channels follow eligible published heads, while reviewed/fixed selections preserve exact provenance and historical interpretation. |
...
| --- | --- | --- |
| subscription_offering | Stable ID, eligible target REF, operating Agent FK, lifecycle/control revision | Target, seller and covered resources are distinct; current authority required; target/operator reverse indexes |
...
| Site delivery profile | Bounded deployment registry: ID/revision, exact origins, required predicate, presentation Realm, optional Zone and endpoint classifications | First release uses explicit Main and fixed-Pro profiles at rezics.com and pro.rezics.com with one frontend codebase. No new Realm hierarchy, canonical address registry or authorization grant; D15 connections retain context |
...
public Agent roster rows. Follow, enrollment, selection subscriptions and notification
