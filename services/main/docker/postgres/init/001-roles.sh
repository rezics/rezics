...
for variable_name in POSTGRES_USER REZICS_DATABASE_USERNAME; do
...
for variable_name in REZICS_DATABASE_PASSWORD; do
...
if [[ "${POSTGRES_USER}" == "${REZICS_DATABASE_USERNAME}" ]]; then
...
psql \
	...
	--set app_password="${REZICS_DATABASE_PASSWORD}" <<'SQL'
SELECT format(
