SELECT
	...
	ELSE jsonb_build_object(
		...
		'filters', jsonb_build_object(
			...
			'creditedProfileIds', coalesce(credited_profile_data.profile_ids, '[]'::jsonb),
...
LEFT JOIN LATERAL (
	SELECT CASE
		WHEN unit_row.kind IN ('book', 'software', 'media', 'series', 'video', 'audio', 'zone') THEN 'units'
...
LEFT JOIN LATERAL (
	...
	WHERE tag_id = source.unit_id
) AS realm_tag_context_data ON true
LEFT JOIN LATERAL (
	SELECT jsonb_agg(
...
LEFT JOIN LATERAL (
	...
	FROM (
		...
		WHERE source_credit.source_unit_id = source.unit_id
	) AS resolved_credit
) AS credited_profile_data ON true
LEFT JOIN LATERAL (
