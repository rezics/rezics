SELECT
	...
	ELSE jsonb_build_object(
		...
		'searchKind', CASE category.value
			...
			WHEN 'entities' THEN entity_row.kind
		...
		'filters', jsonb_build_object(
			...
			'publisherProfileIds', coalesce(publisher_data.profile_ids, '[]'::jsonb),
		...
		'contentLicense', jsonb_build_object(
			'active', content_license_row.unit_id IS NOT NULL
		),
...
LEFT JOIN public.unit_content_license AS content_license_row ON content_license_row.unit_id = source.unit_id AND content_license_row.revoked_at IS NULL
...
LEFT JOIN LATERAL (
	SELECT CASE
		...
		WHEN unit_row.kind = 'entity' THEN 'entities'
...
LEFT JOIN LATERAL (
	...
	FROM public.realm_tag_vote_stat
	WHERE unit_id = source.unit_id
...
LEFT JOIN LATERAL (
	SELECT jsonb_agg(profile_id ORDER BY profile_id) AS profile_ids
	FROM (
		...
		WHERE direct_credit.source_unit_id = source.unit_id
			AND direct_credit.role = 'publisher'
			AND unit_row.kind IN ('entity', 'collection', 'post')
		UNION
		...
		WHERE work_publisher.source_unit_id = source.unit_id
			AND work_publisher.role = 'publisher'
			AND unit_row.kind IN ('book', 'media', 'software')
