...
const allowRules: readonly AllowRule[] = [
	...
	{
		file: "search/service.ts",
		line: /select count\(\*\)(?:::integer)? from (?:accepted|ordered_source|scanned_candidates)/,
		maximumOccurrences: 13,
		reason: "all three materialized relations are capped by the Search scan or page limit",
	},
	{
		file: "recommendations/worker.ts",
		line: /count\(\*\) filter \(where type =/i,
		maximumOccurrences: 4,
