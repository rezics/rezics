...
export async function captureTypedDatabaseSchema() {
	...
	for (const [table, key] of [...Object.entries(sourcePartitionKeys), ...operationalPartitionTables.map(table => [table, "routing_bucket"] as const)])
		if (!tables.get(table)?.has(key)) throw new Error(`Physical partition registry targets a missing table or key: ${table}.${key}`);
