...
import { identityPreference, identityPreferenceEvent } from "../database/schema/identity-preference";

const versionSchema = z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER);
const selectionSchema = z.discriminatedUnion("kind", [
	z.strictObject({ kind: z.literal("entity"), entityId: z.uuid().toLowerCase() }),
...
export async function captureIdentityPreference(
...
): Promise<{ entityId: string | null; source: "client" | "main" | "none"; mainVersion: number; clientVersion: number }> {
	...
	if (client && client.selectionKind !== "inherit-main")
		return { entityId: client.entityId, source: "client", mainVersion, clientVersion };
	return { entityId: main?.entityId ?? null, source: main?.entityId ? "main" : "none", mainVersion, clientVersion };
}

/** Delete at most 500 private receipts or one empty head after the account erasure frontier. @internal */
