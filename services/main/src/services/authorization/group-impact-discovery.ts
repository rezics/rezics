...
type Kind = "subtree" | "group" | "group-context" | "binding" | "binding-context" | "role" | "ceiling" | "representation" | "representation-context" | "membership";
...
function fenceKind(kind: string) {
	if (kind === "subtree" || kind === "group-context") return "group";
...
async function retain(tx: DatabaseTransaction, review: Review, kind: string, key: string) {
	...
	if (!fence) throw new DiscoveryStop("missing");
	...
	if (inserted.length) {
		if (++review.witnessCount > maxNodes * 2 + 1) throw new DiscoveryStop("budget");
	}
}
async function enqueue(tx: DatabaseTransaction, review: Review, kind: Kind, key: string | null | undefined) {
	...
	if (existing) return;
	if (review.nodeCount >= maxNodes) throw new DiscoveryStop("budget");
	await tx.insert(nodes).values({ reviewId: review.id, kind, key });
	review.nodeCount++;
}
function observeTimes(review: Review, payload: unknown) {
	if (!payload || typeof payload !== "object") return;
	for (const [key, value] of Object.entries(payload)) {
		if (["valid_from", "valid_until", "grant_not_after"].includes(key) && typeof value === "string") {
			const time = new Date(value);
			if (!Number.isFinite(time.getTime())) throw new DiscoveryStop("missing");
		...
		} else if (value && typeof value === "object") observeTimes(review, value);
	}
}
async function saveFact(tx: DatabaseTransaction, review: Review, kind: string, payload: Payload) {
	const bytes = Buffer.byteLength(JSON.stringify(payload), "utf8");
	if (review.factCount >= maxFacts || review.byteCount + bytes > maxBytes) throw new DiscoveryStop("budget");
	observeTimes(review, payload);
...
export async function revalidateGroupImpactDiscovery(tx: DatabaseTransaction, review: Review): Promise<void> {
	...
	if (rows.length > maxNodes * 2 + 1) throw new DiscoveryStop("budget");
...
export async function beginGroupImpactDiscovery(tx: DatabaseTransaction, input: z.infer<typeof GroupImpactProposalSchema> & {
	...
	await retain(tx, review, "tree", input.scopeId);
	await retain(tx, review, "group", input.groupId);
	...
	for (const [kind, root] of [["old-ancestor", head.parentId], ["new-ancestor", proposal.proposedParentId]] as const) {
		...
		while (key !== null) {
			...
			await retain(tx, review, "group", key);
...
async function point(tx: DatabaseTransaction, table: string, keyColumn: string, key: string): Promise<Payload> {
	...
	if (!row) throw new DiscoveryStop("missing");
	return row.payload;
}
async function terms(tx: DatabaseTransaction, table: string, column: string, key: string, revision: number) {
	...
	if (!row || row.payload.sealed !== true) throw new DiscoveryStop("missing");
	return row.payload;
}
async function permissions(tx: DatabaseTransaction, table: string, column: string, key: string, revision: number | null, snapshot: Payload) {
	...
	if (rows.length > AccessPermissionValues.length) throw new DiscoveryStop("budget");
...
async function captureEligibility(tx: DatabaseTransaction, review: Review, input: unknown) {
	...
	if (!basis.membership_generation) throw new DiscoveryStop("missing");
	await retain(tx, review, "membership", basis.membership_id);
	...
	if (!admission) throw new DiscoveryStop("missing");
	let selection: Payload | null = null;
	if (basis.selection_group_id) {
		await retain(tx, review, "group", basis.selection_group_id);
...
async function capture(tx: DatabaseTransaction, review: Review, node: Node) {
	await retain(tx, review, fenceKind(node.kind), node.key);
	...
	if (kind === "group") {
		if (!head.scope_id) throw new DiscoveryStop("missing");
		await retain(tx, review, "tree", head.scope_id);
		...
		if (kind === "representation") {
			...
			if (head.parent_grant_id && head.parent_revision) {
				await retain(tx, review, "representation", head.parent_grant_id);
		...
		if (payload.sealed !== true) throw new DiscoveryStop("missing");
		...
		if (!head.manager_binding_id) throw new DiscoveryStop("missing");
		await retain(tx, review, "binding", head.manager_binding_id);
		...
		if (selections.length > 64) throw new DiscoveryStop("budget");
	...
	try {
		...
		await tx.transaction(async page => {
			...
			for (let steps = 0; steps < 8 && candidates < 400; steps++) {
				if (next.workCount >= maxWork) throw new DiscoveryStop("budget");
				...
				else await retain(page, next, fenceKind(node.kind), node.key);
		...
		if (!(error instanceof DiscoveryStop) && !(error instanceof AccessPermissionSnapshotUnavailable)) throw error;
		review.status = "unavailable"; review.reason = error instanceof DiscoveryStop ? error.reason : "missing";
		await persist(tx, review);
	}
}
