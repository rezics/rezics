...
export async function recommendUnits(input: {
 viewer: RecommendationViewer; snapshot: RecommendationSnapshotContext | null;
 owner?: CatalogOwner; shape?: string; seedUnitId?: string; asOf: Date; pageSize: number;
 localizationLanguages: readonly ContentLanguage[]; afterId?: string; requestId: string;
}) {
 const owners = input.owner ? [input.owner] : [...CatalogOwnerValues];
 const target = unitStateRelation(sql`null::uuid`, "search_unit");
 const eligible = and(eq(target.status,"published"),eq(target.visibility,"public"),
 ...
 const selected = await searchGlobalIdentifiers({
  branches: [
   ...
   ...(owners.some(owner=>owner!=="entity") ? [{category:"units" as const,sourceOwners:owners.filter(owner=>owner!=="entity"),sourceShapes:input.shape ? [input.shape] : []}] : []),
  ...
  ...(input.viewer.profileId ? {profileId:input.viewer.profileId} : {}), additionalConditions:[eligible],
  limit:RecommendationPolicy.maxCandidates,sort:"best",bestSnapshotId:input.snapshot?.id??null,
 });
 if (!selected.hits.length) return input.afterId ? null : {items:[],nextId:undefined};
 const ids=selected.hits.map(hit=>hit.id);
 return database.transaction(async tx=>{
  const rows = await tx.select({id:target.id,owner:target.owner,shape:target.shape,
   contentRating:target.contentRating,createdAt:target.createdAt,updatedAt:target.updatedAt,publishedAt:target.publishedAt,
   coverAssetId:resolvedUnitLocalizationImageAssetId(target.id,"cover",input.localizationLanguages),
   bestScore:sql<number>`coalesce(${unitBestScore.score},0)`,
  }).from(unitStatesForIds(ids,"search_unit"))
   .leftJoin(unitBestScore,input.snapshot ? and(eq(unitBestScore.snapshotId,input.snapshot.id),eq(unitBestScore.unitId,target.id)) : sql`false`)
   .where(eligible);
  const ranked=rankRecommendations(rows.flatMap(row=>{
  ...
  const presentations=await readUnitPresentationsInTransaction(tx,page.map(row=>row.id),input.localizationLanguages);
  const items=page.flatMap((row,index)=>{
   const presentation=presentations.get(row.id);if(!presentation) return [];
   const recommendationReason:RecommendationReason=row.bestScore>0 ? "popular_now" : "new_and_relevant";
   return [{...row,language:presentation.language,title:presentation.title,summary:presentation.summary,
    cover:presentImageAsset(row.coverAssetId,"cover"),recommendationReason,
    tracking:createRecommendationTracking(row.id,{requestId:input.requestId,surface:input.seedUnitId ? "unit_related" : "home_catalog",
     position:start+index,policyVersion:input.snapshot?.policyVersion ?? RecommendationPolicyVersion}),
   }];
  });
