...
describe("database schema contracts", () => {
	...
	it("stores public credit relationships as Unit-to-Unit attribution", () => {
		...
		expect(attribution.columns.map((column) => column.name)).toEqual(
			expect.arrayContaining(["source_unit_id", "credited_entity_id", "role", "position"]),
		);
	...
	it("stores a Profile default Score Realm with referential integrity", () => {
		const preference = getTableConfig(accountPreference);
		expect(preference.foreignKeys.map((key) => key.getName())).toContain(
			"account_preference_default_score_realm_id_realm_id_fk",
		);
		expect(preference.indexes.map((index) => index.config.name)).toContain(
			"account_preference_default_score_realm_idx",
		);
		expect(preference.checks.map((constraint) => constraint.name)).toContain(
			"account_preference_content_ratings_check",
		);
		expect(accountPreference.scoreVisibility.enumValues).toEqual(["public", "unlisted", "private"]);
		expect(accountPreference.progressVisibility.enumValues).toEqual([
			"public",
			"unlisted",
			"private",
		]);
		expect(accountPreference.scoreVisibility.default).toBe("public");
		expect(accountPreference.progressVisibility.default).toBe("public");
