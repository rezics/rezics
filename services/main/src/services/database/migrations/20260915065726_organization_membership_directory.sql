...
CREATE INDEX "access_representation_subject_page_idx" ON "access_representation" ("recipient_subject_id","id") WHERE "recipient_subject_id" is not null;
CREATE INDEX "access_role_binding_subject_page_idx" ON "access_role_binding" ("recipient_subject_id","id") WHERE "recipient_subject_id" is not null;
ALTER TABLE "participation_grant" DROP CONSTRAINT "participation_grant_capability_check", ADD CONSTRAINT "participation_grant_capability_check" CHECK ("capability" in ('catalog.read', 'catalog.edit', 'entity.publish', 'entity.security', 'proposal.adopt'));
