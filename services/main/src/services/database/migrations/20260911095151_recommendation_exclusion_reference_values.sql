...
ALTER TABLE "recommendation_exclusion" DROP CONSTRAINT "recommendation_exclusion_unit_id_check", DROP CONSTRAINT "recommendation_exclusion_unit_target_check", DROP CONSTRAINT "recommendation_exclusion_pkey", DROP COLUMN "unit_id", DROP COLUMN "unit_publishing_id", DROP COLUMN "unit_music_id", DROP COLUMN "unit_program_id", DROP COLUMN "unit_software_id", DROP COLUMN "unit_entity_id", DROP COLUMN "unit_grouping_id", DROP COLUMN "unit_reference_id", DROP COLUMN "unit_distribution_id", DROP COLUMN "unit_video_id", DROP COLUMN "unit_audio_id", DROP COLUMN "unit_post_id", DROP COLUMN "unit_poll_id", DROP COLUMN "unit_zone_id", DROP COLUMN "unit_realm_id", DROP COLUMN "unit_realm_rule_id", DROP COLUMN "unit_custom_theme_id", DROP COLUMN "unit_collection_id", DROP COLUMN "unit_tag_id", DROP COLUMN "unit_tag_path_id", DROP COLUMN "unit_label_id", ADD COLUMN "target_reference_id" uuid NOT NULL, ADD PRIMARY KEY ("auth_user_id", "target_reference_id"), ADD CONSTRAINT "recommendation_exclusion_YIFwrfnPSrf4_fkey" FOREIGN KEY ("target_reference_id") REFERENCES "reference_value" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT;
-- Create index "recommendation_exclusion_target_idx" to table: "recommendation_exclusion"
CREATE INDEX "recommendation_exclusion_target_idx" ON "recommendation_exclusion" ("target_reference_id", "auth_user_id");
