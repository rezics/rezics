...
-- Create "access_group_tree" table
CREATE TABLE "access_group_tree" (
  "scope_id" uuid NOT NULL,
  "version" bigint NOT NULL DEFAULT 0,
  PRIMARY KEY ("scope_id"),
  CONSTRAINT "access_group_tree_scope_id_access_scope_id_fkey" FOREIGN KEY ("scope_id") REFERENCES "access_scope" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_tree_version_check" CHECK ((version >= 0) AND (version <= '9007199254740991'::bigint))
);
...
CREATE TABLE "access_group" (
  ...
  "subtree_height" smallint NOT NULL DEFAULT 1,
  PRIMARY KEY ("id"),
  CONSTRAINT "access_group_scope_id_access_group_tree_scope_id_fkey" FOREIGN KEY ("scope_id") REFERENCES "access_group_tree" ("scope_id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_height_check" CHECK ((subtree_height >= 1) AND (subtree_height <= 8)),
  CONSTRAINT "access_group_parent_check" CHECK ((parent_id IS NULL) OR (parent_id <> id)),
  CONSTRAINT "access_group_state_check" CHECK (state = ANY (ARRAY['draft'::text, 'active'::text, 'retired'::text])),
  CONSTRAINT "access_group_version_check" CHECK ((version >= 0) AND (version <= '9007199254740991'::bigint))
);
-- Create index "access_group_id_scope_key" to table: "access_group"
CREATE UNIQUE INDEX "access_group_id_scope_key" ON "access_group" ("id", "scope_id");
-- Create index "access_group_parent_height_idx" to table: "access_group"
CREATE INDEX "access_group_parent_height_idx" ON "access_group" ("scope_id", "parent_id", "subtree_height" DESC NULLS LAST, "id") WHERE (state = 'active'::text);
-- Create index "access_group_scope_id_idx" to table: "access_group"
CREATE INDEX "access_group_scope_id_idx" ON "access_group" ("scope_id", "id");
-- Add post-creation foreign keys to the "access_group" table
ALTER TABLE "access_group" ADD CONSTRAINT "access_group_parent_scope_fk" FOREIGN KEY ("parent_id", "scope_id") REFERENCES "access_group" ("id", "scope_id") ON UPDATE NO ACTION ON DELETE RESTRICT;
-- Create "access_group_event" table
CREATE TABLE "access_group_event" (
  ...
  CONSTRAINT "access_group_event_authority_subject_id_access_subject_id_fkey" FOREIGN KEY ("authority_subject_id") REFERENCES "access_subject" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_event_group_id_access_group_id_fkey" FOREIGN KEY ("group_id") REFERENCES "access_group" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_event_operator_auth_user_id_users_id_fkey" FOREIGN KEY ("operator_auth_user_id") REFERENCES "users" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_event_parent_after_id_access_group_id_fkey" FOREIGN KEY ("parent_after_id") REFERENCES "access_group" ("id") ON UPDATE NO ACTION ON DELETE RESTRICT,
  CONSTRAINT "access_group_event_description_check" CHECK ((description IS NULL) OR (octet_length(description) <= 4096)),
  CONSTRAINT "access_group_event_digest_check" CHECK (request_digest ~ '^[0-9a-f]{64}$'::text),
  CONSTRAINT "access_group_event_label_check" CHECK ((length(btrim(label)) > 0) AND (octet_length(label) <= 512)),
  CONSTRAINT "access_group_event_operation_check" CHECK (operation = ANY (ARRAY['create'::text, 'update'::text, 'reparent'::text, 'retire'::text])),
  CONSTRAINT "access_group_event_state_check" CHECK (((operation = 'retire'::text) AND (state_after = 'retired'::text)) OR ((operation <> 'retire'::text) AND (state_after = 'active'::text))),
  CONSTRAINT "access_group_event_version_check" CHECK ((version >= 1) AND (version <= '9007199254740991'::bigint))
);
-- Create index "access_group_event_operation_key" to table: "access_group_event"
CREATE UNIQUE INDEX "access_group_event_operation_key" ON "access_group_event" ("group_id", "operation_id");
