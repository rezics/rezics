...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM public.tag_expression
		WHERE id = target_expression_id AND sealed_at IS NOT NULL
	) THEN
	...
	WHERE expression_id = target_expression_id;
	IF EXISTS (
		SELECT 1 FROM public.tag_expression
		WHERE id = target_expression_id AND status = 'retired'
	) THEN
		RETURN;
	END IF;
	...
	WITH RECURSIVE reachable(expression_id, evidence_kind, trail, depth) AS (
		SELECT rule.target_expression_id,
			rule.inference_kind,
			ARRAY[target_expression_id, rule.target_expression_id],
			1
		FROM public.tag_expression_inference_rule AS rule
		WHERE rule.source_expression_id = target_expression_id
	...
	SELECT target_expression_id, tag_id, evidence_kind FROM strongest;
END;
$$;
...
AS $$
DECLARE
	changed_expression_id uuid := CASE
		WHEN TG_TABLE_NAME = 'tag_expression' THEN NEW.id
		WHEN TG_OP = 'DELETE' THEN OLD.source_expression_id
		ELSE NEW.source_expression_id
	END;
	impacted_expression_id uuid;
BEGIN
	IF TG_TABLE_NAME = 'tag_expression' AND NEW.sealed_at IS NULL THEN RETURN NULL; END IF;
...
BEGIN
	IF NEW.status <> 'active' OR NEW.target_expression_id IS NULL THEN RETURN NEW; END IF;
	-- Definition writes are rare. Serialize graph changes so concurrent inverse
	-- rules cannot both pass the cycle check under snapshot isolation.
	...
	IF EXISTS (
		...
		SELECT 1 FROM descendant WHERE expression_id = NEW.source_expression_id
	) THEN
