...
ALTER TABLE "music_artist_credit" ADD CONSTRAINT "music_credit_prefix_check" CHECK (((member_count >= 0) AND (member_count <= '9007199254740991'::bigint)) AND ((last_position >= '-1'::integer) AND (last_position <= '9007199254740991'::bigint)) AND ((sealed_at IS NULL) OR ((member_count > 0) AND (last_position = (member_count - 1))))), ADD COLUMN "member_count" bigint NOT NULL DEFAULT 0, ADD COLUMN "last_position" bigint NOT NULL DEFAULT -1, ADD COLUMN "sealed_at" timestamptz(3) NULL, ADD COLUMN "retired_at" timestamptz(3) NULL;
-- Modify "music_artist_credit_name" table
ALTER TABLE "music_artist_credit_name" ADD COLUMN "source_position" bigint NULL;
