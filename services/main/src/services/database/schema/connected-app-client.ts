...
export const connectedAppClient = pgTable("connected_app_client", {
	...
	workloadPrincipalId: uuid().references(() => workloadPrincipal.authUserId, { onDelete: "restrict" }),
	...
	index("connected_app_client_workload_idx").on(table.workloadPrincipalId, table.clientId).where(sql`${table.workloadPrincipalId} is not null`),
...
export const connectedAppClientEvent = pgTable("connected_app_client_event", {
	...
	foreignKey({ name: "connected_app_client_event_retained_terms_fk", columns: [table.clientId, table.retainedTermsRevision],
		foreignColumns: [connectedAppClientRevision.clientId, connectedAppClientRevision.revision] }).onDelete("restrict"),
	check("connected_app_client_event_version_check", sql`${table.version} between 1 and 9007199254740991 and ${table.credentialEpochAfter} between 1 and ${table.version}`),
	check("connected_app_client_event_digest_check", sql`${table.requestDigest} ~ '^[0-9a-f]{64}$'`),
	check("connected_app_client_event_operation_check", inArray(table.operation, ["admit", "revise", "disable", "enable", "revoke"])),
	check("connected_app_client_event_state_check", inArray(table.stateAfter, ["active", "disabled", "revoked"])),
	check("connected_app_client_event_terms_check", sql`(${table.operation} in ('admit','revise') and ${table.retainedTermsRevision} is null) or (${table.operation} not in ('admit','revise') and ${table.retainedTermsRevision} is not null and ${table.retainedTermsRevision} between 1 and ${table.version}-1)`),
]);

/** Sealed App/protocol approval selection; broader manifests or metadata cannot change this snapshot. @internal */
export const connectedAppClientRevision = pgTable("connected_app_client_revision", {
	...
	foreignKey({ name: "connected_app_client_revision_event_fk", columns: [table.clientId, table.revision],
		foreignColumns: [connectedAppClientEvent.clientId, connectedAppClientEvent.version] }).onDelete("restrict"),
	foreignKey({ name: "connected_app_client_revision_owner_fk", columns: [table.clientId, table.appId],
		foreignColumns: [connectedAppClient.clientId, connectedAppClient.appId] }).onDelete("restrict"),
	foreignKey({ name: "connected_app_client_revision_app_fk", columns: [table.appId, table.appRevision],
		foreignColumns: [connectedAppRevision.appId, connectedAppRevision.revision] }).onDelete("restrict"),
	index("connected_app_client_revision_app_idx").on(table.appId, table.appRevision, table.clientId, table.revision),
	check("connected_app_client_revision_epoch_check", sql`${table.protocolCredentialEpoch} between 0 and 9007199254740991`),
	check("connected_app_client_revision_count_check", sql`${table.capabilityCount} between 0 and ${sql.raw(String(AccessPermissionValues.length + ApiPermissionValues.length))}`),
	check("connected_app_client_revision_digest_check", sql`${table.capabilityDigest} ~ '^[0-9a-f]{64}$'`),
]);

/** Literal client capability subset, kept distinct from user consent and resource grants. @internal */
export const connectedAppClientCapability = pgTable("connected_app_client_capability", {
	clientId: uuid().notNull(), revision: bigint({ mode: "number" }).notNull(),
	...
	foreignKey({ name: "connected_app_client_capability_revision_fk", columns: [table.clientId, table.revision],
		foreignColumns: [connectedAppClientRevision.clientId, connectedAppClientRevision.revision] }).onDelete("restrict"),
	check("connected_app_client_capability_registered_check", or(
