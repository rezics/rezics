...
describe("VNDB v11 evidence retention", () => {
	it("uses strict non-deferrable deletion for every independent evidence owner", () => {
		...
		expect(
			getTableConfig(unitStructureApplicationJudgment).foreignKeys.find(
				(key) => key.getName() === "unit_structure_application_judgment_application_fkey",
			)?.onDelete,
		).toBe("restrict");
	});
