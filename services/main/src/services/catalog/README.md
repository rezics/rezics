...
The [source-contract owner](source-contracts/README.md), [artifacts](../../../../../libraries/content-adapters/contracts/catalog/sources.json) and [field inventory](source-contracts/README.md) provide current acquisition definitions, ignored run inventories and reviewed dispositions. Use the existing generator and owning Taskfile, with reproducible network acquisition when needed. A local cache is disposable; no durable specification depends on its location or contents. Raw-only fields and declaration counts do not establish native coverage.
