...
function isAllowed(relPath: string): boolean {
  return (
    ...
    relPath.startsWith("packages/frontend/src/components/mocks/")
  );
}
...
export const uiAutonomyRule: RuleScanner = {
  scan({ tsAndTsxFiles }) {
    ...
    for (const filePath of tsAndTsxFiles) {
      ...
      if (!relPath.startsWith("packages/frontend/src/")) continue;
    ...
    try {
      const content = readFileSync(
        join(REPO_ROOT, "packages/frontend/src/components/shadcn/index.ts"),
        "utf8",
      );
      const lines = content.split("\n");
      for (let i = 0; i < lines.length; i++) {
        if (/from\s+["']\.\/sections["']/.test(lines[i]!)) {
          violations.push({
            rule: "R13",
            path: `packages/frontend/src/components/shadcn/index.ts:${i + 1}`,
            message:
