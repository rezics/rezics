...
// `--easing-*` surface emitted by `packages/ui/src/config/uno-config.ts` is the
// only sanctioned form.
// 匹配任何 `--rezics-*` CSS 变量引用。整个命名空间已废弃；唯一被认可的形式是
// `packages/ui/src/config/uno-config.ts` 输出的扁平
// `--colors-*` / `--radius-*` / `--shadow-*` / `--font-*` / `--duration-*` /
// `--easing-*` 变量集。
const REZICS_VAR_PATTERN = /var\(\s*--rezics-[a-zA-Z0-9_-]+/;

// `packages/ui/src/config/tokens.css` SHALL NOT exist — the tokens TS source is
// authoritative and uno-config.ts emits the runtime CSS variables.
// `packages/ui/src/config/tokens.css` 不应存在——tokens 的 TS 源才是权威，
// 由 uno-config.ts 输出运行时 CSS 变量。
const TOKENS_CSS_PATH = join(REPO_ROOT, "packages/ui/src/config/tokens.css");
...
export const tokenConsumptionRule: RuleScanner = {
  scan({ r9CandidateFiles }) {
    ...
    if (existsSync(TOKENS_CSS_PATH)) {
      violations.push({
        rule: "R9",
        path: "packages/ui/src/config/tokens.css",
        message:
          "`tokens.css` is forbidden — design tokens live in `packages/ui/src/config/tokens/*.ts` and are emitted as flat CSS variables by `uno-config.ts`. Delete this file.",
        spec: SPEC,
      });
    }
