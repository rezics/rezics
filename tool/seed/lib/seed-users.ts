...
import type { AuthPrismaClient, ServerPrismaClient } from "./create-prisma";
import { seedServerUser } from "./seed-server-user";
...
export async function resolveRootUserId(
  ...
  // Verify the user also exists in the server DB (FK target for Unit.userId)
