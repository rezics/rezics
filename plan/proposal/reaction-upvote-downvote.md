...
- [ ] 1.1 Update `package/contract/src/reaction/reaction.schema.ts` so
...
- [ ] 2.1 Add a reaction-service Drizzle migration under
  `package/reaction/drizzle/` that updates existing `Reaction.reaction` and
  `ReactionSummary.reaction` rows from `like`/`dislike` to
  `upvote`/`downvote`.
- [ ] 2.2 Update `package/reaction/src/reaction/reaction.service.test.ts` so
  service create/remove/list/summary/quota coverage uses `upvote`/`downvote`.
- [ ] 2.3 Run or update any reaction repository/service fixtures that encode
...
- [ ] 3.2 Rename the notification kind from `reaction.like` to
...
- [ ] 4.4 Update profile reaction history display in
...
- [ ] 5.1 Update `package/ranking/src/ranking/formulas.ts` and ranking tests so
  score weighting reads `upvote` and `downvote` counts.
- [ ] 5.2 Update factory and mock reaction seed values such as
  `package/server/src/db/factory/data.ts` from `like`/`dislike` to
  `upvote`/`downvote`.
- [ ] 5.3 Update profile/activity comments, fixtures, and tests that display or
  filter reaction keys so examples use `upvote` instead of `like`.
- [ ] 5.4 Search the repo for remaining exact `"like"`/`"dislike"` reaction
  values and update only reaction-owned usages; leave unrelated prose and
  non-reaction fields alone.

## 6. Verification

- [ ] 6.1 Run targeted contract tests for reaction schemas and notification kind
  registry.
- [ ] 6.2 Run targeted reaction service tests for create/remove/list/summary and
  migration-adjacent behavior.
- [ ] 6.3 Run targeted app engagement tests for vote action decisions and
  rendered vote state.
- [ ] 6.4 Run targeted server boundary tests for reaction create/delete and
  notification emission.
- [ ] 6.5 Run targeted ranking tests for score formulas.
- [ ] 6.6 Run `bun run check:convention` if the touched files include generated
