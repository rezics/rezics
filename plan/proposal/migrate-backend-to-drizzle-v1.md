...
- [x] 4.12 Replace Prisma mock helpers in server tests with either focused
  ...
  - Applied progress: realm subscription, membership, rule policy, and
...
- [x] 6.1 Convert `package/search` from `setSearchPrismaClient` to an explicit
  ...
  - Applied progress: moved content search sync (`syncSingleContent`,
    `syncAllContent`, `syncContentSegment`, `syncReleaseContentSegment`, and
    `syncGameMediaContentSegment`) to Drizzle reads and explicit hydration
    across server content, tagging, attribution, realm, series, shelf, post,
    link, game, media, and book tables.
  - Applied progress: moved post realm-id patch segments and contained-unit
    full sync to Drizzle reads against server `Post`, `Unit`, `UnitRealm`,
    `Realm`, and `ShelfUnit`.
  - Applied progress: moved post search sync (`syncSinglePost`,
    `syncAllPosts`, `syncPostSegment`, `syncPostsByAuthor`,
