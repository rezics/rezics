...
- [ ] 1.1 Update `package/server/src/db/factory/scenarios.ts` so
      `runComplexShelf` emits only contract-valid `parentRole` values.
- [ ] 1.2 Replace the current linear chain with a deterministic fixture shape:
...
- [ ] 2.2 Update `package/server/src/shelf/shelf.service.ts`
      ...
      root.
- [ ] 2.3 Make the cursor predicate resolve only root rows. If a stale or child
      cursor is supplied, fail safely rather than using a child position as the
      root boundary.
- [ ] 2.4 Keep child row loading scoped to the visible root item identities,
      preserving relation DTOs for the visible page.
- [ ] 2.5 Update `package/api/src/shelf/shelf.queries.ts` so
      `shelfItemsInfiniteQuery` uses `lastPage.nextCursor` and never infers from
      `lastPage.items.at(-1)`.
- [ ] 2.6 Add server tests in `package/server/src/shelf/shelf.service.test.ts`
      for root cursor behavior when child rows sort after the roots.
- [ ] 2.7 Add API query tests or focused contract tests covering
      `nextCursor`/`hasMore` shape and backwards-compatible pages without a next
      cursor.

## 3. Detail and editor pagination semantics

- [ ] 3.1 Update `package/app/src/shelf/pages/ShelfPage.tsx` so local numbered
      pages are computed from renderable root/stream pages, not blindly from
      `shelf.itemCount`.
- [ ] 3.2 Ensure `ShelfPage` requests more server pages until the selected local
      page has enough renderable entries, and stops cleanly when `hasNextPage`
      is false.
- [ ] 3.3 Reset shelf detail pagination on search, readable filter, sort mode,
      and view mode changes without duplicate effects.
- [ ] 3.4 Update `package/app/src/shelf/sections/ShelfEditorItemsSection.tsx`
      with the same root/rendered-stream pagination rule.
- [ ] 3.5 Revisit `useShelfItemsEditor` background loading in
...
- [ ] 3.6 Add tests around `deriveShelfStream` or new pagination helpers so
      nested roots with children do not inflate visible page count.

## 4. Nested child visibility and edit UX

- [ ] 4.1 Replace `attachmentCountsForEntry` in
...
- [ ] 4.2 Make read-mode nested roots expose child presence even when the child
      is `variant`, `comment`, `annotation`, or another supported non-review
      role.
- [ ] 4.3 Update nested tabs or grouped child sections so labels are i18n-backed
      and role-specific instead of defaulting every non-variant child to review.
- [ ] 4.4 Update edit-mode `UnitCard` summaries or row chrome so a root with any
      child relation visibly shows that fact.
- [ ] 4.5 Add focused tests for `ShelfItemRenderer`/summary helpers or
      `UnitCard` summary mapping so non-review children are represented.
- [ ] 4.6 Audit `canReorderShelfStreamEntry` behavior for nested mode; keep
      root-only reorder if deliberate, but make child relation editing
      discoverable elsewhere.

## 5. Formal bookshelf view mode

- [ ] 5.1 Add `bookshelf` to persisted view-mode normalization in
      `package/app/src/shelf/pages/ShelfPage.tsx` and
      `package/app/src/shelf/pages/ShelfEditPage.tsx`.
- [ ] 5.2 Add `bookshelf` to the read-page and default-view picker options,
      using i18n keys rather than hard-coded labels.
- [ ] 5.3 Add locale keys under `package/i18n/locales/*/entity.json`,
      including Traditional Chinese `shelf_view_bookshelf: "書架"`.
- [ ] 5.4 Decide and implement the bookshelf data policy in
...
- [ ] 5.5 Prefer the existing `package/app/src/bookshelf-view` grid behavior for
      formal bookshelf rendering where it fits; avoid a second incompatible
      cover-grid implementation.
- [ ] 5.6 Update `ShelfItemRenderer` bookshelf links so library kinds route to
      the correct detail page, not always `/book/:id`.
- [ ] 5.7 Add tests for view-mode normalization and bookshelf rendering/fallback
      behavior.

## 6. Shelf UX consistency checks

- [ ] 6.1 Review shelf list, shelf search, shelf-by-book, profile shelf-content
      search, and add-to-shelf dialog pagination patterns for consistency after
      the root-safe cursor change.
- [ ] 6.2 For profile shelf-content search, either add an explicit load-more
      action or document why the current “more exists” notice is intentionally
      non-actionable.
- [ ] 6.3 Ensure all touched frontend display strings go through
      `@rezics/i18n`; do not add hard-coded product copy in React components.
- [ ] 6.4 Add or update TSDoc layout notes only for visual components whose
      layout materially changes.
- [ ] 6.5 Run focused tests for contract, server shelf service, API shelf
