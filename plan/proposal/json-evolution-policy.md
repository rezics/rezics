...
- [x] 1.1 Create `package/contract/src/envelope/envelope.ts` with shared
...
- [x] 1.2 Re-express `package/contract/src/zone/upgrade.ts` through the shared
...
- [x] 1.3 Add tests for upgrade-chain purity and trust-on-read behavior:
...
- [x] 2.1 Add an explicit JSON column classification registry used by
...
- [x] 2.2 Add a convention rule that scans server and auth DB schemas for JSON
...
- [x] 2.3 Add the compatible JSON checks that are mechanically reliable:
...
- [x] 2.4 Add a grep-level convention check that forbids in-database JSON
      mutation helpers such as `jsonb_set` in server source.
- [x] 2.5 Transition: existing unclassified columns may be listed in a
...
- [x] 3.1 Add `tool/src/commands/backfill/` with a resumable CLI skeleton:
...
- [x] 3.2 Expose `task backfill` from the root `Taskfile.yml`; include a
      verification subcommand that reports stored row counts by version.

## 4. Policy Comment Landing Points

- [x] 4.1 Update `package/server/src/db/schema/columns.ts` `jsonData()` JSDoc
      with a short pointer to the envelope module, JSON column registry, and
      convention rule.
- [x] 4.2 Update `package/server/src/db/schema/unit.ts` UnitType comments and
