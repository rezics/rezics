...
- [x] 1.1 Add a shared source-CDC table manifest in a code-owned package or
  module that can be imported by server diagnostics and local service tooling.
- [x] 1.2 Update `package/server/src/diagnostic/system-status.service.ts` to use
  the shared manifest instead of its local `ROUTED_SEQUIN_TABLES`.
- [x] 1.3 Update `tool/src/commands/service/source.ts` to use the same manifest
  instead of its local `TRACKED_TABLES`, while keeping local repair behavior
  scoped to development.
- [x] 1.4 Reconcile `package/job-runner/sequin/sequin.yml` with
  `package/job-runner/src/sequin/router.ts`, including `ContentStructureNode`
  if that route remains active.
- [x] 1.5 Add tests that compare router-handled source tables, the shared
  manifest, and the checked-in Sequin YAML source table list.
- [x] 1.6 Update the recovery report or operations docs only where needed to
  point operators at the code-owned manifest as the source of truth.

## 2. Diagnostic Model

- [x] 2.1 Extend `package/server/src/diagnostic/status.types.ts` and
...
- [x] 2.2 Update `getCdcStatus` to query `max_replication_slots`,
  `max_wal_senders`, `pg_stat_replication`, and publication extra tables.
- [x] 2.3 Update `getHistoryOutboxStatus` to expose pending age buckets, recent
  pending rows, retry-ready failed rows, and a degraded state for suspicious
  pending backlog.
- [x] 2.4 Correlate HistoryOutbox status with queue status enough to identify
  `pending outbox but no history.ingest work` as a distinct warning.
- [x] 2.5 Add server diagnostic tests for slot missing, slot inactive, lag high,
  walsender pressure, publication table drift, pending outbox backlog, and
  failed queue jobs.
- [x] 2.6 Keep diagnostic responses safe: no database URLs, secrets, SQL text
  with credentials, or raw error messages that could leak tokens.

## 3. Admin Status UI

- [x] 3.1 Update `package/admin/src/system-health/pages/StatusCdcPage.tsx` so
...
- [x] 3.3 Remove or avoid duplicate Sequin rows in services/system summary
  rendering so status counts and panels do not double-count the same service.
- [x] 3.4 Add focused admin UI states for the recovery report scenarios:
  healthy, Sequin down, slot missing, publication drift, pending outbox missed
  by CDC, failed history job, and queue down.
- [x] 3.5 Add Storybook or component-level coverage for the focused status
  panels using safe fixture summaries.

## 4. Repair Contract and API

- [x] 4.1 Split `AdminRepairJobScope` so failed pg-boss job retry and
  HistoryOutbox replay are separate scopes, for example `queue-failed-job` and
  `history-outbox-replay`.
- [x] 4.2 Update `package/contract/src/admin/repair-job.ts` so dry-run can
  distinguish `affectedCount`, bounded `sampleTargets`, and the exact target
  set used by `start`.
- [x] 4.3 Add a repair dry-run for missed `HistoryOutbox` rows that can filter
  by status, age, unit id, limit, and explicit outbox ids.
- [x] 4.4 Implement start for HistoryOutbox replay by enqueueing
  `createHistoryOutboxIngestCommand(outboxId)` through `serverJobProducer`.
- [x] 4.5 Preserve existing failed job retry/cancel behavior under the renamed
  queue failed-job scope.
- [x] 4.6 Add privileged audit entries for dry-run/start/retry/cancel with
  scope, target count, reason, correlation id, and safe operation summaries.
- [x] 4.7 Add server repair tests for missed outbox replay, idempotent enqueue
  coalescing, sample-limited dry-runs, explicit target selection, and failed
  job retry after the scope rename.

## 5. Admin Repair UI

- [x] 5.1 Update `package/admin/src/repair/pages/RepairJobsPage.tsx` to show
  separate repair cards for queue failed jobs and HistoryOutbox replay.
- [x] 5.2 Make sample-limited dry-run behavior explicit in the UI and prevent
  accidentally repairing only the first sample when the operator intended all
  affected targets.
- [x] 5.3 Add target controls for HistoryOutbox replay: status filter, age
  threshold, optional unit id, explicit ids, and max enqueue count.
- [x] 5.4 Link `/status/cdc`, `/status/history`, and `/status/queue` warnings
  to prefilled repair scope/search state where safe.
- [x] 5.5 Keep destructive or deployment-level Sequin controls out of the first
  UI pass; show runbook guidance and Sequin UI links instead.

## 6. Validation

- [x] 6.1 Run targeted tests for diagnostic status, repair job service, job
  command creation, and admin status/repair components.
- [x] 6.2 Run `bun run check:convention` after API/domain changes.
- [x] 6.3 Run `bun run check:tokens` if admin JSX class changes touch styling.
- [ ] 6.4 Manually verify in admin after `bun run dev`: `/status`,
  `/status/cdc`, `/status/history`, `/status/queue`, and `/repair` for healthy
  and degraded fixture/local scenarios. Left unchecked because it requires
