...
- [ ] 1.1 Remove the Paraglide legacy import scanner from
...
- [ ] 2.2 Update `package/server/src/chapter/chapter.service.ts` and
  `package/server/src/chapter/chapter.materialization.test.ts` to return and
  assert `contentUnitId` only.
- [ ] 2.3 Update frontend API/app consumers, especially
...
- [ ] 4.4 Remove legacy row counts, row samples, and mismatch candidates from
  `package/server/src/game-media-library/admin-readiness.ts` and its tests.
- [ ] 4.5 Search `package/server`, `package/api`, `package/contract`,
...
- [ ] 5.1 Remove `legacyChangedKeys` from
  `package/contract/src/content/history.ts` and update contract/API tests for
  history payloads.
- [ ] 5.2 Remove `LegacyEditorialRevisionPayload`, `slots` handling, and
...
- [ ] 5.3 Remove tests that assert legacy `book.contentStructure.batch` event
  readability; current structure events should use
  `contentStructure.content.batch`.
- [ ] 5.4 Remove `HISTORY_OUTBOX_POLLER_FALLBACK` from
...
- [ ] 5.5 Update docs in `package/history/README.md`,
...
- [ ] 6.1 Remove "legacy wire shape" wording from
...
- [ ] 6.2 Audit `BookContentStructureItem` / `BookContentStructureResponse`
...
- [ ] 6.3 Keep normal content-structure restore semantics, including
  `fallbackToRoot`, because that is current product behavior rather than legacy
  compatibility.

## 7. Final cleanup pass and checks

- [ ] 7.1 Run targeted `rg` scans for `legacy`, `deprecated`,
...
- [ ] 7.2 Run relevant focused tests for changed packages, then
  `bun run check:convention` and `bun run format:check`.
- [ ] 7.3 Regenerate Prisma client for `@rezics/server` after schema changes and
  run impacted server/history/tool tests.
- [ ] 7.4 Update this plan's completed tasks during apply. Set `status: done`
