...
import type { MarkdownApplicationCommand, MarkdownNativeMenuHost } from "./domain/application-menu";
...
import {
	...
	readStoredMarkdownLocale,
	resolveMarkdownEditorLocale,
	writeStoredMarkdownLocale,
...
export interface MarkdownEditorAppProps {
	...
	readonly nativeMenu?: MarkdownNativeMenuHost;
...
export function MarkdownEditorApp({
	...
	const [locale, setLocale] = useState<MarkdownEditorLocale>(
		() =>
			initialLocale ??
			(typeof localStorage === "undefined" ? undefined : readStoredMarkdownLocale(localStorage)) ??
			resolveMarkdownEditorLocale(
	...
	const runCommand = (command: MarkdownApplicationCommand): void => {
		switch (command) {
			case "new-document":
	...
	const changeLocale = (next: MarkdownEditorLocale): void => {
		setLocale(next);
		if (typeof localStorage !== "undefined") writeStoredMarkdownLocale(localStorage, next);
	};
