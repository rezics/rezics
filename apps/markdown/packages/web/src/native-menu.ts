...
function commandItem(
	id: MarkdownApplicationCommand,
	text: string,
	onCommand: (command: MarkdownApplicationCommand) => void,
	accelerator?: string,
) {
	return {
		id,
...
async function attachLocalizedWindowMenu(
	messages: Parameters<MarkdownNativeMenuHost["install"]>[0]["messages"],
	onCommand: (command: MarkdownApplicationCommand) => void,
): Promise<void> {
...
export const tauriNativeMenuHost: MarkdownNativeMenuHost = {
	install: async ({ messages, onCommand }) => {
		...
		const unlisten = await listen<string>("markdown-menu", (event) => {
			if (isMarkdownApplicationCommand(event.payload)) onCommand(event.payload);
		});
		return unlisten;
	},
};
