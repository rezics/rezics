...
export function PreferencesWorkspace({
	...
	readonly onLocaleChange: (locale: RezicsTextLocale) => void;
								...
								<FieldGroup className="max-w-md gap-4">
									<Field>
										...
										<ChoiceSelect
											...
											onValueChange={(value) => {
												const next = value[0];
												if (isRezicsTextLocale(next)) onLocaleChange(next);
											}}
											options={rezicsTextLocales.map((availableLocale) => ({
									...
									<Field>
										...
										<ChoiceSelect
											...
											onValueChange={(value) => {
												const next = value[0];
												if (isRezicsTextThemePreference(next)) onThemePreferenceChange(next);
											}}
											options={rezicsTextThemePreferences.map((preference) => ({
												value: preference,
