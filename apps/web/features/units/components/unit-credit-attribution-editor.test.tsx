	...
	return {
		...
		EntityPicker: ({
			...
			<actual.Button
				...
				data-index={index}
				data-search-on-open={searchOnOpen || undefined}
				onClick={() =>
					void search?.(index, searchOnOpen ? "" : "Studio", new AbortController().signal)
				}
				type="button"
			/>
		),
...
describe("UnitCreditAttributionEditor", () => {
	it("renders a titled credit section with the first role selected", () => {
	...
	it("separates direct-permission and public Entity searches", () => {
		...
		expect(personal.search).toHaveBeenCalledWith("entities", "", expect.any(AbortSignal), {
		...
		fireEvent.click(screen.getByRole("button", { name: "署名實體 1" }));
		expect(publicEditor.search).toHaveBeenCalledWith("entities", "", expect.any(AbortSignal), {
