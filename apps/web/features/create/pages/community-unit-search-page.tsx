...
type CommunityUnitSearchHit = PostApiSearchByIndexStatus200["hits"][number];

type CommunityUnitSearchState =
	| { readonly status: "idle" }
	| {
			readonly hits: readonly CommunityUnitSearchHit[];
			readonly query: string;
			readonly status: "ready";
	  }
	| { readonly query: string; readonly status: "error" };

export function CommunityUnitSearchPage({
	initialQuery,
	subject,
	unitTagVoteTarget,
}: {
	...
	const resultItems =
		displayedState.status === "ready"
			? displayedState.hits.map((hit) => ({
					avatar: hit.avatar,
					href: communityUnitSearchResultHref(subject, hit.id),
					id: hit.id,
					summary: hit.summary,
