...
import { entityPublicEntrySearchSubject } from "../model/public-entry-search";
import { PublicEntrySearchPrompt } from "./public-entry-search-prompt";

describe("PublicEntrySearchPrompt", () => {
	it("makes the exact Entity search prompt a link with the policy in its hover card", () => {
		render(
			<PublicEntrySearchPrompt
				confirmed={false}
				query="OpenAI"
				subject={entityPublicEntrySearchSubject("organization")}
			/>,
		);
