...
describe("Vike prerender output", () => {
	...
	test("contains SEO, MDX 404, discovery, redirects, and Pages-only assets", async () => {
		const product = await readFile(join(dist, "en", "products", "book", "index.html"), "utf8");
		expect(product).toContain("application/ld+json");
		expect(product).toContain("SoftwareApplication");
		...
		expect(await exists(join(dist, "_redirects"))).toBe(true);
