# Rezics ranking worker (ranking lane) — 排名工作进程（排名通道）
  ...
  group "worker" {
    ...
    task "worker" {
      ...
      env {
        ...
        RANKING_BASE_URL         = "http://127.0.0.1:3006"
      }
...
RANKING_INTERNAL_SECRET={{ .RANKING_INTERNAL_SECRET }}
{{ end }}
EOT
