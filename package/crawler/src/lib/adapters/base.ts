...
export const make_strategy =
    ...
    async (driver, url) => {
        const domainValidation =
            url.hostname === domain ? Either.right(url) : Either.left(`Domain must be ${domain}, got ${url.hostname}`);
...
export const make_test = (strategy: Strategy, url: string) => async (): Promise<string> => {
    try {
