...
- `tags`:
  - `.info-label` → iterate `children` as `HTMLElement` and collect `.innerText`.
- `units` (volumes/chapters):
  - For each `.volume` element, evaluate in-page:
    - Reduce over `volumeElement.parentElement!.children`:
      - When `cur.classList.contains('chapter')`, iterate its children and push `{ title: chapter.innerText }` to `acc.children`.
      - Otherwise ignore.
    - Seed accumulator `{ title: volumeElement.innerText, children: [] }`.
- `completed`:
  - `tags.some(tag => tag.includes('完结'))`.
- `length`:
  - `.info-count-word .detail` innerText is a number of 万 (ten-thousands). Multiply by `1_0000`.
- `last_update`:
...
### Notes
- Chapter list is not strictly nested under each volume; the code navigates sibling structures to gather chapters belonging to the preceding `.volume`.
