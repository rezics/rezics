...
import { buildMetadataByType, buildUnitTitleByType } from "./generators.js";
...
import { generateParagraph, pickN, randomBoolean, randomInt } from "./utils.js";
      ...
      for (let i = 0; i < countForType; i++) {
        ...
        if (type === UnitTypeEnum.REMARK || type === UnitTypeEnum.REVIEW) {
          ...
          await prisma.rating.upsert({
            where: {
              unitId_domain: { unitId: bookUnitId, domain: bookUnitId },
            },
            update: {
              domain: bookUnitId,
              totalScore: {
