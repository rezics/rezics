...
const serverPool = new Pool({
  connectionString: readEnv("package/server/.env", "DATABASE_URL"),
});
const authPool = new Pool({
  connectionString: readEnv("package/auth/.env", "DATABASE_URL"),
});
const notifyPool = new Pool({
  connectionString: readEnv("package/notify/.env", "NOTIFY_DATABASE_URL"),
});
const historyPool = new Pool({
  connectionString: readEnv("package/history/.env", "HISTORY_DATABASE_URL"),
});
const rankingPool = new Pool({
  connectionString: readEnv("package/ranking/.env", "RANKING_DATABASE_URL"),
});
