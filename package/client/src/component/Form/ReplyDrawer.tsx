...
export namespace ReplyDrawer {
	...
	export const Show: React.FC<Show> = (
		{ open, onClose, onSubmit, content, onContentChange },
	) => {
		return (
			<Drawer
				open={open}
	...
	export const Container: React.FC<Container> = ({ dialogId, onSubmit }) => {
		const entry = useDialogStore((state) => state.dialogs[dialogId]);
		const setDialogVisible = useDialogStore((state) =>
			state.setDialogVisible
		);
		const setDialogContent = useDialogStore((state) =>
