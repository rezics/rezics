...
本文档介绍了基于 `@material/material-color-utilities` 实现的动态主题系统，该系统能够根据用户选择的种子颜色自动生成协调的 Material Design 3 配色方案。
...
```typescript
function generateDynamicColors(sourceColor: string, isDark: boolean): DynamicColorScheme
```
根据种子颜色生成完整的动态颜色方案。
...
```typescript
function dynamicColorsToPalette(colors: DynamicColorScheme, mode: "light" | "dark"): PaletteOptions
```
...
import { generateDynamicColors, applyDynamicThemeToDOM } from "./config/dynamicTheme";
...
通过这个动态主题系统，用户可以轻松地自定义应用的外观，而开发者则可以享受到 Material Design 3 带来的现代化设计体验。