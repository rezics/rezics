    ...
    return (
      <div className="flex flex-col items-center gap-1">
        <span className="text-xs font-bold tracking-[0.12em] uppercase text-white/65">
          {t("book:hero_score_rezics_label")}
        </span>
        {hasRating ? (
            ...
            <div className="flex flex-col items-start leading-tight">
              ...
              <span className="text-xs text-white/55 tabular-nums">
                {typeof count === "number" && count > 0
                  ? t("book:hero_score_count", { count })
                  : t("book:hero_score_empty_short")}
              </span>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 min-h-[2.25rem] text-white/70">
            ...
            <div className="flex flex-col items-start leading-tight">
              <span className="text-base font-medium text-white/80">
                {t("book:hero_score_empty")}
              </span>
              <span className="text-xs text-white/45">
                {t("book:hero_score_be_first")}
              </span>
            </div>
          </div>
        )}
    ...
    return (
      <div className="flex flex-col items-end gap-1 text-white">
        <span className="text-[0.7rem] uppercase tracking-wider text-white/60">
          {t("book:hero_score_label")}
        </span>
        <RatingInput
          value={null}
          onChange={(v) => onRate?.(v ?? 0)}
          readOnly={!onRate}
          aria-label={t("book:hero_score_label")}
        />
