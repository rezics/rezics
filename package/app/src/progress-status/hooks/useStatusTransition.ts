...
export function useStatusTransition(
  ...
  const runShelfOp = async (op: ShelfOp) => {
  ...
  const dispatchShelfOps = async (ops: ShelfOp[]) => {
  ...
  const handlePartialFailure = (
    ...
    if (progressFailed && shelfFailed) {
      showRetryToast(
        `${unitId}:both`,
        getI18nRuntime().i18n.t("community:progress_status_toast_both_failed"),
        async () => {
      ...
      showRetryToast(
        `${unitId}:shelf`,
        getI18nRuntime().i18n.t("community:progress_status_toast_shelf_failed"),
        retryShelf,
      );
    }
  };
