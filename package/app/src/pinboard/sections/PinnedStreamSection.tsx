...
export interface PinnedStreamSectionProps {
  realmUnitId: string;
  /**
...
/**
 * Renders the pinned region above a realm stream. No-ops when the list is
 * empty, silent on errors in the stream-adjacent position to avoid crowding
 ...
 * +------------------------------------------------------------+
 *
 * 在 realm stream 上方渲染置顶区域。列表为空时不渲染任何内容；在紧邻 stream 的位置
 * 对错误保持静默，以免拥挤视口——错误仍会通过骨架屏回退状态显现。卡片宽度
 * 由 carousel item basis 静态决定；窄屏以横向滚动处理不足宽度，宽屏以 30% /
 * 24% 卡片保留内容密度。
 */
export const PinnedStreamSection: React.FC<PinnedStreamSectionProps> = ({
  realmUnitId,
  linkFor,
}) => {
