...
// or `/z/<slug>/search` when scope is a slug route (short slug-prefix) and
// `/user/<unitId>/search` or `/zone/<unitId>/search` otherwise.
...
export function buildHeaderSubmitPath(pathname: string, value: string): string {
  ...
  if (scope.kind === "zoneSlug") {
    const params = new URLSearchParams();
    if (value) params.set("q", value);
    const qs = params.toString();
    return `/z/${scope.zoneSlug}/search${qs ? `?${qs}` : ""}`;
  }
