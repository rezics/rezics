...
import { DynamicColorScheme, dynamicColorsToPalette, generateDynamicColors } from "./dynamicTheme";

/**
 * 根据 mode（'light' | 'dark'）返回对应的颜色、组件覆盖等设计 Token
 */
const getDesignTokens = (mode: "light" | "dark", customColor?: string): ThemeOptions => ({
  palette: {
    mode,
    ...(mode === "light"
      ? {
  ...
  components: {
    ...
    MuiAppBar: {
      styleOverrides: {
        root: {
          backgroundColor: mode === "light" ? "#f4606c" : "#af3b45",
        },
      },
    },
  },
 ...
 */
export const getTheme = (mode: "light" | "dark", customColor?: string): Theme => {
