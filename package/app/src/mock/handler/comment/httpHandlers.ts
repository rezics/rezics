...
function pickRandomUser() {
  return mockUsers[Math.floor(Math.random() * mockUsers.length)] ?? { id: "1", name: "John Doe", avatar: "" };
}
...
export const commentHttpHandlers = [
  ...
  http.get("/api/comments/by-depth", ({ request }) => {
    ...
    if (!rootPostId) return HttpResponse.json({ items: [], offset: 0, totalItems: 0 }, { status: 200 });
    ensureSeedForRoot(rootPostId);
    const list = (commentStore.get(rootPostId) ?? []).filter((c) => Number(c.depth) === Number(depth));
