...
export const readlistHttpHandlers = [
  ...
  http.get("/api/readlists/:id", () => HttpResponse.json({ ...mockABookList01 }, { status: 200 })),
  ...
  http.post("/api/readlists", async ({ request }) => {
    ...
    const newList = {
      ...
      creator: { name: "Mock User", avatar: "https://api.dicebear.com/9.x/pixel-art/svg?seed=user" },
      likes: 0,
    } as any;
  ...
  http.patch("/api/readlists/:id", async ({ params, request }) => {
    ...
    return HttpResponse.json({ id: String((params as any).id), ...(body as any) }, { status: 200 });
  }),

  // DELETE /api/readlists/:id
  http.delete("/api/readlists/:id", () => HttpResponse.json({}, { status: 204 })),
];
