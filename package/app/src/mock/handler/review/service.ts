...
function joinReview(row: ReviewRow, users: User[]): Review {
  const user =
    users.find(u => String(u.id) === String(row.userId)) ??
...
export function getReviewById(id: Id): Review | undefined {
  const row = (Array.isArray(mockReviews) ? mockReviews : []).find(
...
export function listQuotes(opts: {limit?: number; bookId?: Id}): Quote[] {
  ...
  const filtered = bookId
    ? source.filter(q => String(q?.bookId ?? '') === String(bookId))
    : source;
...
export function listCommentsByBook(opts: {bookId: Id; limit?: number}) {
  const {limit = 5} = opts ?? {};
  const source = Array.isArray(mockBookShortReviews)
    ? mockBookShortReviews
    : [];
...
export function createReview(payload: {
  ...
  const user =
    mockUsers[0] ?? ({id: '1', name: 'John Doe', avatar: ''} as User);
  const row: ReviewRow = {
    id,
