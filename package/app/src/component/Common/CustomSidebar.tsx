...
export function CustomSidebar({ section, setSection }: { section: string; setSection: (section: string) => void }) {
  return (
          ...
          {[
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div
                    className={`font-semibold ${section === item.id ? "text-white" : "text-gray-900"}`}
                  >
                    {item.label}
                  </div>
                  <div
                    className={`text-sm ${section === item.id ? "text-rose-100" : "text-gray-500"}`}
                  >
                    {item.id === "Profile"
