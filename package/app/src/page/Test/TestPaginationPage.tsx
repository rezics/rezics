    ...
    return (
        <Paper elevation={1} sx={{ p: 2, mb: 2, borderRadius: 2 }}>
            <Grid container spacing={2} alignItems="center">
                ...
                <Grid sx={{ xs: 12, sm: "auto" }}>
                    <ToggleButtonGroup value={sortOrder} exclusive onChange={(_, v: 'asc' | 'desc' | null) => v && onSortChange({ order: v })}>
...
export default function TestPaginationPage() {
    const [sortConfig, setSortConfig] = useState<{ type: 'time' | 'name' | 'popular' | 'agree'; order: 'asc' | 'desc' }>({
        type: "popular",
        order: "desc",
    });
    ...
    const fetchDataBlock = useCallback(async (page: number, currentSort: { type: 'time' | 'name' | 'popular' | 'agree'; order: 'asc' | 'desc' }) => {
        setIsLoading(true);
        try {
            const query: PaginationQuery = { page, limit: EXTERNAL_PAGE_SIZE, type: currentSort.type, order: currentSort.order };
    ...
    const handleSortChange = (newSort: { type?: string; order?: 'asc' | 'desc' }) => {
        setSortConfig((prev) => ({
            type: (newSort.type && ["time", "name", "popular", "agree"].includes(newSort.type)) ? newSort.type as 'time' | 'name' | 'popular' | 'agree' : prev.type,
