...
export const safeArray = {
   ...
   */
  lookup: <T>(index: number) => (arr: ReadonlyArray<T>): O.Option<T> => A.lookup(index)(arr),
  
  /**
   * Find element that satisfies predicate
   */
  find: <T>(predicate: (item: T) => boolean) => (arr: ReadonlyArray<T>): O.Option<T> => 
...
export const nullable = {
   ...
   */
  map: <T, U>(f: (value: T) => U) => (value: T | null | undefined): U | null => 
...
export const validation = {
   ...
   */
  tryCatch: <T>(f: () => T): E.Either<Error, T> => E.tryCatch(f, (error) => error instanceof Error ? error : new Error(String(error))),
  
  /**
   * Chain multiple validations
   */
  chain: <E, A, B>(f: (a: A) => E.Either<E, B>) => (either: E.Either<E, A>): E.Either<E, B> => 
    E.chain(f)(either),
  
  /**
   * Fold Either into a single value
   */
  fold: <E, A, B>(onLeft: (e: E) => B, onRight: (a: A) => B) => (either: E.Either<E, A>): B => 
...
export const stringUtils = {
   ...
   */
  split: (separator: string | RegExp) => (str: string): ReadonlyArray<string> => 
    pipe(str.split(separator), A.filter(s => s.length > 0)),
  
  /**
   * Trim and convert to Option (None if empty)
   */
  trimToOption: (str: string): O.Option<string> => 
    pipe(str.trim(), s => s.length > 0 ? O.some(s) : O.none),
  
  /**
   * Check if string matches pattern
   */
  matches: (pattern: RegExp) => (str: string): boolean => pattern.test(str),
};
...
export const objectUtils = {
   ...
   */
  prop: <T, K extends keyof T>(key: K) => (obj: T): O.Option<T[K]> => 
    O.fromNullable(obj[key]),
  
  /**
   * Update object property immutably
   */
  updateProp: <T, K extends keyof T>(key: K, value: T[K]) => (obj: T): T => 
    ({ ...obj, [key]: value }),
  
  /**
   * Pick specific properties from object
   */
  pick: <T, K extends keyof T>(keys: ReadonlyArray<K>) => (obj: T): Pick<T, K> => 
...
export const asyncUtils = {
   ...
   */
  fromPromise: <T>(promise: Promise<T>) => (): Promise<E.Either<Error, T>> => 
